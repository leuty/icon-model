...
MODULE mo_time_config
  ...
  USE mtime,                    ONLY: datetime, timedelta, newDatetime, newTimedelta, &
    ...
    &                                 MAX_CALENDAR_STR_LEN, MAX_TIMEDELTA_STR_LEN,    &
  ...
  TYPE t_timeshift
    REAL(wp)                 :: dt_shift    = 0._wp   ! in s
    TYPE(timedelta), POINTER :: mtime_shift => NULL()
  ...
  TYPE t_time_config
    ...
    ! time increment, by which tc_current_date is time-shifted.
    ! Since 'timeshift' must be negative (or zero), tc_current_date will be
    ! shifted backwards in time. This is required for the Incremental Analysis
    ! Update (IAU) procedure.
    ! Please note that, in contrast to most other elements of t_time_config,
    ! this is a component-specific information.

    TYPE(t_timeshift) :: timeshift
  CONTAINS
    !
...
CONTAINS
  ...
  SUBROUTINE time_config__copy(me, tc_new)
    ...
    TYPE(t_time_config), INTENT(OUT) :: tc_new

    tc_new%tc_exp_refdate        => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_exp_startdate      => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_exp_stopdate       => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_startdate          => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_stopdate           => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_current_date       => newDatetime('0001-01-01T00:00:00')
    tc_new%tc_dt_checkpoint      => newTimedelta("PT0S")
    tc_new%tc_dt_restart         => newTimedelta("PT0S")
    tc_new%tc_dt_model           => newTimedelta("PT0S")
    tc_new%timeshift%mtime_shift => newTimedelta("PT0S")
    ...
    tc_new%timeshift%dt_shift    = me%timeshift%dt_shift
    tc_new%timeshift%mtime_shift = me%timeshift%mtime_shift
  ...
  SUBROUTINE time_config__destruct(me)
    ...
    IF (ASSOCIATED(me%tc_exp_refdate))  CALL deallocateDatetime(me%tc_exp_refdate)
    NULLIFY(me%tc_exp_refdate)
    !
    IF (ASSOCIATED(me%tc_exp_startdate)) CALL deallocateDatetime(me%tc_exp_startdate)
    NULLIFY(me%tc_exp_startdate)
    !
    IF (ASSOCIATED(me%tc_exp_stopdate))  CALL deallocateDatetime(me%tc_exp_stopdate)
    NULLIFY(me%tc_exp_stopdate)
    !
    IF (ASSOCIATED(me%tc_startdate))  CALL deallocateDatetime(me%tc_startdate)
    NULLIFY(me%tc_startdate)
    !
    IF (ASSOCIATED(me%tc_stopdate))  CALL deallocateDatetime(me%tc_stopdate)
    NULLIFY(me%tc_stopdate)
    !
    IF (ASSOCIATED(me%tc_current_date))  CALL deallocateDatetime(me%tc_current_date)
    NULLIFY(me%tc_current_date)
    !
    IF (ASSOCIATED(me%tc_dt_checkpoint))  CALL deallocateTimedelta(me%tc_dt_checkpoint)
    NULLIFY(me%tc_dt_checkpoint)
    !
    IF (ASSOCIATED(me%tc_dt_restart))  CALL deallocateTimedelta(me%tc_dt_restart)
    NULLIFY(me%tc_dt_restart)
    !
    IF (ASSOCIATED(me%tc_dt_model))  CALL deallocateTimedelta(me%tc_dt_model)
    NULLIFY(me%tc_dt_model)
    !
    IF (ASSOCIATED(me%timeshift%mtime_shift))  CALL deallocateTimedelta(me%timeshift%mtime_shift)
    NULLIFY(me%timeshift%mtime_shift)
  ...
  ! set time increment, by which tc_current_date is shifted
  ! backwards in time at model start.
  SUBROUTINE set_tc_timeshift(dt_shift)
    REAL(wp), INTENT(in) :: dt_shift   ! in s
    ! local
    ...
    ENDIF
    !
    IF (ASSOCIATED(time_config%timeshift%mtime_shift)) THEN
      CALL deallocateTimedelta(time_config%timeshift%mtime_shift)
    END IF
    !
    time_config%timeshift%dt_shift = dt_shift
    CALL getPTStringFromSeconds(dt_shift, dt_shift_string)
    time_config%timeshift%mtime_shift => newTimedelta(dt_shift_string)
    !
    WRITE(message_text,'(a,a)') 'Set model timeshift: ', TRIM(dt_shift_string)
