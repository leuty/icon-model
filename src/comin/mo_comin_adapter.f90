...
MODULE mo_comin_adapter
  ...
  USE comin_host_interface,       ONLY : t_comin_var_handle,                      &
    ...
    &                                    comin_request_list_iterator_begin,       &
  ...
  USE mo_util_vcs,                ONLY: get_remote_url, &
       ...
       &                                get_revision,   &
       &                                get_icon_version
  USE iso_c_binding,              ONLY: C_LOC, C_PTR, C_NULL_PTR, C_DOUBLE, C_FLOAT, C_F_POINTER, &
    &                                   C_INT, C_BOOL, C_ASSOCIATED, c_char, c_size_t
#ifdef _OPENACC
...
CONTAINS
  ...
  SUBROUTINE set_variable_descs_from_comin_metadata(metadata, ibits, cf_desc, grib2_desc)
    TYPE(t_comin_var_metadata), INTENT(INOUT) :: metadata
    INTEGER, INTENT(IN) :: ibits
    TYPE(t_cf_var), INTENT(OUT) :: cf_desc
    TYPE(t_grib2_var), INTENT(OUT) :: grib2_desc

    CHARACTER(LEN=:), ALLOCATABLE :: charbuff
    INTEGER :: comin_hgrid_id

    CALL comin_metadata_get_or(metadata, "units", charbuff, "")
    cf_desc%units         = charbuff
    CALL comin_metadata_get_or(metadata, "standard_name", charbuff, "")
    cf_desc%standard_name = charbuff
    CALL comin_metadata_get_or(metadata, "long_name", charbuff, "")
    cf_desc%long_name     = charbuff
    CALL comin_metadata_get_or(metadata, "short_name", charbuff, "")
    cf_desc%short_name    = charbuff
    CALL comin_metadata_get_or(metadata, "grib_discipline", grib2_desc%discipline, 255)
    CALL comin_metadata_get_or(metadata, "grib_category", grib2_desc%category, 255)
    CALL comin_metadata_get_or(metadata, "grib_number", grib2_desc%number, 255)
    CALL comin_metadata_get_or(metadata, "hgrid_id", comin_hgrid_id, COMIN_HGRID_UNSTRUCTURED_CELL)
  ...
  END SUBROUTINE set_variable_descs_from_comin_metadata
    ...
    DO WHILE (.NOT. comin_request_list_iterator_is_end(it))
      ...
      IF (.NOT. tracer) THEN
        IF (tracer_turb .OR. tracer_conv) THEN
          WRITE (message_text,'(A,A,A,L1,A,L1,A,L1)') 'Requested variable ', descr%name, &
      ...
      ALLOCATE(this_info)
      this_info%next => comin_config%comin_icon_domain_config(jg)%tracer_info_head
      comin_config%comin_icon_domain_config(jg)%tracer_info_head => this_info

      CALL comin_metadata_get_or(metadata, "hgrid_id",  hgrid_id, COMIN_HGRID_UNSTRUCTURED_CELL)
      IF (hgrid_id /= COMIN_HGRID_UNSTRUCTURED_CELL) THEN
        WRITE (message_text,'(A,A,A,L1,A,L1,A,L1)') 'Requested tracer ', descr%name, &
          &                             'with horizontal grid id ',hgrid_id,', which is not supported'
        CALL finish(routine, message_text)
      END IF

      CALL comin_metadata_get_or(metadata, "tracer_hadv",   ihadv,       MIURA      )
      CALL comin_metadata_get_or(metadata, "tracer_vadv",   ivadv,       ippm_v     )
      CALL comin_metadata_get_or(metadata, "tracer_hlimit", ihlimit,     ifluxl_sm  )
      CALL comin_metadata_get_or(metadata, "tracer_vlimit", ivlimit,     islopel_vsm)
      ! NetCDF and GRIB metadata
      CALL set_variable_descs_from_comin_metadata(metadata, ibits, cf_desc, grib2_desc)
      ...
      DO jt = 1, ntl
        ...
        tracer_name   = descr%name//TRIM(suffix)
  ...
  SUBROUTINE icon_append_comin_tracer_phys_tend(p_patch)
    ...
    DO WHILE (.NOT. comin_request_list_iterator_is_end(it))
      ...
      DO WHILE (ASSOCIATED(this_info))
        IF ( this_info%name == descr%name ) THEN
          EXIT
        END IF
      ...
      IF (.NOT. ASSOCIATED(this_info)) THEN
        WRITE (message_text,*) 'Internal error: tracer ', descr%name, &
            & ' on domain ', jg, ' has no tracer_info associated.'
        CALL finish(routine, message_text)
      END IF
      ...
      CALL comin_metadata_get_or(metadata, "tracer_turb", tracer_turb, .FALSE.)
      IF (tracer_turb) THEN
        name_tend   = "ddt_"//descr%name//"_turb"
        ...
        IF (msg_level >= 10) THEN
          WRITE (message_text,'(A,A,A,A,A,I2)') "Tendency due to turbulence '",TRIM(name_tend), &
            &                                   "' added for tracer '", descr%name, &
            &                                   "' in domain ",jg
          CALL message(routine, message_text)
        ENDIF

      END IF !metadata%tracer_turb

      ! Adding tendencies of tracers due to convection
      CALL comin_metadata_get_or(metadata, "tracer_conv", tracer_conv, .FALSE.)
      IF (tracer_conv) THEN
        name_tend   = "ddt_"//descr%name//"_conv"
  ...
  SUBROUTINE icon_append_comin_variables(p_patch)
    ...
    ! Create varlists for each domain.
    DO jg=1,n_dom
    ...
    DO WHILE (.NOT. comin_request_list_iterator_is_end(it))
      ...
      CALL comin_metadata_get_or(metadata, "datatype", datatype, default_datatype)
      CALL comin_metadata_get_or(metadata, "restart", restart, .FALSE.)
      CALL comin_metadata_get_or(metadata, "zaxis_id", comin_vgrid_id, COMIN_ZAXIS_3D)
      CALL set_variable_descs_from_comin_metadata(metadata, ibits, cf_desc, grib2_desc)

      SELECT CASE (grib2_desc%subgridtype)
      ...
      IF (msg_level >= 10) THEN
        WRITE (message_text,'(a,a,a,i2,a)') "add variable '", descr%name, &
          &  "', requested by third party plugins to domain", jg, "."
        CALL message(routine, message_text)
      ENDIF
    ...
    DO WHILE(vl_iter%next())
      ...
      DO iv = 1, vl_iter%cur%p%nvars
        ...
        ELSE
          pos_jcjkjb =  [1, 2, 3]
        END IF
        dim_semantics(:) = COMIN_DIM_SEMANTICS_UNUSED

        is_container = elem%info%lcontainer
        is_reference = elem%info%lcontained

        ! Irregular variables don't follow the rules above, so we cannot assign dimension semantics.
        is_irregular = .FALSE.
        IF (.NOT. is_container .AND. .NOT. is_reference) THEN
          IF (.NOT. is_2d_field .AND. elem%info%ndims /= 3) THEN
            is_irregular = .TRUE.
          ELSE IF (is_2d_field .AND. elem%info%ndims /= 2) THEN
            is_irregular = .TRUE.
          END IF
        END IF
        ...
        ELSE
          IF (is_container .OR. is_reference) THEN
            pos_jn = elem%info%var_ref_pos

            IF (is_container) THEN
              ! Containers may still have var_ref_pos = -1 if no pos was specified by the user.
              ! This replicates the default of add_ref, which assumes the last dimension.
              IF (pos_jn < 0) pos_jn = elem%info%ndims
              dim_semantics(pos_jn) = COMIN_DIM_SEMANTICS_CONTAINER !container_dimid=6XS
            END IF

            ! Shift the dimension positions to make room for the container dimension.
            IF (pos_jn <= pos_jcjkjb(1)) THEN
              pos_jcjkjb(1:3) = pos_jcjkjb(1:3) + 1
            ELSE IF (pos_jn <= pos_jcjkjb(2)) THEN
              pos_jcjkjb(2:3) = pos_jcjkjb(2:3) + 1
            ELSE IF (pos_jn <= pos_jcjkjb(3)) THEN
        ...
        ! Skip variables with noncontiguous memory.
        IF (is_reference .AND. .NOT. is_irregular) THEN
          DO ic = 5,1,-1
            IF (dim_semantics(ic) /= COMIN_DIM_SEMANTICS_UNUSED) EXIT
          END DO
          ! There is a used dimension behind the reference container dimension.
          ! The slice taken below will be noncontiguous.
          IF (elem%info%var_ref_pos < ic) CYCLE
        END IF
...
#ifdef _OPENACC
          device_ptr = acc_deviceptr(C_LOC(s_ptr))
#endif
...
#ifdef _OPENACC
          device_ptr = acc_deviceptr(C_LOC(i_ptr))
#endif
        ELSE
  ...
  SUBROUTINE icon_expose_descrdata(msg_level, n_dom, max_dom, nproma, min_rlcell_int,    &
    ...
    &                              start_time, end_time, time_config, dtime, l_limited_area)
    ...
    CALL comin_descrdata_set_simulation_interval(sim_start, sim_end, &
         & run_start, run_stop)
    CALL comin_current_set_datetime(run_start)
  ...
  SUBROUTINE icon_expose_descrdata_global(n_dom, max_dom, nproma, min_rlcell_int, min_rlcell, &
    ...
    LOGICAL,  INTENT(IN) :: l_limited_area
    ! local variables
    TYPE(t_comin_descrdata_global_data) :: comin_descrdata_global_data
#ifdef _OPENACC
    ...
    comin_descrdata_global_data%l_limited_area   = l_limited_area
    comin_descrdata_global_data%wp               = wp
  ...
  SUBROUTINE icon_expose_descrdata_domain(patch, number_of_grid_used, vgrid_buffer, start_time, end_time)
    ...
    !local var
    INTEGER :: jg
    TYPE(t_comin_descrdata_domain_data) :: comin_descrdata_domain(SIZE(patch))
    ...
    DO jg=1,SIZE(patch)
      ...
      comin_descrdata_domain(jg)%cells%e_bln_c_s => p_int_state(jg)%e_bln_c_s
      comin_descrdata_domain(jg)%cells%geofac_div => p_int_state(jg)%geofac_div
      comin_descrdata_domain(jg)%cells%geofac_n2s => p_int_state(jg)%geofac_n2s
      comin_descrdata_domain(jg)%cells%geofac_grg => p_int_state(jg)%geofac_grg

      comin_descrdata_domain(jg)%cells%rbf_vec_idx => p_int_state(jg)%rbf_vec_idx_c
      comin_descrdata_domain(jg)%cells%rbf_vec_blk => p_int_state(jg)%rbf_vec_blk_c
      comin_descrdata_domain(jg)%cells%rbf_vec_stencil => p_int_state(jg)%rbf_vec_stencil_c
      comin_descrdata_domain(jg)%cells%rbf_vec_coeff => p_int_state(jg)%rbf_vec_coeff_c
      ...
      comin_descrdata_domain(jg)%verts%rbf_vec_idx => p_int_state(jg)%rbf_vec_idx_v
      comin_descrdata_domain(jg)%verts%rbf_vec_blk => p_int_state(jg)%rbf_vec_blk_v
      comin_descrdata_domain(jg)%verts%rbf_vec_stencil => p_int_state(jg)%rbf_vec_stencil_v
      comin_descrdata_domain(jg)%verts%rbf_vec_coeff => p_int_state(jg)%rbf_vec_coeff_v

      comin_descrdata_domain(jg)%verts%glb_index => patch(jg)%verts%decomp_info%glb_index
      comin_descrdata_domain(jg)%verts%decomp_domain => patch(jg)%verts%decomp_info%decomp_domain
      ...
      comin_descrdata_domain(jg)%edges%tangent_orientation => patch(jg)%edges%tangent_orientation
      comin_descrdata_domain(jg)%edges%nudgecoeff_e => p_int_state(jg)%nudgecoeff_e

      comin_descrdata_domain(jg)%edges%rbf_vec_idx => p_int_state(jg)%rbf_vec_idx_e
      comin_descrdata_domain(jg)%edges%rbf_vec_blk => p_int_state(jg)%rbf_vec_blk_e
      comin_descrdata_domain(jg)%edges%rbf_vec_stencil => p_int_state(jg)%rbf_vec_stencil_e
      comin_descrdata_domain(jg)%edges%rbf_vec_coeff => p_int_state(jg)%rbf_vec_coeff_e

      comin_descrdata_domain(jg)%edges%c_lin_e => p_int_state(jg)%c_lin_e

      comin_descrdata_domain(jg)%edges%glb_index => patch(jg)%edges%decomp_info%glb_index
      comin_descrdata_domain(jg)%edges%decomp_domain => patch(jg)%edges%decomp_info%decomp_domain
    END DO
    ...
    CALL comin_descrdata_set_fct_glb2loc_edge(glb2loc_index_edge)
    CALL comin_descrdata_set_fct_glb2loc_vert(glb2loc_index_vert)
  ...
  INTEGER(c_int) FUNCTION glb2loc_index(jg, glb) RESULT(loc) BIND(C)
  ...
  INTEGER(c_int) FUNCTION glb2loc_index_edge(jg, glb) RESULT(loc) BIND(C)
    INTEGER(c_int), VALUE :: jg
    INTEGER(c_int), VALUE :: glb

    loc = get_local_index(p_patch(jg)%edges%decomp_info%glb2loc_index,glb)
  END FUNCTION glb2loc_index_edge

  INTEGER(c_int) FUNCTION glb2loc_index_vert(jg, glb) RESULT(loc) BIND(C)
    INTEGER(c_int), VALUE :: jg
    INTEGER(c_int), VALUE :: glb

    loc = get_local_index(p_patch(jg)%verts%decomp_info%glb2loc_index,glb)
  END FUNCTION glb2loc_index_vert
  ...
  SUBROUTINE icon_update_expose_variables(tlev_source, tlev, updatelocation)
    ...
    DO WHILE (ASSOCIATED(current))
      ...
      CASE(COMIN_VAR_DATATYPE_DOUBLE)
        ...
        IF (current%icon_var%info%lcontained) THEN
          ic = current%icon_var%info%ncontained
          SELECT CASE(current%icon_var%info%var_ref_pos)
          CASE (1)
  ...
  SUBROUTINE icon_call_callback(ep, jg, lacc)
    ...
    CALL comin_callback_context_call(ep, jg, LOGICAL(lzacc, C_BOOL))
    IF (timers_level > 2) CALL timer_stop(timer_comin_callbacks)
  END SUBROUTINE icon_call_callback

  SUBROUTINE icon_sync_variable(ptr, device_ptr, datatype, array_shape, direction) BIND(C)
    ...
    SELECT CASE (datatype)
    CASE (COMIN_VAR_DATATYPE_DOUBLE)
      CALL C_F_POINTER(ptr, dp_ptr, array_shape)
      IF (direction /= 0) THEN ! to device
        !$ACC UPDATE DEVICE(dp_ptr)
      ELSE
        !$ACC UPDATE HOST(dp_ptr)
      END IF
    CASE (COMIN_VAR_DATATYPE_FLOAT)
      CALL C_F_POINTER(ptr, sp_ptr, array_shape)
      IF (direction /= 0) THEN ! to device
      ...
      END IF
    CASE (COMIN_VAR_DATATYPE_INT)
      CALL C_F_POINTER(ptr, i_ptr, array_shape)
      IF (direction /= 0) THEN ! to device
  ...
  SUBROUTINE icon_halo_sync_variable(jg, ptr, datatype, array_shape, dim_semantics) BIND(C)
    ...
    SELECT CASE (datatype)
    CASE (COMIN_VAR_DATATYPE_DOUBLE)
      iref_pos = FINDLOC(dim_semantics, COMIN_DIM_SEMANTICS_UNUSED, 1) !get the index of the container dimension
      CALL C_F_POINTER(ptr, dp_ptr, array_shape)
      IF(.NOT. ANY(dim_semantics == COMIN_DIM_SEMANTICS_LEVEL)) THEN
  ...
  !> Adapter for ICON's finish routine to be called by comin.
  SUBROUTINE icon_comin_finish_c(routine, text) BIND(C)
    CHARACTER(c_char) :: routine(*)
    CHARACTER(c_char) :: text(*)

    CHARACTER(len=:), ALLOCATABLE :: routine_f
    CHARACTER(len=:), ALLOCATABLE :: text_f
    INTEGER :: i

    ALLOCATE(CHARACTER(len=my_strlen(routine)) :: routine_f)
    ALLOCATE(CHARACTER(len=my_strlen(text)) :: text_f)

    DO i = 1, LEN(routine_f)
      routine_f(i:i) = routine(i)
    END DO
    ...
    CALL finish(routine_f, text_f)

  END SUBROUTINE icon_comin_finish_c

  ! We cannot use c_strlen directly, declared with an assumed-size character array, because
  ! NAG insists we use the same declaration as CDI, which takes a c_ptr. And we cannot
  ! declare `routine` and `text` TARGETs because that would not match the procedure interface
  ! for the C error routine. So we need to take the detour via this function.
  INTEGER(c_size_t) FUNCTION my_strlen(str)
    CHARACTER(c_char), TARGET :: str(*)

    INTERFACE
      INTEGER(c_size_t) FUNCTION c_strlen(ptr) BIND(C, name='strlen')
        IMPORT c_size_t, c_ptr
        TYPE(c_ptr), VALUE :: ptr
      END FUNCTION
    END INTERFACE

    my_strlen = c_strlen(C_LOC(str))
