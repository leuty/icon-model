...
!  INTEGER  :: jk, jl !< loop indices
  ...
  !--------------------------------
  REAL (wp) :: reimin, reimax, relmin, relmax
  !
  !DA TODO: rearrange the data section to reduce memory consumption
  !
  !$ACC DATA PRESENT(xm_ice, xm_liq, dz, pcos_mu0, emissivity) &
...
!  ngptsw = k_dist_sw%get_ngpt()
  ...
  !
  CALL prepare_cloud_mass_for_optics( ncol,          klev,          &
                                    & xm_liq,        xm_ice,        &
                                    & zlwp,          ziwp           )
  !
  ...
  reimin = cloud_optics_lw%get_min_radius_ice() ! 10.0_wp  !
  reimax = cloud_optics_lw%get_max_radius_ice() ! 124.0_wp !

  relmin = cloud_optics_lw%get_min_radius_liq() ! 2.5_wp  !
  relmax = cloud_optics_lw%get_max_radius_liq() ! 21.5_wp !

  IF (relmax < relmin .OR. reimax < reimin) THEN
    CALL finish('rte_rrtmgp_interface_onBlock (mo_rte_rrtmgp_interface.f90)', &
                'Droplet minimum size must not be bigger than maximum size.')
  END IF

  CALL prepare_cloud_particle_size_for_optics( ncol,        klev,         &
                                             & relmin,      relmax,       &
                                             & reimin,      reimax,       &
                                             & zlwp,        ziwp,         &
                                             & cdnc,        reff_ice,     &
  ...
  reimin = cloud_optics_sw%get_min_radius_ice() ! 10.0_wp  !
  reimax = cloud_optics_sw%get_max_radius_ice() ! 124.0_wp !

  relmin = cloud_optics_sw%get_min_radius_liq() ! 2.5_wp  !
  relmax = cloud_optics_sw%get_max_radius_liq() ! 21.5_wp !
  ...
  CALL prepare_cloud_particle_size_for_optics( ncol,        klev,         &
                                             ...
                                             & laland,      laglac,       &
                                             & re_drop,     re_cryst      )
  CALL modify_hydrometeor_conc_by_inhomogeneity ( ncol,         klev,            cinhomi,        &
                                                & cinhoml_cfo,  cinhoml_cfl,     cinhoml_sf,     &
                                                & cinhoml_del1, cinhoml_del2,    cinhoml_jk,     &
                                                & tk_fl,        tk_sfc,                          &
                                                & pp_fl,        pp_sfc,                          &
...
SUBROUTINE prepare_cloud_mass_for_optics( ncol,          klev,      &
                                        & xm_liq,        xm_ice,    &
                                        & zlwp,          ziwp       )

  INTEGER, INTENT(in)  :: ncol, klev !< number of columns and layers
  REAL(wp), INTENT(in) :: xm_liq(:,:), xm_ice(:,:)      !< liquid and ice mass [kg/m^2] in grid cell
  REAL(wp), INTENT(out):: zlwp(:,:), ziwp(:,:)          !< Cloud liquid and ice mass [g/m^2] in grid cell,
                                                        !< including a minimum threshold
  INTEGER              :: jk, jl
  REAL (wp), PARAMETER :: &
  ccwmin = 1.e-7_wp       ! min condensate for lw, iw, and sw cloud opacity

  !$ACC KERNELS DEFAULT(PRESENT) ASYNC(1)
  ziwp(:,:) = MERGE(xm_ice(:,:)*1000.0_wp,0._wp,xm_ice(:,:)>ccwmin)
  zlwp(:,:) = MERGE(xm_liq(:,:)*1000.0_wp,0._wp,xm_liq(:,:)>ccwmin)
  !$ACC END KERNELS
END SUBROUTINE prepare_cloud_mass_for_optics

! -------------------------------------------------------------------------------------

SUBROUTINE prepare_cloud_particle_size_for_optics( ncol,        klev,         &
                                                 & relmin,      relmax,       &
                                                 & reimin,      reimax,       &
                                                 & zlwp,        ziwp,         &
                                                 & cdnc,        reff_ice,     &
  ...
  INTEGER,  INTENT(in)     :: ncol, klev  !< number of column and layers
  REAL(wp), INTENT(in)     :: relmin, relmax, reimin, reimax  !< min, max of liquid, ice particles in respective tables.
  REAL(wp), INTENT(in)     :: zlwp(:,:), ziwp(:,:), cdnc(:,:), reff_ice(:,:), dz(:,:)
  LOGICAL,  INTENT(in)     :: &
                              &   laland(:),   & !< land sea mask, land=.true.
                              &   laglac(:)      !< glacier mask, glacier=.true.
  REAL(wp), INTENT(out)    :: re_drop(:,:), re_cryst(:,:)
  ...
  INTEGER                  :: jk, jl
  REAL(wp)                 :: zlwc, zkap, effective_radius

  ! effective radius of ice crystals
  !$ACC KERNELS DEFAULT(PRESENT) ASYNC(1)
  re_cryst(:,:) = MERGE(MAX(reimin, MIN(reimax, 1.e6_wp * reff_ice(:,:))), reimin, ziwp(:,:)>0._wp)
  !$ACC END KERNELS

  !effective radius droplets
  effective_radius = 1.0e6_wp * droplet_scale * (3.0e-9_wp / (4.0_wp * pi * rhoh2o))**(1.0_wp/3.0_wp)
  !$ACC PARALLEL DEFAULT(PRESENT) ASYNC(1)
  !$ACC LOOP GANG VECTOR COLLAPSE(2)
  DO jk = 1, klev
    DO jl = 1, ncol
      IF (zlwp(jl,jk)>0._wp) THEN
        zlwc = zlwp(jl,jk)/dz(jl,jk) ! --- cloud water concentration [g/m3]
        zkap = zkap_mrtm
  ...
  END DO
    !$ACC END PARALLEL
END SUBROUTINE prepare_cloud_particle_size_for_optics

! -------------------------------------------------------------------------------------

SUBROUTINE modify_hydrometeor_conc_by_inhomogeneity ( ncol,         nlay,            cinhomi,        &
                                                    & cinhoml_cfo,  cinhoml_cfl,     cinhoml_sf,     &
                                                    & cinhoml_del1, cinhoml_del2,    cinhoml_jk,     &
                                                    & tk_fl,        tk_sfc,                          &
                                                    & pp_fl,        pp_sfc,                          &
                                                    & laland,       laglac,          lts,            &
                                                    & cinhoml,      zlwp,            ziwp            )
  INTEGER,  INTENT(in)      :: ncol, nlay      !< number of column and layers
  REAL(wp), INTENT(IN)      :: cinhomi         !< inhomogeneity factor for ice
  REAL(wp), INTENT(IN)      :: cinhoml_cfo,  & !< inhomogeneity factor cumuliform over ocean
                            &  cinhoml_cfl,  & !< inhomogeneity factor cumuliform over land
                            &  cinhoml_sf,   & !< limiting inhomogeneity for liquid cumulus and stratus
                            &  cinhoml_del1, &
                            &  cinhoml_del2
  INTEGER,  INTENT(in)      :: cinhoml_jk      !< level index for blending function of inhoml factors
  REAL(wp), INTENT(in)      :: tk_fl(:,:),   & !< full level temperature in K
                            &  tk_sfc(:),    & !< surface temperature in K
                            &  pp_fl(:,:),   & !< full level pressure in Pa
                            &  pp_sfc(:)       !< surface pressure in Pa
  LOGICAL,  INTENT(in)      ::               &
                            &  laland(:),    & !< land sea mask, land=.true.
                            &  laglac(:)       !< glacier mask, glacier=.true.
  REAL(wp), INTENT(out)     :: lts(:)          !< lower tropospheric stability
  REAL(wp), INTENT(out)     :: cinhoml(:)
  REAL(wp), INTENT(inout)   :: zlwp(:,:),    & !< cloud liquid water path [g/cm^2]
                            &  ziwp(:,:)       !< cloud ice water path [g/cm^2]
  INTEGER                   :: jl, ic, nlay_min

  !$ACC DATA CREATE(cinhoml) &
  !$ACC   PRESENT(tk_fl, tk_sfc, pp_fl, pp_sfc, zlwp, ziwp)
  ...
  IF (cinhoml_sf /= cinhoml_cfo) THEN
    nlay_min=MIN(cinhoml_jk,nlay)
    !$ACC PARALLEL LOOP DEFAULT(PRESENT) GANG VECTOR ASYNC(1)
    DO jl = 1, ncol
       lts(jl) = tk_fl(jl,nlay_min)*(1e5_wp/pp_fl(jl,nlay_min))**(rd_o_cpd) &
  ...
  DO jl = 1, nlay
    DO ic = 1, ncol
      zlwp(ic,jl) = zlwp(ic,jl) * cinhoml(ic)
      ziwp(ic,jl) = ziwp(ic,jl) * cinhomi
    END DO
  END DO
  !$ACC END PARALLEL
  !$ACC END DATA
END SUBROUTINE modify_hydrometeor_conc_by_inhomogeneity
