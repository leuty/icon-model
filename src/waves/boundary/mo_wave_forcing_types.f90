...
MODULE mo_wave_forcing_types
  ...
  TYPE :: t_wave_forcing
    ...
    REAL(wp), POINTER, CONTIGUOUS ::  &
      ...
      & dir10m(:,:),      & ! wind direction in 10m (nproma,nblks_c)  ( rad )
      & taua_x(:,:),      & ! zonal surface stress                    (nproma,nblks_c) ( (m/s)^2 )
      & taua_y(:,:),      & ! meridional surface stress               (nproma,nblks_c) ( (m/s)^2 )
      & taua(:,:),        & ! total wind stress magnitude             (nproma,nblks_c) ( (m/s)^2 )
      & sea_ice_c(:,:),   & ! sea ice concentration at centers       (nproma,nblks_c) (frac. of 1)
