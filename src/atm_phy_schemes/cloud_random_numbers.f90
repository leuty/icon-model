...
!  public :: rng_type
  ...
  integer(kind=jpim), parameter :: NMaxStreams0 = 5120
 ...
 ! integer(kind=jpib), parameter :: IMinstdM  = 2147483647
  ...
  !  integer(kind=jpim) :: istate(NMaxStreams0)
    real(kind=jprb) :: istate(NMaxStreams0)
    integer(kind=jpim) :: nmaxstreams = NMaxStreams0
...
contains
  ...
  subroutine initialize(this, itype, iseed, nmaxstreams)
    ...
    integer(kind=jpim), allocatable :: iseednative(:)
    integer(kind=jpim) :: nseed, jseed
    real(kind=jprb)    :: rnd_init, rseed
    ...
    if (this%itype == IRngMinstdVector) then
      ...
      do jseed = 1,this%nmaxstreams
        rnd_init = nint(mod( rseed*jseed*(1._jprb-0.05_jprb*jseed+0.005_jprb*jseed**2) &
             & *IMinstdA0, IMinstdM),kind=jpib)
