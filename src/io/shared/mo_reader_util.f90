...
MODULE mo_reader_util
  ...
  USE mtime, ONLY: julianday, juliandelta, datetime, timedelta, newDatetime, newTimedelta, &
      ...
      & timeDeltaToJulianDelta, OPERATOR(*), OPERATOR(+), no_of_ms_in_a_day
...
CONTAINS
  ...
  RECURSIVE FUNCTION shift_time (base, unit, len) RESULT(jd)
    TYPE(datetime), INTENT(IN) :: base
    TYPE(timedelta), INTENT(IN) :: unit
    REAL(wp), INTENT(IN) :: len
    TYPE(julianday) :: jd
    ...
    IF (unit%year == 0 .AND. unit%month == 0) THEN
      BLOCK
        ...
        jdelta%sign = MERGE('+', '-', len >= 0._wp .NEQV. junit%sign == '+')
    ...
    ELSE
      BLOCK
        ...
        REAL(wp) :: step_in_months
        INTEGER :: whole_months
        REAL(wp) :: month_fraction
        ...
        big_step = timedelta( &
            & flag_std_form=1, &
            & sign=MERGE('+', '-', len >= 0._wp .NEQV. unit%sign == '+'), &
        ...
        IF (month_fraction > 1._wp / (no_of_ms_in_a_day * 31)) THEN
          BLOCK
        ...
        jd = shift_time(intermediate_dt, small_unit, len)
