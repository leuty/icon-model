...
MODULE mo_upatmo_phy_diag
  ...
  USE mo_upatmo_timer,           ONLY: timer_upatmo_phy_diag
...
CONTAINS
  ...
  SUBROUTINE update_diagnostic_variables(mtime_datetime,    &  !in
    ...
    IF (upatmo_nwp%l_extdat_stat(iUpatmoExtdatStat%required)) THEN
      ...
      ELSE  ! Resumed simulation
        ...
        DO jext = 1, next
          ...
          END IF  !IF (lupdate_extdat(jext))

        END DO  !jext

      END IF  !IF (.NOT. lrestart)
    ...
    IF (lupdate_gas) THEN
      ...
      DO jgas = 1, ngas
        ...
        IF (jgas /= igasDiag) THEN
          ...
          DO jb = i_startblk, i_endblk
            ...
            END IF  !IF (upatmo_nwp_gas(jgas)%imode == imodeExtdat)
      ...
      END IF  !IF (upatmo_phy_config%lsanitycheck)
