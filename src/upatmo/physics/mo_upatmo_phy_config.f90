  ...
  SUBROUTINE configure_upatmo_physics(jg,                      & !in
    ...
    CASE (inwp)
      ...
      DO jprc = 1, iUpatmoPrcId%nitem
        ...
        END IF  !Group enabled and not in offline-mode?
  ...
  SUBROUTINE print_config_upatmo_physics(jg,                      & !in
    ...
    IF (msg_level >= imsg_thr%low) THEN
      ...
      CASE (inwp)
        ...
        END IF  !IF (jg == 1 .AND. upatmo_nwp_phy_config%l_phy_stat( iUpatmoPrcStat%enabled ))
      ...
      IF (msg_level >= imsg_thr%med) THEN
        ...
        IF (msg_level >= imsg_thr%high) THEN
          ...
          CASE (iaes)
            ...
            IF (jg == 1 .AND. upatmo_aes_phy_config%l_enabled) THEN
              ...
              DO jprc = 1, iUpatmoPrcId%nitem
                ...
                CALL message(' ', message_text)
              END DO  !jprc
            END IF  !IF (jg == 1 .AND. upatmo_aes_phy_config%l_enabled)
          ...
          CASE (inwp)
                  ...
                  DO jprc = 1, iUpatmoPrcId%nitem
                    IF (upatmo_nwp_phy_config%prc(jprc)%igrp == jgrp) THEN
                      ...
                      WRITE(message_text, '(7a,i0,a,i0,a)') &
                          ...
                          & hght_msg_sfx(ladj_h:)
                      CALL message(' ', message_text)
                    END IF  !IF (upatmo_nwp_phy_config%prc( jprc )%igrp == jgrp)
                  END DO  !jprc
                END IF  !IF (upatmo_nwp_phy_config%grp( jgrp )%l_stat( iUpatmoPrcStat%enabled ))
              END DO  !jgrp
            ...
            END IF  !IF (upatmo_nwp_phy_config%l_phy_stat( iUpatmoPrcStat%enabled ))
            !
            IF (jg == 1 .AND. upatmo_nwp_phy_config%l_gas_stat(iUpatmoGasStat%enabled)) THEN
              ...
              DO jgas = 1, iUpatmoGasId%nitem
                IF (upatmo_nwp_phy_config%gas(jgas)%l_stat(iUpatmoGasStat%enabled)) THEN
                  ...
                  END IF
                  CALL message(' ', message_text)
                END IF  !IF (upatmo_nwp_phy_config%gas( jgas )%l_stat( iUpatmoGasStat%enabled ))
              END DO  !jgas
              !
            END IF  !IF (jg == 1 .AND. upatmo_nwp_phy_config%l_gas_stat( iUpatmoGasStat%enabled ))
            !
