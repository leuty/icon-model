...
MODULE mo_upatmo_state
  ...
  USE mo_upatmo_timer,         ONLY: timer_upatmo_constr, timer_upatmo_destr, &
      &                              timer_upatmo
...
CONTAINS
  ...
  SUBROUTINE construct_upatmo_state(n_dom,             & !in
    ...
    IF (ANY(upatmo_config(:)%nwp_phy%l_phy_stat(iUpatmoPrcStat%enabled))) THEN
      ...
      DO jg = 1, n_dom
        ...
        END IF  !Physics switched on on domain?
    ...
    END IF !Any upper-atmosphere physics switched on?
  ...
  SUBROUTINE new_upatmo_tend_list(jg, nlev, nblks_c, nblks_e, nproma, listname, vname_prefix, &
    ...
    IF (ANY(upatmo_nwp_phy_config%l_any_update(:))) THEN
        ...
        DO jgrp = 1, ngrp
          IF (upatmo_nwp_phy_config%grp(jgrp)%l_update(jtnd)) THEN
            ...
            ELSE
              ...
              END IF
            END IF  !IF (jgrp == 1)
          END IF  !IF (upatmo_nwp_phy_config%grp( jgrp )%l_update( ntnd ))
        END DO  !jgrp
      ...
      END IF !Any update of temperature?
      ...
      END IF !Any update of Exner pressure?
      ...
      END IF !Any update of horizontal wind component?
      ...
      END IF !Any update of tracers?

    END IF  !Accumulative tendencies required?
