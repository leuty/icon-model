...
!!
!! Revision History
!! Initial version by Gergely Boeloeni (2021-03-01)
!!
!!
MODULE mo_nh_isotherm_rest_atm_gwp
  ...
  USE mo_nwp_msgwam_utils,     ONLY: sync_wave
  USE mo_msgwam_config,        ONLY: a0,branch, dk_ini, dl_ini, dm_ini, envtype, &
                                     k_ini, lambda_lat_ini, lambda_lon_ini, lambda_z_ini, &
                                     lat0, latrmax, latrmin, l_ini, lon0, lonrmax, lonrmin, &
                                     ltest_hprop, m_ini, nrk, nrl, nrlat, nrlon, nrm, nrz, &
                                     p0, sigmax_wp, sigmay_wp, sigmaz_wp, t0, x0, xrmax, xrmin, &
                                     y0, yrmax, yrmin, z0
  USE mo_setup_msgwam_interface,   ONLY: domxmax, domxmin, domymax, domymin, &
                                         filename_ltest_hprop, height_triangle, &
                                         t_gridinfo4ray, t_ray, jkmax_ini, jkmin_ini
...
!
  ...
  SUBROUTINE init_nh_isotherm_rest_atm_gwp(p_patch, p_nh_prog, p_nh_diag, &
    ...
    TYPE(t_gridinfo4ray), INTENT(IN) :: &  !< MS-GWaM stencil for neighbor search
      &  p_gridinfo4ray
    ...
    REAL(wp) :: dxray, dyray, delta_lon, distance, base_small
    ...
    ! Method:
    !         -- see Muraschko et al. (2015), Boeloeni et al. (2016),
    !                Wei et al. (2019)
    !----------------------------------------------------------------------
...
!$OMP END PARALLEL
    ...
    CALL sync_patch_array_mult(SYNC_C, p_patch, 2, lacc=.FALSE., f3din1=p_nh_diag%temp, f3din2=p_nh_diag%pres)

    CALL convert_thdvars(p_patch, p_nh_diag%pres, p_nh_diag%temp, &
                      & p_nh_prog%rho, p_nh_prog%exner, p_nh_prog%theta_v  )

    ! Synchronize
    CALL sync_patch_array_mult(SYNC_C, p_patch, 5, lacc=.FALSE., f3din1=p_nh_prog%rho, f3din2=p_nh_diag%rho_ic, &
                                                  f3din3=p_nh_prog%theta_v, f3din4=p_nh_diag%theta_v_ic, &
                                                  f3din5=p_nh_prog%exner)
    CALL sync_patch_array_mult(SYNC_C, p_patch, 3, lacc=.FALSE., f3din1=p_nh_diag%u, f3din2=p_nh_diag%v, f3din3=p_nh_prog%w)
...
!$OMP PARALLEL
!$OMP DO PRIVATE(jc,jk,jb,i_startidx,i_endidx,omega,Bw,dx,dy,dz,env,vec_thispos,vec_ref,           &
!$OMP            base_small,minlat,maxlat,distance,jray,jz,jlon,jlat,jmm,jll,jkk,dzray,zray,latray,&
!$OMP            delta_lon,lonray,dyray,dxray,mray,dmray,lray,dlray,kray,dkray,jkmid                )
      DO jb = i_startblk, i_endblk
                    ...
                    DO jmm = 1, nrm
                      ...
                      DO jll = 1, nrl
                        ...
                        DO jkk = 1, nrk
                          ...
                          dyray  = distance / REAL(nrlat,wp)

                          ! lon extent of ray volume
                          dxray  = distance / REAL(nrlon,wp)
  ...
  !                        IF (jz > CEILING(nrz*0.5_wp))  jkmid = jk-1
                          ...
                          p_ray%dx    (jc,jray,jb)    = dxray
                          p_ray%lat   (jc,jray,jb)    = latray
                          p_ray%dy    (jc,jray,jb)    = dyray
...
!$OMP PARALLEL
!$OMP DO PRIVATE(jc,jk,jb,i_startidx,i_endidx,vec_thispos,vec_ref,dx,dy,dz,omega,Bw,env,vecx,jvec,&
!$OMP lonray_ref,base_small,vecy,miny,maxy,dxray,dyray,jray,jz,jlon,jlat,jmm,jll,jkk,dzray,zray,latray,&
!$OMP            lonray,mray,dmray,lray,dlray,kray,dkray,jkmid)
      DO jb = i_startblk, i_endblk
        ...
        DO jc = i_startidx, i_endidx
          ...
          dxray  = SQRT(p_patch%cells%area(jc,jb)) / REAL(nrlon,wp)
          dyray  = SQRT(p_patch%cells%area(jc,jb)) / REAL(nrlat,wp)
