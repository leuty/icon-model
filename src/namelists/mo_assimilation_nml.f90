...
MODULE mo_assimilation_nml
  ...
  LOGICAL                          ::           &
    ...
    lhn_relax(max_dom)   ,& ! horizontal filtering of lhn t-increments
    ...
    lhn_spqual(max_dom)  ,& ! switch for use of a spatial quality function
    lhn_black(max_dom)   ,& ! use blacklist for radar data
    lhn_qrs              ,& ! calculate the integrated precipitation flux
    lhn_logscale         ,& ! apply logarithmic scaling factors
    lhn_wweight(max_dom) ,& ! apply a weighting with respect to the mean horizontal wind
    lhn_bright(max_dom)  ,& ! apply bright band detection
  ...
  INTEGER ::  &
    nlhn_relax(max_dom)  ,& ! number of interations of horizontal filtering
  ...
  REAL (KIND=wp)                   ::           &
    nlhn_start(max_dom)  ,& ! start of latent heat nudging period in seconds
    nlhn_end(max_dom)    ,& ! end of latent heat nudging period in seconds
    nlhnverif_start      ,& ! start of latent heat nudging period in seconds
    nlhnverif_end        ,& ! end of latent heat nudging period in seconds
    lhn_coef(max_dom)    ,& ! factor for reduction of lhn t-increments
    lhn_dt_obs           ,& ! time step of input data in minutes
    abs_lhn_lim(max_dom) ,& ! absolute limit for lhn t-increments (used if lhn_limit or lhn_limitp)
    ...
    fac_lhn_up(max_dom)  ,& ! limiting factor for upscaling of model heating profile
    fac_lhn_down(max_dom),& ! limiting factor for downscaling model heating profile
    thres_lhn            ,& ! threshold of rain rates to be consinderd within lhn approach
    rqrsgmax(max_dom)    ,& ! ratio of maximum of qrsgflux, needed for reference precipitation
...
!    noobs_date  (:)    = '            '
    ...
    DO jg= 1,max_dom
        ...
        assimilation_config(jg)%lhn_relax       = lhn_relax(jg)
        ...
        assimilation_config(jg)%lhn_wweight     = lhn_wweight(jg)
        assimilation_config(jg)%lhn_spqual      = lhn_spqual(jg)
        assimilation_config(jg)%lhn_black       = lhn_black(jg)
        assimilation_config(jg)%lhn_incloud     = lhn_incloud
        assimilation_config(jg)%lhn_artif_only  = lhn_artif_only
        assimilation_config(jg)%lhn_bright      = lhn_bright(jg)
        assimilation_config(jg)%lhn_refbias     = lhn_refbias
        assimilation_config(jg)%nlhn_start      = INT(nlhn_start(jg))
        assimilation_config(jg)%nlhn_end        = INT(nlhn_end(jg))
        assimilation_config(jg)%nlhnverif_start = INT(nlhnverif_start)
        assimilation_config(jg)%nlhnverif_end   = INT(nlhnverif_end)
        assimilation_config(jg)%nradar          = nradar(jg)
        assimilation_config(jg)%lhn_updt_rule   = lhn_updt_rule(jg)
        assimilation_config(jg)%nlhn_relax      = nlhn_relax(jg)
        assimilation_config(jg)%lhn_dt_obs      = lhn_dt_obs/60.
        assimilation_config(jg)%lhn_coef        = lhn_coef(jg)
        assimilation_config(jg)%abs_lhn_lim     = abs_lhn_lim(jg)
        assimilation_config(jg)%fac_lhn_artif   = fac_lhn_artif
        assimilation_config(jg)%fac_lhn_artif_tune = fac_lhn_artif_tune
        assimilation_config(jg)%fac_lhn_up      = fac_lhn_up(jg)
        assimilation_config(jg)%fac_lhn_down    = fac_lhn_down(jg)
        assimilation_config(jg)%thres_lhn       = thres_lhn
        assimilation_config(jg)%rqrsgmax        = rqrsgmax(jg)
