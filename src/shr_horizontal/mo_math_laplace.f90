...
CALL nabla2_vec_atmos_lib( vec_e, &
  ...
  &                        ptr_patch%nlev, ptr_patch%nblks_c, ptr_patch%nblks_v, slev, elev, nproma, lacc=i_am_accel_node )
...
CALL nabla2_scalar_lib( psi_c, ptr_patch%cells%neighbor_idx, ptr_patch%cells%neighbor_blk, &
  ...
  &                     ptr_patch%nlev, slev, elev, nproma, ptr_patch%nblks_e, ptr_patch%geometry_info%cell_type, lacc=i_am_accel_node )
...
CALL nabla2_scalar_avg_lib( psi_c, ptr_patch%cells%neighbor_idx, ptr_patch%cells%neighbor_blk, &
  ...
  &                         ptr_patch%nlev, slev, elev, nproma, l_limited_area, lacc=i_am_accel_node)
