...
19: tropospheric natural 'Kinne' aerosols from pre--industrial period
(the 1850--file has to be linked for all simulated years under a name
that does not contain the year) +
parameterized time dependent anthropogenic 'simple plumes'&
aes\_phy\_config(jg)\% dt\_rad > 0.000s \tabularnewline
\end{longtab}
...
L &
.FALSE. &  &
.TRUE.: Activates the coupling of aes atmosphere to Kinne aerosole input files. \newline
        Kinne aerosol input is taken from python processes rather than direct reading from pre-processed input files. rte-rrtmgp radiation supported only. In this case yac coupling routines are called.&
\tabularnewline
...
L &
.FALSE. &  &
.TRUE.: Activates the coupling of aes atmosphere to o3 input files. \newline
        O3 input is taken from python processes rather than direct reading from pre-processed input files. In this case yac coupling routines are called.&
\tabularnewline
