...
aerosols\\
18: tropospheric background aerosol (Kinne, filename without year) + stratospheric cmip6
...
&
&
Interval of SST/CI files as ISO 8601 duration.
Default is "P1M" (1 month). &
sstice\_mode=6
\tabularnewline
...
2: as option 1, but for all 5 aerosol types. Requires fields emi\_bc, emi\_oc and emi\_so2 in the extpar dataset \\
3: as option 2, but including wildfire emissions for bc, oc and so2. Emission data set can be specified with fire2d\_filename (\&initicon\_nml)
& irad\_aero=6.
\tabularnewline
...
simulations!) + time dep. volcanic stratospheric aerosols for CMIP6, both from file + param.
