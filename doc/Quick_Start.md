...
The configuration step is typically executed by running the
[./configure](/configure) script with command-line arguments. These arguments
specify the locations of libraries and tools needed for building. Users should
be aware that the list of arguments needed for successful configuration can be
extensive and complex. Therefore, instead of running the generic
[./configure](/configure) script directly, it is recommended to use a platform-
or machine-specific *configuration wrapper*. This wrapper automatically sets the
necessary compiler and linker flags along with the recommended configuration
options. The wrappers can be found in the respective subdirectories of the
...
specifying the number of jobs to run simultaneously. Usually 8 is a good choice. For example,
...
To run ICON it will be necessary to create a runscript that sets required environment variables and calls the executable. The quickest way to output is to run the "bubble" test using a runscript and experimental configuration generated by the python utility ```mkexp```.

Assuming you are still in DKRZ environment and in the main ICON directory (which would correspond to an in-source-build) execute

```sh
cd run
cp examples/bubble.config .
```
In the DKRZ environment edit ```bubble.config``` and set ```ACCOUNT``` to a valid project number.  Then execute:
```sh
mkexp bubble.config
`````
This makes a runscript called ```bubble.run_start``` and creates the directory```../experiments/bubble/scripts```.  To run ICON simply go to the directory and submit the script as a batch job, i.e.,
```sh
cd ../experiments/bubble/scripts
sbatch bubble.run_start
`````

The output will be directed to the ```work``` directory identified after the execution of *mkexp*.  On ICON this defaults to ```/scratch```.

To run other configurations or on different environments see [Running](#running).
...
the code generated by `CXX` and the host compiler used by `CUDACXX`, `HIPCXX`.
[↩](#f15-back)
...
based on the arguments provided to the [configure](/configure) script of ICON as
follows:
...
configure time are emitted not by the [configure](/configure) script of ICON but
by the configure scripts of the bundled libraries. Each of the scripts generates
its own `config.log` file, which can help in better understanding of the
reported issue. The log files are put to the corresponding subdirectories of the
...
[configure](/configure) script of ICON from the following environment variables:
...
To ensure consistency between configuration and building environments, you can
set the `BUILD_ENV` argument of the [configure](/configure) script with a series
of shell commands that initialize the necessary environment variables. If the
`BUILD_ENV` variable is not empty, the [configure](/configure) script will run
the commands it contains before running any checks. Additionally, the commands
will be saved to the `Makefile`, so they will be executed each time the `make`
command is launched for building. The shell script that is provided as value of
the `BUILD_ENV` argument must be a one-liner ending with a semicolon (;) symbol,
for example:
...
recommend considering the following features:

1. Ensure that the wrapper script passes command-line arguments to the
[configure](/configure) script, allowing users to override default configuration
options. For example:
    ```sh
    ./config/dkrz/levante.intel --enable-openmp
    ```
...
and `gcc`, each ready for building ICON with Intel and GCC compilers,
respectively.
...
standard output stream. This allows for better recognition of the warnings
...
the source files to the [src](/src) subdirectory of the source root directory of
ICON.
...
following preprocessing procedures.
...
by the [dependency generator](/utils/mkhelper/depgen.py). Undetectable
dependencies of the first type are related to
...
The [dependency listing script](/utils/mkhelper/deplist.py) reads the dependency
makefiles, builds a source dependency graph and traverses it starting with the
vertex associated with the `src/drivers/icon.o` object file. Each vertex of the
graph accessible from the starting one is printed to the output. The output is
then filtered by `make` in order to generate the list of object files required
for the ICON executable. This is the main purpose of the listing script.
Additionally, the tool can run
...
the help of the [pvcs.py](/utils/pvcs.py) script. The script generated a source
...
The executable of ICON is usually not launched directly. Rather it is called
from a runscript, which sets up a working directory, populates it with all
required input files (grid files, namelists, etc.), sets enviornment variables,
runs the model, and postprocesses its output.
...
steps before it can be used. Three methods are frequently used for executing
ICON:
1. using the python code make_experiment ([*mkexp*](./mkexp.md))
2. using the shell script [*make_runscript*](make_runscript.md).
3. getting a script that worked for someone else and hand editing it.
