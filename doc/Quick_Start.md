...
1. [Quick start](#quick-start)
    ...
    - [Script and submit](#script-and-submit-run)
        - [mkexp](#quick-start-with-mkexp)
        - [make_runscripts](#quick-start-with-make_runscripts)
...
4. [Scripting and running](#scripting-and-running)
    - [More on make_runscript](#more-about-make_runscripts)
...
Create a local clone of one of ICON's git repositories and its submodules, e.g.

```sh
git clone --recursive git@gitlab.dkrz.de:icon/icon.git /path/to/icon
```
...
```sh
module load /path/to/icon/etc/Modules/icon-levante
cd /path/to/icon
./config/dkrz/levante.intel --enable-openmp
```

>>>
The first line,`module load`, is to support mkexp functionality and needs to be 
executed before configuring icon.
>>>
...
However, as was done in this case, scripting for out-of-source-builds only 
supports build directories in the `/path/to/icon`.
...
populates it with all required input files (grid files, namelists, etc.), runs 
the model, and postprocesses its output.

This can be done either by using [mkexp](#quick-start-with-mkexp) or by using 
[make_runscripts](#quick-start-with-make_runscripts).

### Quick start with *mkexp*

Running mkexp requires four steps:
1. setting up the build environment
2. selecting and editing an example script
3. generating the scripts and workflow environment
4. submitting the job

#### Set up the build environment

For an in-source build *mkexp* assumes that the build directory is given as 
*build* and nothing needs to be specified. In the case of an
[out of source build](#out-of-source-configuration-building)
the *build- directory* must be set explicitly, i.e.,

```sh
export ICON_BUILD_DIR=build-directory
```

With the build directory clarified the modules required to run *mkexp* can be 
loaded as follows:

```sh
module use /path/to/icon-srcdir/etc/Modules && module add icon-levante
```

> **_NOTE:_**
The build environment must be set up for each new session.

#### Create an experiment config file

This is used by mkexp to create the experiment runscript, set the run 
environment and configure a directory structure for the run.

Start by picking a name for the experiment identifier (exp_id).  The Hamburg 
convention is that this is a three initial identifier followed by a number 
sequence.  I.e., for Maxine Xavier Musterding's third experiment the exp_id 
would be `mxm00003.config`.

Select an experiment (`example.config`) from the 
`/path/to/icon-srcdir/run/examples` and copy this to a configuration file 
identified by your exp_id, i.e.,


```sh
cp /path/to/icon-srcdir/run/examples/example.config exp_id.config
```

Edit the exp_id.config to include your options. Note that adapting `ACCOUNT` is 
needed on most HPC systems.

#### Generate the workflow environment and scripts

Executing *mkexp* by entering:
```sh
mkexp /path/to/icon-srcdir/run/exp_id.config
```
creates the scripts and a directory structure for organizing the output from 
your experiment.

Assuming an m-account number, this should result in output like
...
```sh
cd /path/to/icon-srcdir/experiments/exp_id/scripts
sbatch exp_id.run_start
```

This will run ICON following the script *exp_id.run_start*.

For further runs one can simply edit this script, but then you need to know 
what you are doing.  Alternatively, for a cleaner workflow and less chances of 
overwriting old experiments, repeat the above steps (2-4) with a new exp_id 
edited to reflect the desired changes.

### Quick start with *make_runscripts*
...
[Scripting and running](#scripting-and-running).)
...
model, refer to section [Scripting and running](#scripting-and-running).
...
<a name="icon-deptable">*Table 1. Topologically sorted list of the ICON 
dependency graph*</a>
...
| [NetCDF-C](https://www.unidata.ucar.edu/software/netcdf/docs/) | `--without-external-cdi` or `--enable-coupling`<sup><a name="f9-back" href="#f9">9</a></sup> | `CPPFLAGS='-I/path/to/netcdf/include' LDFLAGS='-L/path/to/netcdf/lib' LIBS='-lnetcdf'` |
...
| [STDC++](https://isocpp.org/)<sup><a name="f15-back" href="#f15">15</a></sup> | `--enable-gpu=openacc+cuda` or `--enable-gpu=openacc+hip` or `--enable-comin` | `LDFLAGS='-L/path/to/gcc/used/by/CUDACXX-or-HIPCXX-or-COMIN/lib' LIBS='-lstdc++'` (depends on the implementation) |
...
regardless of whether external or the bundled versions of YAC is used.
[↩](#f3-back)
...
the code generated by `CXX` and/or the host compiler of `CUDACXX`/`HIPCXX`.
[↩](#f15-back)
...
[Table 1](#icon-deptable)). The [configure](../configure) script of ICON fails
if the flags are not set correctly or enable a version of the library that is
known to be incompatible with ICON.
...
> **_NOTE:_** Oftentimes, error and warning messages that are printed at
configure time are emitted not by the [configure](../configure) script of ICON
but by the configure scripts of the bundled libraries. Each of the scripts
generates its own `config.log` file, which can help in better understanding of
the reported issue. The log files are put to the corresponding subdirectories of
the `./externals` directory residing in the root build directory of ICON.
...
If the variables are set, the [configure](../configure) script will check
whether their values meet the requirements, otherwise the script will try to
guess suitable values for them. Thus, if you want to make sure that a particular
...
[configure](../configure) script of ICON generates an additional linker flag
that puts the `<path>` on the list of runtime library search paths of the ICON
...
(see [commit 36ab00dd56](https://gitlab.dkrz.de/icon/icon/-/commit/36ab00dd5641419e5afbb918b47cdf697c54b737#87db583be5c13c1f7b3c958b10e03d67b6a2ca06)),
...
[configure](../configure) script will run the commands it contains before
running any checks. Additionally, the commands will be saved to the `Makefile`,
so they will be executed each time the `make` command is launched for building.
The shell script that is provided as value of the `BUILD_ENV` argument must be a
...
default values of the configure options, for example:
    ...
    ./config/dkrz/levante.intel --enable-openmp
    ```
    Also, account for the case of calling the wrapper with the `--help`
    argument.
2. Account for out-of-source building:
    ...
    following runscript generation (see section
    [Scripting and running](#scripting-and-running) for more details).
...
the aforementioned extensions and reside in the [src](../src) and
...
dependencies is associated with Fortran external procedures: when a Fortran
source file contains a call to an external procedure, i.e. a function or a
subroutine that is not part of any Fortran module. Dependencies of the second
...
2. **Two or more Fortran modules depend on each other in a circular way.**
...
the files are compared for binary identity with the `cmp` command, second, if
the first check shows the difference, the contents of the files are compared
with the [fortmodcmp.py](../utils/mkhelper/fortmodcmp.py) script, which employs
...
# Scripting and running

## More about make_runscripts
This section provides further information on how to generate runscripts
using make_runscripts and standard templates.
...
First, we recommend to check whether there is a suitable
...
(e.g. https://gitlab.dkrz.de/icon/icon-model/-/tree/release-2024.01-public).
