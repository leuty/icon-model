...
- [CMake](https://cmake.org)-based packages:
  ...
  - compiler flags that are passed to the {{ '[`configure`]({}/configure)'.format(base_url) }} script of ICON are modified as described in section [](ref_buildrun_configuration_flags) and passed as the respective `-DCMAKE_<LANG>_FLAGS_ICON` arguments (together with `-DCMAKE_BUILD_TYPE=ICON`);
...
- `FCFLAGS` &mdash; Fortran compiler flags to be used when configuring, compiling and linking ICON, as well as passed to the configure and CMake (via the `-DCMAKE_Fortran_FLAGS_ICON` argument) scripts of the bundled packages (defaults to an empty string);
...
- `CFLAGS` &mdash; C compiler flags to be used when configuring and compiling ICON, as well as passed to the configure and CMake (via the `-DCMAKE_C_FLAGS_ICON` argument) scripts of the bundled packages (defaults to an empty string);
- `CPPFLAGS` &mdash; C preprocessor flags to be used when configuring and compiling ICON, as well as passed to the configure and CMake (via the `-DCMAKE_C_FLAGS_ICON` and `-DCMAKE_CXX_FLAGS_ICON` arguments) scripts of the bundled packages (defaults to an empty string);
...
- `CXXFLAGS` &mdash; C++ compiler flags to be passed to the configure and CMake (via the `-DCMAKE_CXX_FLAGS_ICON` argument) scripts of the bundled packages (defaults to an empty string);
...
- `CUDAFLAGS` &mdash; CUDA C++ compiler flags to be used when configuring and compiling ICON, as well as passed to the configure and CMake (via the `-DCMAKE_CUDA_FLAGS_ICON` argument) scripts of the bundled packages (defaults to an empty string);
- `ICON_CUDAFLAGS` &mdash; CUDA C++ flags to be appended to `CUDAFLAGS` when configuring and compiling ICON (defaults to an empty string);
- `ICON_BUNDLED_CUDAFLAGS` &mdash; CUDA C++ flags to be appended to `CUDAFLAGS` when configuring the bundled packages (defaults to `ICON_CUDAFLAGS`);
- `HIPFLAGS` &mdash; HIP C++ compiler flags to be used when configuring and compiling ICON, as well as passed to the configure and CMake (via the `-DCMAKE_HIP_FLAGS_ICON` argument) scripts of the bundled packages (defaults to an empty string);
- `ICON_HIPFLAGS` &mdash; HIP C++ flags to be appended to `HIPFLAGS` when configuring and compiling ICON (defaults to an empty string);
- `ICON_BUNDLED_HIPFLAGS` &mdash; HIP C++ flags to be appended to `HIPFLAGS` when configuring the bundled packages (defaults to `ICON_HIPFLAGS`);
- `LDFLAGS` &mdash; common Fortran, C and C++ compiler flags to be used when configuring and linking ICON, as well as passed to the configure and CMake (via the `-DCMAKE_EXE_LINKER_FLAGS`, `-DCMAKE_MODULE_LINKER_FLAGS` and `-DCMAKE_SHARED_LINKER_FLAGS` arguments) scripts and of the bundled packages (defaults to an empty string);
