...
MODULE mo_art_sedi_interface
  ...
  USE mo_art_aerosol_utilities,         ONLY: akt
  ...
  USE mo_util_string,                   ONLY: str_replace
...
! sedimentation and deposition routines
  ...
  USE mo_art_impl_constants,            ONLY: IART_ACC_SEDIM, IART_VARNAMELEN
  USE mo_art_gas_depo,                  ONLY: art_calc_v_drydep
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_sedi_interface(p_patch, p_dtime, p_prog, prog_list, p_metrics, p_diag, &
...
! Local Variables
  ...
  INTEGER                      :: &
    ...
    &  jg, jsp, i, n, iv,         & !< jg: patch id, jsp/i/n/iv: counters
  ...
  CHARACTER(LEN=IART_VARNAMELEN) :: speciesname
...
!$omp end parallel do
    ...
    IF (art_config(jg)%lart_chem) THEN
      ...
      DO iv = 1, prog_list%p%nvars
        info_dyn => prog_list%p%vl(iv)%p%info_dyn
        IF(info_dyn%tracer%lis_tracer) THEN
          !Check for class of info_dyn%tracer  being t_CHEM_META_PARAM
          SELECT TYPE (meta => info_dyn%tracer)
            CLASS IS (t_chem_meta_param)
              IF(.NOT.meta%ldep_tracer) THEN
                CYCLE
              ENDIF
              speciesname = str_replace(TRIM(meta%name),"_chemtr","")
              CALL p_art_data(jg)%dict_tracer%get(TRIM(speciesname),tracer_id,ierror)
              IF(ierror/=SUCCESS) THEN
                CYCLE
              END IF

              ! loop over jb (parallel)
!$omp parallel do default(shared) private(jb, istart, iend, turb_id, i )
              DO jb = art_atmo%i_startblk, art_atmo%i_endblk
                turb_id=0
                DO i=1,art_config(jg)%nturb_tracer
                  IF( p_prog%turb_tracer(jb,i)%idx_tracer == tracer_id) THEN
                    turb_id = i
                    EXIT
                  END IF
                ENDDO
                IF(turb_id==0) CYCLE
                CALL art_get_indices_c(jg, jb, istart, iend)
                CALL art_calc_v_drydep(art_atmo%temp(:,nlev,jb),art_atmo%temp_ifc(:,nlev+1,jb),       &
                  &                    art_atmo%u(:,nlev,jb), art_atmo%v(:,nlev,jb),art_atmo%rho(:,nlev,jb),   &
                  &                    art_atmo%rh_2m(:,jb),                                      &
                  &                    art_atmo%tcm(:,jb), art_atmo%tch(:,jb),                    &
                  &                    tracer(:,nlev,jb,iqv),tracer(:,nlev,jb,iqc),               &
                  &                    tracer(:,nlev,jb,iqr), art_atmo%theta_v(:,nlev,jb),          &
                  &                    art_atmo%gz0(:,jb), art_atmo%dz(:,nlev,jb),                         &
                  &                    p_art_data(jg)%air_prop%art_dyn_visc(:,nlev,jb),           &
                  &                    istart,iend,meta%name,meta%vdmol1,meta%vdmol2,meta%reac,   &
                  &                    meta%heff, art_atmo%swflx_par_sfc(:,jb),                   &
                  &                    ext_data(jg)%atm%lc_class_t(:,jb,:),                           &
                  &                    ext_data(jg)%atm%frac_t(:,jb,:),                               &
                  &                    p_art_data(jg)%turb_fields%vdep(:,jb,turb_id))
...
!$omp end parallel do
            CLASS IS (t_chem_meta_mecca)
              IF(.NOT.meta%ldep_tracer) THEN
                CYCLE
              ENDIF
              speciesname = str_replace(TRIM(meta%name),"_full","")
              CALL p_art_data(jg)%dict_tracer%get(TRIM(speciesname),tracer_id,ierror)
              IF(ierror/=SUCCESS) THEN
                CYCLE
              END IF
