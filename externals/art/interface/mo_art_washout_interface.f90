...
MODULE mo_art_washout_interface
  ...
  USE mo_run_config,                    ONLY: lart,iqr,iqnr,iqs,iqc
...
! Washout Routines
  ...
  USE mo_art_diagnostics,               ONLY: art_save_aerosol_wet_deposition,  &
                                          &   art_save_gas_wet_deposition

  USE mo_art_chem_types,                ONLY: t_chem_meta_mecca
  USE mo_art_gas_depo,                  ONLY: art_calc_loss_wetdepo
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_washout_interface(pt_prog, pt_prog_list, pt_diag, dtime, p_patch, &
...
!!
  ...
  TYPE(t_var_list_ptr), INTENT(inout) :: &
    &  pt_prog_list                      !< List of prognostic variables
  ...
  REAL(wp),ALLOCATABLE    :: &
    ...
    &  loss_tracer_washout(:,:)  !< Loss of gas tracer due to washout [kg/kg]
  INTEGER                 :: &
            ...
            DO jb = i_startblk, i_endblk
              ...
              !2nd call: (washout due to convective precipitation)
                ...
                CALL finish('mo_art_washout_interface:art_washout_interface',  &
                  &  'iart_aero_washout > 0 is not (yet) supported on GPU')
#endif
...
!$omp end parallel do
    ...
    IF (art_config(jg)%lart_chem) THEN
      ...
      ALLOCATE(loss_tracer_washout(nproma,nlev))
      loss_tracer_washout=0.0_wp
      
      ! Loop over tracers
      ! get tracerlist
      DO iv = 1, pt_prog_list%p%nvars
        info_dyn => pt_prog_list%p%vl(iv)%p%info_dyn
        IF(info_dyn%tracer%lis_tracer) THEN
          SELECT TYPE (meta => info_dyn%tracer)
            CLASS IS (t_chem_meta_mecca)
              IF(meta%iwash_tracer == 0) THEN
                CYCLE
              ENDIF
              ...
              DO jb = i_startblk,i_endblk
                CALL get_indices_c(p_patch, jb, i_startblk, i_endblk, &
                &                istart, iend, i_rlstart, i_rlend)
                CALL art_calc_loss_wetdepo(tracer(:,:,jb,tracer_id), tracer(:,:,jb,iqr),&
                  &                tracer(:,:,jb,iqc), &
                  &                dtime, rho(:,:,jb), &
                  &                istart, iend, nlev, &
                  &                prm_diag%rain_con_rate_3d(:,:,jb), &
                  ...
                  &                meta%win, meta%wsub,&
                  &                loss_tracer_washout)
                CALL art_save_gas_wet_deposition(p_art_data(jg), loss_tracer_washout, tracer_id, &
                  &                rho, p_metrics%ddqz_z_full(:,:,:), jb, istart, iend, nlev)
              ENDDO ! jb
            CLASS DEFAULT
          END SELECT
        ENDIF ! lis_tracer
      ...
      DEALLOCATE(loss_tracer_washout)
