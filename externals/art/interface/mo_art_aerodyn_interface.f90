...
SUBROUTINE art_aerodyn_interface( p_patch,p_dtime,p_prog_list,p_prog, &
  &                               p_metrics,p_diag,tracer, prm_diag, tend, lacc )
...
#ifdef _OPENACC
    CALL finish('mo_art_aerodyn_interface:art_aerodyn_interface', &
...
!      ALLOCATE(ch2so4(nproma,nlev))  ! TEMP
      ...
      CALL init(rh, lacc=lzacc)
      CALL init(totmasscond, lacc=lzacc)
...
#ifdef _OPENACC
        CALL finish('mo_art_aerodyn_interface:art_aerodyn_interface', &
                &    'iart_isorropia > 0 is not (yet) supported on GPU!')
#endif
...
! Mode shift
      ...
      !$ACC WAIT
      !$ACC EXIT DATA DELETE(rh, totmasscond) IF(lzacc)
      DEALLOCATE(rh)
