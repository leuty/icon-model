  ...
  IF (lart) THEN 
    ...
    IF (l_output_step) THEN
      ...
      DO jb = art_atmo%i_startblk, art_atmo%i_endblk
        ...
        ! --------------------------------------
        CALL art_calc_aod(rho, p_trac, dz,                                      &
        ...
        IF (art_config(jg)%iart_dust > 0    .OR. &
...
#ifdef _OPENACC
          CALL finish('mo_art_diagnostics_interface:art_diagnostics_interface', &
            & 'iart_dust > 0, iart_seasalt > 0, iart_fire > 0, iart_volcano > 0 &
            & are not (yet) supported on GPU')
#endif
            ...
            DO WHILE(ASSOCIATED(this_mode))
              ...
              CLASS is (t_fields_2mom)
                ...
                CASE('dusta','dustb','dustc')
                  ...
                  CALL art_calc_aodvar(rho, p_trac, dz,                                     &
          ...
          CALL art_calc_bsc(rho, p_trac, dz,                                               &
            &               istart, iend, art_atmo%nlev, jb, jg, p_art_data(jg))
