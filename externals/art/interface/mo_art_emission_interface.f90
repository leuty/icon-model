...
! Emission Routines
  ...
  USE mo_art_emission_pollen,           ONLY: art_emiss_pollen, art_pollen_get_nstns, &
                                          ...
                                          &   art_prepare_saisl, art_emiss_spp
  ...
  IF (lart) THEN
    ...
    IF (art_config(jg)%lart_pntSrc) THEN
      ...
      CALL finish('mo_art_emission_interface:art_emission_interface', &
              &    'lart_pntSrc = .TRUE. is not (yet) supported on GPU!')
#endif
      ! Point sources
...
#ifdef _OPENACC
            CALL finish('mo_art_emission_interface:art_emission_interface', &
                    &    'iart_fire = 1 is not (yet) supported on GPU!')
#endif
                      ...
                      CASE('volc_fplume')
                        ...
                        DO inumb_volc = 1,art_config(jg)%iart_volc_numb
                          IF (p_art_data(jg)%fplume_init_all%p(inumb_volc)%tri_iblk_loc == jb) THEN
                            emiss_rateM(:,:,:) = 0.0_wp
                            emiss_rate0(:,:,:) = 0.0_wp
                            CALL art_fplume_emission(p_art_data(jg), inumb_volc, art_atmo%z_mc(:,:,jb),&
...
! JF:               END SELECT
              ...
              ! Emitting corresponding SPPs if applicable
              IF(ASSOCIATED(fields%spp_field)) THEN
!$omp parallel do default (shared) private (jb, istart, iend)
                DO jb = art_atmo%i_startblk, art_atmo%i_endblk
                  CALL art_get_indices_c(jg, jb, istart, iend)

                  CALL art_emiss_spp(fields, dtime, p_art_data(jg)%ext%pollen_prop, &
                    &                tracer(:,:,:,:), art_atmo%temp(:,:,jb),        &
                    &                art_atmo%pres(:,:,jb), art_atmo%exner(:,:,jb), &
                    &                iqv, istart, iend, nlev, jb) 
                END DO !jb
!$omp end parallel do
              END IF
...
#ifdef _OPENACC
        CALL finish('mo_art_emission_interface:art_emission_interface',  &
          &  'iart_volcano = 1 is not (yet) supported on GPU')
#endif
    ...
    IF (art_config(jg)%lart_chem) THEN
      ...
      END IF
    ENDIF !lart_chem
    CALL sync_patch_array_mult(SYNC_C, p_patch, ntracer, lacc=lacc, f4din=tracer(:,:,:,:))
