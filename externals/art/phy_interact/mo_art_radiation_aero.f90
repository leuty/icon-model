...
! Modifications:
!  2024-10-24: Enrico P. Metzner, KIT
!  - variable var_med_dia moved into opt_props%use_var_med_dia to avoid hardcoded switches
    ...
    DO WHILE(ASSOCIATED(this_mode))
      ...
      SELECT TYPE (fields=>this_mode%fields)
        CLASS IS (t_fields_2mom)
          ...
          CALL get_tau_vr(tracer,rho,jb,nlong,nshort,ks,ke,jcs,jce,jg,fields,tau_vr)
          CALL get_tau_s_vr(jb,nlong,nshort,ks,ke,jcs,jce,fields,tau_vr,tau_s_vr)
          CALL get_tauasy_vr(jb,nlong,nshort,ks,ke,jcs,jce,fields,tau_s_vr,tauasy_vr)
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE get_tau_vr(tracer,rho,jb,nlong,nshort,ks,ke,jcs,jce,jg,fields,tau_vr)
!<
...
! Modifications:
!  2024-10-24: Enrico P. Metzner, KIT
!  - switch for usage of variable median diameter is moved into opt_props
!! YYYY-MM-DD: <name>, <institution>
!! - ...
!>
  REAL(wp),INTENT(in)    ::  &
  ...
  DO jspec=1,nlong+nshort
    DO jk = ks, ke
      jk_vr = ke+1-jk
      IF (fields%opt_props%use_var_med_dia) THEN
!NEC$ ivdep
        DO jc = jcs,jce
          ...
          ENDIF
        ENDDO
      ELSE !> ~fields%opt_props%use_var_med_dia
!NEC$ ivdep
        DO jc = jcs,jce
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE get_tau_s_vr(jb,nlong,nshort,ks,ke,jcs,jce,fields,tau_vr,tau_s_vr)
!<
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE get_tauasy_vr(jb,nlong,nshort,ks,ke,jcs,jce,fields,tau_s_vr,tauasy_vr)
!<
