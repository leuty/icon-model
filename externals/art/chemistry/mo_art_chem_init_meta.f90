...
MODULE mo_art_chem_init_meta
  ...
  USE mo_art_read_linozv3,              ONLY: art_linoz_readv3
#ifdef __ART_GPL
  ...
  DO iv = 1, p_prog_list%p%nvars
    ...
    SELECT TYPE(meta => p_prog_list%p%vl(iv)%p%info_dyn%tracer)
      ...
      TYPE IS (t_chem_meta_linozv3)
        IF (.NOT. p_art_data(jg)%chem%param%linozv3%is_init) THEN
          CALL art_linoz_readv3(p_art_data(jg)%chem%param%linozv3,  &
                     &        art_atmo%nproma,                  &
                     &        art_atmo%nlev)
        END IF

        CALL meta%init(art_atmo%nproma,art_atmo%nlev,art_atmo%nblks)
        CALL linozv3_fill_init(meta,jg,p_prog_list%p%vl(iv)%p%info%ncontained)
