...
! ART
  ...
  USE mo_art_OH_chem_predcorr_method,   ONLY: art_calculate_OH_number_concentration, &
                                          &   art_calculate_O1D_number_concentration, &
...
!>
  ...
  IF (art_param%OH_chem_meta%is_init) THEN
    ...
    CALL art_calculate_O1D_number_concentration(jg,OH_chem_meta,                                &
           &  art_atmo%temp,art_chem%photo%rate(:,:,:,4))               !The fourth index (4) of photolysis rate refers to O1D.
  ...
  DO iv = 1, p_prog_list%p%nvars
    ...
    IF (info_dyn%tracer%lis_tracer) THEN
      ...
      SELECT TYPE(tracer => info_dyn%tracer)
        ...
        TYPE IS (t_chem_meta_linozv3)
          IF (tracer%polarchem == IART_POLARCHEM) THEN
            CALL tracer%set_tracer_linozv3(p_tracer_now(:,:,:,jsp),  &
                           &               p_tracer_now(:,:,:,art_indices%iTRNOy),&
                           &               p_tracer_now(:,:,:,art_indices%iTR_cold)&
                                          )
          ELSE
            CALL finish('mo_art_chemtracer:','no polarchem is implemented') 
          END IF

          CALL art_loop_cell_tracer(jg, tracer, art_calc_colv3)

          IF (tracer%O3_paramet == IART_LINOZ_ANA) THEN
            CALL art_loop_cell_tracer_td(jg, tracer,current_date,p_dtime, art_calc_linoz_anav3)
          ELSE
            CALL art_loop_cell_tracer_td(jg, tracer,current_date,p_dtime, art_calc_linozv3)
          END IF

          SELECT CASE(tracer%polarchem)
            CASE(IART_POLARCHEM)
              CALL art_loop_cell_tracer_td2(jg, tracer, p_dtime,  &
                          &                art_calc_linoz_polarchemv3)
            CASE(IART_LINOZ_LT)
              CALL art_loop_cell_tracer_td2(jg, tracer, p_dtime,   &
        ...
        TYPE IS (t_chem_meta_simnoy)
          IF (jsp == art_indices%iTRNOy) THEN
            ...
            SELECT CASE (tracer%polarchem)
              CASE (IART_POLARCHEM)
                CALL art_loop_cell_tracer_nonproma(jg, tracer,  &
                              &           art_noy_polarchem)
