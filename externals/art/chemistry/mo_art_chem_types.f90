...
! ART
  ...
  TYPE, EXTENDS(t_chem_meta) :: t_chem_meta_mecca
    ...
    INTEGER :: &
      ...
      &  init_mode
    REAL(wp) :: &
      &  win,     & ! in-cloud scavenging ratio
      &  wsub       ! sub-cloud scavenging ratio
...
!>
  ...
  ! Initializing parameters for drydepo routine

  CALL this%opt_meta%get('idep', idep_tracer, ierror)
  IF (ierror /= SUCCESS) THEN
    CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
    WRITE(tracer_name,'(A)') c_tmp
    WRITE (message_text,*) 'ART: WARNING: metadata idep for '// TRIM(tracer_name)//  &
                  &          ' not found, using default'
    CALL message ('mo_art_chem_types:init_param_meta', message_text)
    this%ldep_tracer = .FALSE.    !< Default value 
  ELSE
    ! ldep_tracer = .TRUE. if idep_tracer not equal to 0
    this%ldep_tracer = idep_tracer /= 0 
  ENDIF

  ! Read in heff, vdmol1, vdmol2, reac

  IF (this%ldep_tracer) THEN 

    CALL this%opt_meta%get('heff',this%heff, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_param_meta',  &
        &         'Element heff missing for tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%heff = 1.0E-2_wp   !< Default value 
    END IF

    CALL this%opt_meta%get('reac',this%reac, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_param_meta',  &
        &         'Element reac missing for tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%reac = 0.0_wp   !< Default value 
    END IF

    CALL this%opt_meta%get('vdmol1',this%vdmol1, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_param_meta',  &
        &         'Element vdmol1 missing for tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%vdmol1 = 1.0_wp   !< Default value 
    END IF

    CALL this%opt_meta%get('vdmol2',this%vdmol2, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_param_meta',  &
        &         'Element vdmol2 missing for tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%vdmol2 = 1.0_wp  !< Default value 
    END IF
  END IF 
...
!>
  ...
  IF (ierror /= SUCCESS) THEN
    ...
    WRITE (message_text,*) 'ART: WARNING: metadata idep for '// TRIM(tracer_name)//  &
                  &          ' not found, using default'
    CALL message ('mo_art_chem_types:init_mecca_meta', message_text)
    this%ldep_tracer = .FALSE.    !< Default value 
  ELSE
    ! ldep_tracer = .TRUE. if idep_tracer not equal to 0
    this%ldep_tracer = idep_tracer /= 0 
  ...
  IF (this%ldep_tracer) THEN 
    ...
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element heff missing for MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%heff = 1.0E-2_wp   !< Default value 
    END IF
    ...
    IF (ierror /= SUCCESS) THEN
      ...
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element reac missing for MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%reac = 0.0_wp   !< Default value 
    END IF
    ...
    IF (ierror /= SUCCESS) THEN
      ...
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element vdmol1 missing for the MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%vdmol1 = 1.0_wp   !< Default value 
    END IF
    ...
    IF (ierror /= SUCCESS) THEN
      ...
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element vdmol2 missing for the MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%vdmol2 = 1.0_wp  !< Default value 
    END IF
  END IF 

  ! Initializing parameters for wet depo routine

  this%win = 0.0_wp
  this%wsub = 0.0_wp

  CALL this%opt_meta%get('iwash', iwash_tracer, ierror)
  IF (ierror /= SUCCESS) THEN
    CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
    WRITE(tracer_name,'(A)') c_tmp
    WRITE (message_text,*) 'ART: WARNING: metadata iwash for '// TRIM(tracer_name)//  &
                  &          ' not found, using default (no washout)'
    CALL message ('mo_art_chem_types:init_mecca_meta', message_text)
    this%iwash_tracer = 0    !< Default value 
  ELSE
  ...
  IF (this%iwash_tracer > 0) THEN 

    CALL this%opt_meta%get('win',this%win, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element win missing for MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%win =  0.3e6_wp !< Default value (SO2)
    END IF

    CALL this%opt_meta%get('wsub',this%wsub, ierror)
    IF (ierror /= SUCCESS) THEN
      CALL key_value_storage_as_string(this%opt_meta,'name', c_tmp)
      WRITE(tracer_name,'(A)') c_tmp
      CALL message('mo_art_chem_types:init_mecca_meta',  &
        &         'Element wsub missing for MECCA tracer ' &
        &       // TRIM(tracer_name) // '. Using default value.')
      this%wsub = 0.15e6_wp  !< Default value (SO2)
    END IF
