...
MODULE mo_art_chem_utils
  ...
  USE mo_physical_constants,      ONLY: avo, argas, amw, amd, grav
...
SUBROUTINE art_calc_colv3(jg,jb,jcs,jce ,&
            &             nproma,nlev, tracer)
!<
! Adaption of SUBROUTINE art_calc_col to LinozV3
!>
  IMPLICIT NONE
  INTEGER, INTENT(in) ::  &
    &  jg,                & !< patch id
  ...
  ! we use interface levels. nlevp1 with z(nlevp1) = 0.
  ! z_ifc/pres_ifc(1) = interface between levels 1 and 2
  DO jk=2,nlev
    DO jc = jcs,jce
      tracer%column(jc,jk,jb) = tracer%tracer(jc,jk,jb)*1.e6_wp/avo    & ! mol/m^-3
                          &    / art_atmo%pres(jc,jk,jb)               & ! divide by air density
                          &    * (argas * art_atmo%temp(jc,jk,jb))     & ! to get mixing ratio
                          &    * ( art_atmo%pres_ifc(jc,jk+1,jb)       & ! delta pressure on interface layers 
                          &      - art_atmo%pres_ifc(jc,jk,jb) )       & ! is the weight of the layer
                          &    /(amd*1.e-3_wp*grav)                    & ! mol/m2 in the layer. Note: amd gram/mol. 
                          & + tracer%column(jc,jk-1,jb)
    ENDDO
  ENDDO
  ...
  DO jk=1,nlev
    DO jc = jcs,jce
      tracer%column(jc,jk,jb) = tracer%column(jc,jk,jb) / (0.44615e-3_wp)  ! Convert mol/m2 to DU
    END DO
  END DO
END SUBROUTINE art_calc_colv3
