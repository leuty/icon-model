...
!  USE mo_art_data,                      ONLY: akt, rakt
  ...
  REAL (wp), POINTER :: akt !< von-Karman constant (being associated by SUB 'art_sedi_interface')
...
! local variables
  ...
  IF(PRESENT(lstar_inv_out)) THEN
    lstar_inv_out = lstar_inv
  END IF
...
! Local Variables
  ...
  total_mass(istart:iend,kstart:nlev)   = 0._wp
  density(istart:iend,kstart:nlev)      = 0._wp
  diameter(istart:iend,kstart:nlev)     = dg0e
  ! diameter(istart:iend,kstart:nlev)     = 1.E-09_wp
  knudsen_nr(istart:iend,kstart:nlev)   = 0._wp
  third_moment(istart:iend,kstart:nlev) = 0._wp
