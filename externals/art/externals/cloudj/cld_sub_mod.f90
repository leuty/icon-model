!------------------------------------------------------------------------------
!          UCI fast-JX  cloud-JX v-7.3c (8/2015)                                        !
!------------------------------------------------------------------------------
!
...
!
      ...
      SUBROUTINE CLOUD_JX (U0,SZA,REFLB,SOLF,FG0,LPRTJ,PPP,ZZZ,TTT, &
             ...
             CLDFLAG,NRANDO,IRAN,LNRG,NICA,JCOUNT, SKPERD_out)
...
!---calling sequence variables
      ...
      real(wp),  intent(in)                    :: U0,SZA,REFLB,SOLF,FG0, CLDCOR
      ...
      real(wp),  intent(in), dimension(L1U  )  :: CLDF
...
!-----------------------------------------------------------------------
      ...
      real(wp), dimension(L1U)  :: LWPX,IWPX,REFFLX,REFFIX
      real(wp)  CLDFR, XRAN, FSCALE, QCAOD, WTRAN, G0LIQ,G0ICE

      real(wp), dimension(LWEPAR) ::  CLTL,CLTI, CLT,CLDX
      integer                   ::  NRG, IRANX
! max number of max-overlap cloud groups set at 9
      ...
      real(wp), dimension(CBIN_)  ::  CFBIN
      real(wp), dimension(ICA_) ::    WCOL,OCOL, OCDFS
!      real(wp), dimension(LWEPAR,ICA_) :: TCOL
      integer, dimension(ICA_) :: ISORT
      real(wp), dimension(LWEPAR+1)   :: TCLD,TTCOL,SOLT,TAUG
      real(wp), dimension(L1U-1,NJXU)::  VALJXXX
      real(wp),  dimension(NQD_)     :: WTQCA
...
!-----------------------------------------------------------------------
      call PHOTO_JX (U0,SZA,REFLB,SOLF, LPRTJ0, PPP,ZZZ,TTT,  &
                  ...
                  AERSP,NDXAER, L1U,ANU, VALJXX,NJXU, LDARK, SKPERD_out)
...
!-------------------------------------------------------------------------
      call ICA_NR(CLDX,CLT,IWPX,LWPX,ZZZ, CLDIW,LTOP,LNRG,CBIN_,ICA_, &
             CFBIN,NCLDF, GFNR,GCMX,GNR,GBOT,GTOP,GLVL,NRG,NICA)


!---call ICA_ALL to generate the weight and cloud total OD of each ICA
!-------------------------------------------------------------------------
      call ICA_ALL(CLT,LTOP,CBIN_,ICA_, CFBIN,     &
...
!-----------------------------------------------------------------------
      SUBROUTINE ICA_NR(CLDF,CLTAU,IWPX,LWPX,ZZZ,CLDIW,LTOP,LNRG,CBIN_, &
            ICA_,CFBIN,NCLDF, GFNR,GCMX,GNR,GBOT,GTOP,GLVL,NRG,NICA)
!-----------------------------------------------------------------------
...
!---set up for correlated max-overlap groups based on observations
      ...
      real(wp), dimension(NRG6_), parameter:: Zbin =                 &
          [0.d5, 1.5d5, 3.5d5, 6.0d5, 9.0d5, 13.d5]

      integer,intent(in) :: LTOP, LNRG, CBIN_, ICA_
      integer,intent(in),dimension(LTOP) :: CLDIW
      real(wp), intent(in),dimension(LTOP) :: CLDF, ZZZ
      real(wp), intent(inout),dimension(LTOP) :: CLTAU,IWPX,LWPX
      ...
      real(wp),  intent(out), dimension(CBIN_) :: CFBIN

      real(wp)   FBIN, FSCALE, CLF_MIN, CLF_MAX, FSCALE2
      integer                   ::  NRGX, NICAX
...
!-----------------------------------------------------------------------
      SUBROUTINE ICA_ALL(CLT,LTOP,CBINU,ICAU, CFBIN,CLDCOR,NCLDF,  &
...
!       doi:10.1029/2006JD008007
      ...
      real(wp),  intent(in), dimension(LTOP)  :: CLT
      real(wp),  intent(in), dimension(CBINU) :: CFBIN
      real(wp),  intent(in)                 :: CLDCOR
      real(wp),  intent(out),dimension(ICAU) :: WCOL,OCOL
!      real(wp),  intent(out),dimension(LTOP,ICAU) :: TCOL

      real(wp)  ODCOL,WTCOL,CF0(51),  FWT(10,51),FWTC(10,51),FWTCC(10,51)
      real(wp)  FIG1,FIG2,GCORR,GCOWT,CORRFAC, FCMX(10)
      integer I, II, IG1,IG2, G, L,  IGNR(10),GRP1,GRP2
      logical L_CLR1,L_CLR2  ,LSKIP
!-----------------------------------------------------------------------
        CF0(1) = 0.d0
      do L = 1,CBINU
        CF0(L+1) = CFBIN(L)
...
!   also no correlation fix for top MAX-GRP
      do G = 1,NRG-1
       ...
       if (.not.LSKIP) then
        ...
        do I = 2,GNR(G)
! correlation factor: increase fract-area of cloudy member under cloudy section of upper layer 
! (FIG2)
...
!-----------------------------------------------------------------------
      SUBROUTINE ICA_III(CLT,LTOP,CBINU,III, &
...
!      and loads the REFF/WPs for a FAST_JX calculation.
      ...
      integer, intent(in), dimension(9,CBINU+1) :: GFNR
      real(wp),  intent(in), dimension(LTOP)  :: CLT
      real(wp),  intent(out),dimension(LTOP) :: TCOL
...
!-----------------------------------------------------------------------
      ...
      real(wp),  intent(in), dimension(ICAU)      :: WCOL,OCOL

      real(wp), intent(out), dimension(NQDU)      :: WTQCA
      integer, intent(out), dimension(ICAU)     :: ISORT
      integer, intent(out), dimension(NQDU)     :: NQ1,NQ2,NDXQS

      real(wp),  dimension(ICA_) :: OCDFS, OCOLS
      integer I, N, N1, N2

      real(wp), parameter:: OD_QUAD(4) =[0.5d0, 4.0d0, 30.d0, 1.d9]
!-----------------------------------------------------------------------
      ISORT(:) = 0
      WTQCA(:)  = 0.d0
      NDXQS(:) = 0
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  :: U0, G0L,G0I
      integer, intent(in) :: LTOP, ICAU, CBINU, NICA, NRG
      real(wp),  intent(in), dimension(LTOP)  :: CLF,CLT, TAULC,TAUIC
      integer, intent(in), dimension(LTOP) :: NCLDF
      integer, intent(in), dimension(9)     :: GBOT,GTOP,GNR
      integer, intent(in), dimension(9,CBINU+1) :: GFNR
      real(wp), intent(in), dimension(ICAU)   :: WCOL
      real(wp), intent(out),dimension(LTOP+1)   :: TCLD,TTCOL,SOLT,TAUG
!---Local variables
      real(wp)  SOLZ, ATTEN, USZA, TAUALL
      integer II, L
...
!  calc g0 to get equivlent isootropic OD below
       ...
       enddo
       do II = 1,NICA
         call ICA_III(CLT,LTOP,CBIN_, II, &
            NCLDF, GFNR,GNR,GBOT,GTOP,NRG,NICA, TTCOL)
         SOLZ = 1.d0
        do L = LTOP,1,-1
...
!-----------------------------------------------------------------------
!  classic heapsort, sorts real(wp) array A(N) into ASCENDING order,
...
!                           and IX(N) = original loc. of largest A
      ...
      real(wp), dimension(ND),intent(in)  :: A
      real(wp), dimension(ND),intent(out) :: AX
