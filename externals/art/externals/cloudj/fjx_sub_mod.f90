!------------------------------------------------------------------------------
!     'fjx_sub_mod.F90'  for fast-JX code v7.4+
!------------------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
      ...
      real(wp),  intent(in)  ::  GMTIME,YGRDJ,XGRDI
      integer, intent(in)  ::  NDAY
      real(wp),  intent(out) ::  SZA,COSSZA,SOLFX
!
      real(wp)  LOCT
      real(wp)  SINDEC, SOLDEK, COSDEC, SINLAT, SOLLAT, COSLAT
!
...
!-----------------------------------------------------------------------
      ...
      real(wp),  intent(in)  :: YLATD
      integer, intent(in)  :: MONTH, L1U
      real(wp),  intent(in),  dimension(L1U+1) :: PPP
      real(wp),  intent(out), dimension(L1U)   :: TTT,OOO
      real(wp), dimension(LREF)   :: OREF2,TREF2,HREF2,CREF2
      real(wp), dimension(LREF+1) :: PSTD
      integer  K, L, M, N
      real(wp)   DDDL,DLOGP,F0,T0,H0,C0,PB,PC,XC
...
!---calling sequence variables
      ...
      real(wp),  intent(in)                    :: U0,SZA,REFLB,SOLF
      logical, intent(in)                    :: LPRTJ
      real(wp),  intent(in), dimension(L1U+1)  :: PPP,ZZZ
      real(wp),  intent(in), dimension(L1U  )  :: TTT,DDD,RRR,OOO,  &
                                                LWP,IWP,REFFL,REFFI
      real(wp),  intent(in), dimension(L1U,ANU):: AERSP
      integer, intent(in), dimension(L1U,ANU):: NDXAER
      real(wp), intent(out), dimension(S_+2,JXL1_) :: SKPERD_out
      

! reports out the JX J-values, upper level program converts to CTM chemistry J's
      real(wp), intent(out), dimension(L1U-1,NJXU)::  VALJXX
...
!-----these are dimensioned JXL_, and must have JXL_ .ge. L_
      real(wp), dimension(JXL1_+1) :: TTJ,DDJ,OOJ,ZZJ
      real(wp), dimension(JXL1_+1) :: PPJ
      integer,dimension(JXL2_+1) :: JXTRA
!
      real(wp), dimension(W_+W_r)         :: FJTOP,FJBOT,FSBOT,FLXD0,RFL
      real(wp), dimension(JXL_,W_+W_r)    :: AVGF, FJFLX
      real(wp), dimension(JXL1_,W_+W_r)   :: DTAUX, FLXD
      real(wp), dimension(8,JXL1_,W_+W_r) :: POMEGAX
!
      real(wp), dimension(S_,JXL1_)   ::  FFX
      real(wp), dimension(S_, 8)      ::  FFXNET
      real(wp), dimension(8)          ::  FFXNETS
!---flux/heating arrays (along with FJFLX,FLXD,FLXD0)
      real(wp)  FLXJ(JXL1_),FFX0,FXBOT,FABOT
      real(wp)  ODABS,ODRAY,ODL
!      real(wp)  ODI
      real(wp)  RFLECT,FREFS,FREFL,FREFI, FREF1,FREF2,PREF1,PREF2
      real(wp)  AMF2(2*JXL1_+1,2*JXL1_+1)
!------------key SCATTERING arrays for clouds+aerosols------------------
      real(wp)  QQEXT(S_),SSALB(S_),SSLEG(8,S_),OPTX(S_),DDENS
      real(wp)  OD(S_,JXL1_),SSA(S_,JXL1_),SLEG(8,S_,JXL1_)
      real(wp)  OD600(JXL1_)
      real(wp)  PATH,RH,RE_LIQ,RE_ICE,TE_ICE
!------------key arrays AFTER solving for J's---------------------------
      real(wp)  FFF(W_,JXL_)
      real(wp)  FLXUP(S_),FLXDN(S_),DIRUP(S_),DIRDN(S_)

      integer  LU,L2U, I,J,K,KR,KR0,KG,JG,L,M, NAER
!      integer  RATIO(S_)
      integer  KDOKR(W_+W_r),LDOKR(W_+W_r)
      real(wp)   XQO3,XQO2,TTTX, DPKL, SKPERD(S_+2,JXL1_)
      real(wp)   ODRRTM,FRRTM
!-----------------------------------------------------------------------
...
!---  UMich aerosols use relative humidity (RH)
        do M = 1,ANU
          ...
          if (PATH .gt. 0.d0) then
            if (NAER .gt. 2) then
              call OPTICA (OPTX,SSALB,SSLEG, PATH, NAER)
              do K = 1,S_
                OD(K,L)  = OD(K,L)  + OPTX(K)
                SSA(K,L) = SSA(K,L) + SSALB(K)*OPTX(K)
                do I = 1,8
...
! all these quantities (are for print) and are scaled to the input flux (SZA & solar distance)
            ...
            FFXNET(K,1) = FFXNET(K,1) + FLXD0(KG)  *SJSUB(K,JG)*SOLF*FW(K)
            ! direct(solar) flux dep onto LB (surface)
            FFXNET(K,2) = FFXNET(K,2) + FSBOT(KG)  *SJSUB(K,JG)*SOLF*FW(K)
            ! total solar into atmopshere+surface
            FFXNET(K,3) = FFXNET(K,3) + (FLXD0(KG)+FSBOT(KG))*SJSUB(K,JG)*SOLF*FW(K)  
            ! diffuse flux leaving top-of-atmos
            FFXNET(K,4) = FFXNET(K,4) + FJTOP(KG)  *SJSUB(K,JG)*SOLF*FW(K)            
            ! diffuse flux absorbed in atmos
            FFXNET(K,5) = FFXNET(K,5) + FFX0*SOLF*FW(K)                               
            ! total (dir+dif) absorbed at LB (surface)
            FFXNET(K,6) = FFXNET(K,6) + FABOT      *SJSUB(K,JG)*SOLF*FW(K)            
            ! direct flux dep onto LB (surface diags)
            FFXNET(K,7) = FFXNET(K,7) + FSBOT(KG)  *SJSUB(K,JG)*SOLF*FW(K)            
            ! diffuse flux dep onto LB (surface)
            FFXNET(K,8) = FFXNET(K,8) + FJBOT(KG)  *SJSUB(K,JG)*SOLF*FW(K)            
         endif
...
! convert to K/day
    ...
    !   write(6,'(1x,a,72(a6,3x))') 'L=  ',(TITLEJX(JIND(K)), K=1,NJX)
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  ::  DTAUX(JXL1_,W_+W_r),POMEGAX(8,JXL1_,W_+W_r)
      real(wp), intent(in)  ::  AMF2(2*JXL1_+1,2*JXL1_+1)
      real(wp), intent(in)  ::  U0, RFL(W_+W_r)
      integer, intent(in) ::  JXTRA(JXL2_+1), LDOKR(W_+W_r),LU
      real(wp), intent(out) ::  FJACT(JXL_,W_+W_r),FJTOP(W_+W_r),FJBOT(W_+W_r),FSBOT(W_+W_r)
      real(wp), intent(out) ::  FJFLX(JXL_,W_+W_r),FLXD(JXL1_,W_+W_r),FLXD0(W_+W_r)

      integer JNELEV(JXL1_)
!      integer JNDLEV(JXL_)
      integer JADDLV(JXL2_+1),JADDTO(JXL2_+1),L2LEV(JXL2_+1)
      integer I,II,K,L,LL,   L2,L2L,L22,LZ,LZZ,ND
      integer L1U,L2U,   LZ0,LZ1
      real(wp)   SUMT,SUMJ

      real(wp)  DTAU(JXL1_+1,W_+W_r),POMEGAJ(M2_,JXL2_+1,W_+W_r),TTAU(JXL2_+1,W_+W_r)
      real(wp)  FTAU2(JXL2_+1,W_+W_r),POMEGAB(M2_,W_+W_r)
      real(wp)  ATAUA,ATAUZ,XLTAU,TAUDN,TAUUP,DTAUJ,FJFLX0
      real(wp), dimension(W_+ W_r) :: TAUBTM,TAUTOP,FTOP,ZFLUX
!      real(wp), dimension(W_+ W_r) :: FBTM
!--- variables used in mie code-----------------------------------------
      real(wp), dimension(W_+W_r)           :: FJT,FJB
      real(wp), dimension(N_,W_+W_r)        :: FJ,FZ,ZTAU
      real(wp), dimension(M2_,N_,W_+W_r)    :: POMEGA
      real(wp), dimension(2*JXL1_,W_+W_r)   :: FLXD2
...
#ifndef __ART_TMP_IFDEF

          if (U0 .gt. 0.02d0) then
            FZ(LZZ,K) = FTOP(K) * exp((TAUTOP(K)-ZTAU(LZZ,K))/U0)
          else
            !print*, "THIS IS FBTM", FBTM(K), K
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  ::  POMEGA(M2_,N_,W_+W_r),FZ(N_,W_+W_r),ZTAU(N_,W_+W_r) &
                             ,RFL(W_+W_r),U0,ZFLUX(W_+W_r)
      real(wp), intent(out) ::  FJ(N_,W_+W_r),FJT(W_+W_r),FJB(W_+W_r)

      real(wp)  PM(M_,M2_),PM0(M2_)
      integer I, IM
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(out) ::  FJ(N_,W_+W_r),FJTOP(W_+W_r),FJBOT(W_+W_r)

      real(wp), dimension(M_,N_,W_+W_r)    ::  A,C,H,   RR

      real(wp), dimension(M_,M_,N_,W_+W_r) ::  B,AA,CC,  DD
      real(wp), dimension(M_,M_) ::  E
      real(wp)  SUMB,SUMBX,SUMT
      integer I, J, K, L
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  ::  POMEGA(M2_,N_),PM(M_,M2_),PM0(M2_)
      real(wp), intent(in)  ::  ZFLUX,RFL
      real(wp), intent(in),dimension(N_) :: FZ,ZTAU

      real(wp), intent(out),dimension(M_,M_,N_) ::  B,AA,CC
      real(wp), intent(out),dimension(M_,N_) ::  A,C,H

      integer I, J, L1,L2,LL
      real(wp)  SUM0, SUM1, SUM2, SUM3
      real(wp)  DELTAU, D1, D2, SURFAC
!
      real(wp), dimension(M_,M_) :: S,T,U,V,W
!---------------------------------------------
...
!
      ...
      real(wp), intent(in) ::    REFF         ! effective radius of liq water cloud
      real(wp), intent(out) ::   DDENS        ! density of cloud particle (g/cm^3)
      real(wp), intent(out)::    QQEXT(S_)    ! optical depth of layer
      real(wp), intent(out)::    SSALB(S_)    ! single-scattering albedo
      real(wp), intent(out)::    SSLEG(8,S_)  ! scatt phase fn (Leg coeffs)
...
!
      ...
      real(wp), intent(in) ::    TEFF         ! effective temperature of ice water cloud
      real(wp), intent(out) ::   DDENS        ! density of cloud particle (g/cm^3)
      real(wp), intent(out)::    QQEXT(S_)    ! optical depth of layer
      real(wp), intent(out)::    SSALB(S_)    ! single-scattering albedo
      real(wp), intent(out)::    SSLEG(8,S_)  ! scatt phase fn (Leg coeffs)
...
!>>> but the output OPTD, SSALB,SLEG now has a full SX-=27 wavelengths, not 5 (200-300-..-999mm)
      ...
      real(wp), intent(in)::     PATH         ! path (g/m2) of aerosol/cloud
      integer,intent(inout)::     K            ! index of cloud/aerosols
      real(wp), intent(out)::    OPTD(S_)    ! optical depth of layer
      real(wp), intent(out)::    SSALB(S_)   ! single-scattering albedo
      real(wp), intent(out)::    SLEG(8,S_) ! scatt phase fn (Leg coeffs)

      integer I,J, KK
      real(wp)  XTINCT, REFF,RHO
...
!------------------------------------------------------------------------------
      subroutine OPTICA (OPTD,SSALB,SLEG, PATH,K)
!------------------------------------------------------------------------------
...
!17 MDust4.0  (R.V. Martin generated phase fns)   reff=4.000___G=1.000_rho=2.600
      ...
      integer I,J,JMIE
      real(wp)  XTINCT, REFF,RHO,WAVE, QAAX,SAAX
...
!                      FF02, ...FF14(14%BC), BB00, BB02, ...BB30(30%BC)]
      ...
      real(wp), intent(out)::    SLEG(8,S_)  ! scatt phase fn (Leg coeffs)
      real(wp), intent(in)::     PATH       ! path (g/m2) of aerosol/cloud
      real(wp), intent(in)::     RELH       ! relative humidity (0.00->1.00)
      integer,intent(in)::     LL         ! index of cloud/aerosols

      integer KR,J,L, JMIE
      real(wp)  GCOS, XTINCT, WAVE
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  ::  PPJ(LU+1),TTJ(LU+1)
      real(wp), intent(inout)  ::  FFF(W_,LU)
      real(wp), intent(out), dimension(LU,NJXU) ::  VALJL

      real(wp)  VALJ(X_)
      real(wp)  QO2TOT, QO3TOT, QO31DY, QO31D, QQQT
      real(wp)  TT,PP
      integer J,K,L
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)::  TINT,T1,T2,T3, X1,X2,X3
...
!---the pressure and altitude(Z) are on layer edge (LU+2)
      ...
      real(wp), intent(in), dimension(LU+1) :: DDJ,OOJ
!-----------------------------------------------------------------------
      integer  L
      real(wp)   XCOLO2,XCOLO3
!      real(wp)   DAIR,DOZO
...
!-----------------------------------------------------------------------
      ...
      real(wp), intent(in)  ::   U0,RAD,ZHL(L1U+1)
      real(wp), intent(out) ::   AMF2(2*LJX1U+1,2*LJX1U+1)

      integer, parameter  ::  LSPH_ = 130
...
!     XL      Slant path between points
      ...
      integer  I, J, II, L2
      real(wp)   XMU1,XMU2,XL,DIFF,SHADHT,RZ(LSPH_+1)
      real(wp)   RZ2(2*LSPH_+1),RQ2(2*LSPH_+1)
...
!
      ...
      real(wp),  intent(in) ::  DTAUX(L1X)      !cloud+3aerosol OD in each layer
      real(wp),  intent(in) ::  ATAU,ATAU0
      integer, intent(out)::  JXTRA(L2X+1)    !number of sub-layers to be added
!
      integer JTOTL,L,L2
      real(wp)  TTAU(L2X+1),DTAUJ, ATAULN,ATAUM,ATAUN1
!
!---Reinitialize arrays
      TTAU(:)  = 0.d0
      JXTRA(:) = 0
