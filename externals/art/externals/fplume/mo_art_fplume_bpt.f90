...
MODULE mo_art_fplume_bpt 
  ...
  USE mo_art_fplume_dlsode,                 ONLY: dlsode
...
SUBROUTINE initialize_plume_wind(profile_nz,profile_z,profile_rho,profile_p,profile_T,  &
  ...
  REAL   (wp), INTENT(IN) :: profile_z   (profile_nz-1) !< Height of layers
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE solve_plume_BPT(fplume_phase_value,plume_status,plume_MER,plume_z)
  !************************************************************************
  !*
    ...
    SELECT CASE(plume%solve_for)
      ...
      !
        ...
        ! n_MFR_max = 0.5_wp*(n_MFR_min+n_MFR_max)
          IF (MFR_iter>2) n_MFR_max = LOG10(M0)
        END IF
        !
        MFR_iter = MFR_iter + 1
        !
  ...
  ELSE
    ...
    DO is = 1,plume%ns-plume%np
      ...
      CALL temperature_mixture(xa,xp,xw,plume%T(plume%np+is), &
         & Pa(plume%z(plume%np+is)),plume%xv(plume%np+is), &
