...
SUBROUTINE reainp(luinpname,iphase, tc_exp_refdate,tc_dt_model,fplume_init)
  ...
  REAL(wp)  :: phase_H2O,plume_H2O(plume_ndt_max)   ! MER of H2O
  ...
  ELSE
     phase_SO2 = plume_SO2(iphase) ! Copy default values
  ENDIF
  !
  CALL get_input_npar(luinpname,'SOURCE','MER_H2O',ndt0,istat,mess)
  IF (istat==0) THEN
     CALL get_input_rea(luinpname,'SOURCE','MER_H2O',plume_H2O,ndt0,istat,mess)
     IF (istat>0) THEN
       WRITE(message_text,*) mess
       CALL message('',message_text)
     ENDIF
     IF (istat<0) CALL finish(thisroutine,mess)
     phase_H2O = plume_H2O(iphase)
  ELSE
     phase_H2O = plume_H2O(iphase)  ! Copy default values
  END IF
  !
  ...
  CALL fplume_init%phase(iphase)%init(phase_beg,phase_end,phase_SO2,phase_H2O,                   &
