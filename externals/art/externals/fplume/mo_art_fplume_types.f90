...
! ART
  ...
  TYPE t_fplume_phases
    ...
    REAL(wp), ALLOCATABLE :: prof_z(:)                ! profiles from file (if ext_meteo_prof =.TRUE.)
    REAL(wp), ALLOCATABLE :: prof_rho(:)
    REAL(wp), ALLOCATABLE :: prof_T(:)
    REAL(wp), ALLOCATABLE :: prof_u(:)
    REAL(wp), ALLOCATABLE :: prof_v(:)
    REAL(wp), ALLOCATABLE :: prof_sh(:)
    REAL(wp), ALLOCATABLE :: prof_pres(:)
    REAL(wp)              :: vset_aggr
    REAL(wp)              :: diam_aggr
    REAL(wp)              :: Dfo
...
SUBROUTINE init_fplume_phase(this_phase,phase_b,phase_e,phase_SO2,phase_H2O,phase_Hdt,phase_Mdt, &
  ...
  REAL(wp), INTENT(IN)    :: &
    ...
    &  phase_SO2,              &          ! MER of SO2
    &  phase_H2O                          ! MER of H2O
