...
! ART
  ...
  PUBLIC :: art_save_gas_wet_deposition
...
!!-------------------------------------------------------------------------
!!
SUBROUTINE art_save_gas_wet_deposition(p_art_data, loss_tracer,    &
  &           idx_tracer, rho, dz, jb, istart, iend, nlev)
  !<
  ! SUBROUTINE save_art_gas_wet_deposition
  ! computes the accumulated 2d wet deposition of gases
  !>
...
! --- in/out --------------------------------------------------------------
  ...
  REAL(wp),            INTENT(in)    :: &
    & loss_tracer(:,:),                 & !< Loss of gas tracer due to washout
                                          ! tracer_before - tracer_after
    & dz(:,:,:),                        & !< Vertical layer thickness (m)   
    & rho(:,:,:)                          !< Air density (kg/m3)  
...
! -- begin ---------------------------------------------------------------
  ...
  ! get index in diagnostics container (given the index in the tracer container)
    idx_diag = art_diag_tracer_index(IART_ACC_WETDEPO_GAS, idx_tracer)

    ! save wet deposition
    IF ( idx_diag > 0 ) THEN
      ! compute accumulated wet deposition of tracer (gas-phase)
      DO jk = 1, nlev
        DO jc = istart, iend
          p_art_data%diag%acc_wetdepo_gas(jc,jb,idx_diag) =      &
            & p_art_data%diag%acc_wetdepo_gas(jc,jb,idx_diag)    &
            & + loss_tracer(jc,jk) * rho(jc,jk,jb) * dz(jc,jk,jb)
        ENDDO !jc
      ENDDO !jk
    END IF


END SUBROUTINE art_save_gas_wet_deposition
!!
!!-------------------------------------------------------------------------
!!
