            ...
            IF (current_date%time%hour == 0) THEN
              ...
              ELSE IF (jsp == art_indices%iTR_bgn) THEN
                CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_bgn),   &
                  &          art_emiss_TR_bgn)
              ELSE IF (jsp == art_indices%iTR_gol) THEN
                CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_gol),   &
                  &          art_emiss_TR_gol)
              END IF
            END IF

            IF (jsp == art_indices%iTR_phto) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phto),   &
                &          art_emiss_TR_phto)
            ELSE IF (jsp == art_indices%iTR_phnwp) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnwp),   &
                &          art_emiss_TR_phnwp)
            ELSE IF (jsp == art_indices%iTR_phnep) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnep),   &
                &          art_emiss_TR_phnep)
            ELSE IF (jsp == art_indices%iTR_phnao) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnao),   &
                &          art_emiss_TR_phnao)
            ELSE IF (jsp == art_indices%iTR_phnpo) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnpo),   &
                &          art_emiss_TR_phnpo)
            ELSE IF (jsp == art_indices%iTR_phnin) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnin),   &
                &          art_emiss_TR_phnin)
            ELSE IF (jsp == art_indices%iTR_phsin) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phsin),   &
                &          art_emiss_TR_phsin)
            ELSE IF (jsp == art_indices%iTR_phech) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phech),   &
                &          art_emiss_TR_phech)
            ELSE IF (jsp == art_indices%iTR_phsea) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phsea),   &
                &          art_emiss_TR_phsea)
            ELSE IF (jsp == art_indices%iTR_pheur) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_pheur),   &
                &          art_emiss_TR_pheur)
            ELSE IF (jsp == art_indices%iTR_phmea) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phmea),   &
                &          art_emiss_TR_phmea)
            ELSE IF (jsp == art_indices%iTR_phnaf) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnaf),   &
                &          art_emiss_TR_phnaf)
            ELSE IF (jsp == art_indices%iTR_phnam) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phnam),   &
                &          art_emiss_TR_phnam)
            ELSE IF (jsp == art_indices%iTR_phcas) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_phcas),   &
                &          art_emiss_TR_phcas)
            ELSE IF (jsp == art_indices%iTR_ascci_eur) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_eur),   &
                &          art_emiss_TR_ascci_eur)
            ELSE IF (jsp == art_indices%iTR_ascci_nam) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_nam),   &
                &          art_emiss_TR_ascci_nam)
            ELSE IF (jsp == art_indices%iTR_ascci_cas) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_cas),   &
                &          art_emiss_TR_ascci_cas)
            ELSE IF (jsp == art_indices%iTR_ascci_nwp) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_nwp),   &
                &          art_emiss_TR_ascci_nwp)
            ELSE IF (jsp == art_indices%iTR_ascci_nep) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_nep),   &
                &          art_emiss_TR_ascci_nep)
            ELSE IF (jsp == art_indices%iTR_ascci_nat) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_nat),   &
                &          art_emiss_TR_ascci_nat)
            ELSE IF (jsp == art_indices%iTR_ascci_tro) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_tro),   &
                &          art_emiss_TR_ascci_tro)
            ELSE IF (jsp == art_indices%iTR_ascci_arc) THEN
              CALL art_loop_cell_array(jg, p_tracer_now(:,:,:,art_indices%iTR_ascci_arc),   &
                &          art_emiss_TR_ascci_arc)
            ENDIF
