...
! ICON
  ...
  USE mo_physical_constants,            ONLY: argas, rhoh2o
  ...
  USE mo_art_modes,                     ONLY: t_fields_pollen, t_fields_2mom
...
!!!-------------------------------------------------------------------------
!!!
  SUBROUTINE art_emiss_spp(pollen_fields, p_dtime, pollen_prop, p_tracer,  &
    &                      p_temp, p_pres, p_exner, iqv, istart, iend, nlev, jb)                  
  !!-----------------
  !!-- This subroutine computes the emission of subpollen particles (SPP)
  !!-- through the bursting of pollen in sufficiently humid environments
  !!-- by parameterizing the turgor pressure increase and determining
  !!-- the fraction of pollen to burst regarding the maximum turgor pressure
  !!-- they can hold. (for more details see (Werchner et al., 2022))
  !!-----------------
  !!-- Author: Sven Werchner, KIT
  !!-----------------
  
    TYPE(t_art_pollen_properties), INTENT(INOUT) :: &
      &  pollen_prop              !< pollen property container
      
    TYPE(t_fields_pollen), INTENT(INOUT) :: &
      &  pollen_fields            !< pollen mode
    
    REAL(wp), INTENT(IN) ::     &
      &  p_dtime,               & !< time step
      &  p_temp(:,:),           & !< temperature (K)
      &  p_pres(:,:),           & !< pressure (Pa)
      &  p_exner(:,:)             !< exner function (needed for RH)
    REAL(wp), INTENT(INOUT) ::  &
      &  p_tracer(:,:,:,:)        !< concentration of pollen 
                                  !< (and subpollen) species
      
    INTEGER, INTENT(IN) ::      &
      &  iqv,                   & !< water vapour tracer index
      &  istart, iend,          & !< start and end of nproma
      &  nlev, jb                 !< number of levels
    ...
    INTEGER            :: &
      ...
      &  jpoll,           & !< counter for pollen in pollen prop
      &  ipoll,           & !< Index of (current) pollen particle in 
      &  ispp0,ispp3        !< tracer container ("source")
      
    REAL(wp)           :: &
      &  poll_diam,       & !< pollen diameter
      &  poll_rho,        & !< pollen density
      &  rh,              & !< relative humidity
      &  spp_rh_thresh,   & !< value at which pollen burst can occur
      &  spp_rh_max,      & !< value corresponding to pollen wall strength
      &  deltaLnRHmax,    & !< = ln(spp_rh_max) - ln(spp_rh_thresh)
      &  spp_delta_r,     & !< expansion radius (before bursting)
      &  factor,          & !< determines amount of pollen bursting
      &  conversion,      & !< #/kg -> ug/kg
      &  conversion0,     & !< #poll/kg -> #spp/kg
      &  ratio,           & !< needed for normaldistribution
      &  turgorCrit,      & !< Critical turgor pressure (regarding burst)
      &  turgornew          !< turgor pressure built up in this timestep
                            !<  (currently only temporarily available)
                            
    REAL(wp), PARAMETER ::  &
      &  E   = 2.15e9_wp,   & !< Bulk modulus fluid elasticity of water (in Pa)
      &  k   = 5e-17_wp,    & !< water permeability of pollen (currently wheat)
                              !    unit: m s-1 Pa-1
      &  R_gas = argas,     & !< gas constant (J mol-1 K-1)
      &  M_W = 1.807e-5_wp, & !< mol. volume of water (m3 mol-1)
      &  mu  = 0.5_wp,      & !< used for normaldistribution (corresponds to RH=90.06%)
      &  sigma = 0.181925_wp  !< used for mormaldist -> mu-sigma corresponds to RH=88%
    ...
    !----------------------------------------------------------------------------
    poll_diam = pollen_fields%diam
    poll_rho  = pollen_fields%rho
    jpoll     = pollen_fields%pollen_prop_idx
    ipoll     = pollen_fields%itr 
    spp_field => pollen_fields%spp_field
    ispp0     = spp_field%itr0
    ispp3     = spp_field%itr3(1)
    ! set necessary pollen-spp-data
    spp_rh_thresh = pollen_prop%pollen_type(jpoll)%spp_rh_thresh
    spp_rh_max    = pollen_prop%pollen_type(jpoll)%spp_rh_max
    spp_delta_r   = pollen_prop%pollen_type(jpoll)%spp_delta_r
    deltaLnRHmax  = LOG(spp_rh_max/spp_rh_thresh)
  
    conversion    = pi / 6._wp * poll_diam**3 * poll_rho * 1.e9_wp
    conversion0   = poll_diam**3 * EXP(4.5_wp * LOG(spp_field%info%sg_ini)**2) &
      &           / spp_field%info%diameter_ini_mass**3
  
      
    !----------------------------------------------------------------------------
    !-- Perform computation of pollen bursting ----------------------------------
    !----------------------------------------------------------------------------
    DO jk = 1, nlev
      DO jc = istart, iend
        ! Cycle if no pollen present ...
        IF (p_tracer(jc,jk,jb,ipoll) == 0.0_wp) CYCLE
        
        ! ... or if RH-threshold for this SPP is not met
        rh = rel_hum(p_temp(jc,jk),p_tracer(jc,jk,jb,iqv),p_exner(jc,jk)) / 100._wp
        IF (rh < spp_rh_thresh) CYCLE


        ! Burst the pollen particles and produce subpollen particles
        ! - Calculate critical turgor pressure
        !     turgorCrit = p_0 + R*T/M_W * (log(0.96) - log(0.845))<< birch
        ! - Calculate change in turgor pressure (in this timestep)
        !     turgornew = p_0 + R*T/M_W * (log(p_rh)-log(0.845)) * (1 - 
        !                 exp(-3*E*k/r*rho_w/rho_0*p_dtime))
        ! - Burst pollen amount depending on ratio between new and max
        !   turgor pressure (via normal-distribution of turgor)
        
        turgorCrit = p_pres(jc,jk) + R_gas * p_temp(jc,jk) / M_W * deltaLnRHmax
        turgornew  = p_pres(jc,jk) + R_gas * p_temp(jc,jk) / M_W * LOG(rh/spp_rh_thresh) &
          &        * (1._wp - EXP(-3._wp * E * k / (poll_diam/2.0_wp + spp_delta_r)  &
          &        * rhoh2o / poll_rho * p_dtime))
  
        ! maximum turgor is turgorCrit
        IF (turgornew > turgorCrit) turgornew = turgorCrit

        ! Normal-Distribute turgor-pressure
        ratio = (turgornew - p_pres(jc,jk))/(turgorCrit - p_pres(jc,jk))
        ratio = (ratio - mu)/(SQRT(2._wp) * sigma)

        factor = 0.5_wp + SIGN(0.5_wp,ratio) * SQRT(1._wp - EXP(-4._wp * ratio * ratio / pi))
        
        p_tracer(jc,jk,jb,ispp3) = p_tracer(jc,jk,jb,ispp3) + factor       &
          &                      * p_tracer(jc,jk,jb,ipoll) * conversion
        p_tracer(jc,jk,jb,ispp0) = p_tracer(jc,jk,jb,ispp0) + factor       &
          &                      * p_tracer(jc,jk,jb,ipoll) * conversion0
        p_tracer(jc,jk,jb,ipoll) = p_tracer(jc,jk,jb,ipoll) * ( 1._wp      &
          &                      - factor )
