...
!+un 12.3.20
  ...
  USE mo_art_modes,                     ONLY: t_fields_2mom, t_fields_pollen
  ...
  USE mo_art_init_radiation,            ONLY: art_init_radiation,            &
    &                                         art_init_radiation_diagnostics
  USE mo_art_read_opt_props,            ONLY: art_open_optprops_nc,          &
    &                                         art_close_optprops_nc
#ifdef __ART_GPL
...
!  irad_aero of aes has to be taken into account when running aes physics
  ...
  !OEM
  INTEGER :: ierror, j
  INTEGER :: ncid                  !< access identifier for prescriped optics netcdf
  LOGICAL :: loaded_netcdf
  ...
  IF (art_config(jg)%iart_fplume /= 0) THEN
    CALL art_fplume_init(p_patch(jg),p_art_data(jg)%fplume_init_all,art_config(jg)%iart_volc_numb,&
                       & art_config(jg)%cart_fplume_inp,tc_exp_refdate,tc_dt_model,art_atmo%nlev)
  ENDIF
...
! the value given in the namlist be ignored
    ...
    ENDIF
!+un  12.3.20
    IF ( (irad_aero == 9 .AND. artconf%iart_ari>0) .OR. artconf%lart_diag_out ) THEN
        ...
        DO WHILE(ASSOCIATED(current_mode))
          SELECT TYPE (fields=>current_mode%fields)
            CLASS IS (t_fields_2mom)
              ...
              IF (.NOT. loaded_netcdf) THEN
                !wrap open call in this if, so that it is only open if it realy necessary
                !e.g. avoids opening in diagnostic cases without aerosols interacting with radiation
                CALL art_open_optprops_nc(jg, ncid)
                loaded_netcdf = .TRUE.
              END IF

              IF (irad_aero==9 .AND. artconf%iart_ari>0) THEN
                CALL art_init_radiation(jg, ncid,                        &
                &                     TRIM(fields%opt_props%lut_optics), &
                &                     fields%opt_props%use_var_med_dia,  &
              ...
              END IF
              IF (artconf%lart_diag_out) THEN
                CALL art_init_radiation_diagnostics(ncid,                &
                &                   TRIM(fields%opt_props%lut_optics),   &
                &                   fields%diag_opt_props%ext_aeronet,   &
                &                   fields%diag_opt_props%ext_snglwave,  &
                &                   fields%diag_opt_props%ext_satellite, &
                &                   fields%diag_opt_props%bsc_satellite)
              END IF
            CLASS DEFAULT
          END SELECT
        ...
        IF (loaded_netcdf) THEN
          CALL art_close_optprops_nc(ncid)
        END IF

      ENDIF

    ! Pollen:
    ! Enter index in pollen_prop into fields-container
    ! (used for access to meta information in emission-spp)
    current_mode => p_mode_state(jg)%p_mode_list%p%first_mode
    DO WHILE(ASSOCIATED(current_mode))
      SELECT TYPE (fields => current_mode%fields)
        CLASS IS (t_fields_pollen)
          DO j=1,p_art_data(jg)%ext%pollen_prop%npollen_types
            IF( TRIM(ADJUSTL(fields%name)) ==                                &
              & TRIM(ADJUSTL(p_art_data(jg)%ext%pollen_prop%pollen_type(j)%sname))) THEN
              fields%pollen_prop_idx = j
              EXIT
            END IF
          END DO
        CLASS DEFAULT
