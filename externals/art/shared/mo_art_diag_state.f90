...
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'iart_radioact = 1 is not (yet) supported on GPU')
#endif
...
!  </aerosol>
    ...
    IF (art_config(jg)%lart_chem) THEN
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'lart_chem = .TRUE. is not (yet) supported on GPU')
#endif
      ...
      cf_desc    = t_cf_var('O1D_Nconc', '# / cm3', 'O1D_Nconc', datatype_flt)                
      grib2_desc = grib2_var(0, 254, 253, DATATYPE_PACK16, GRID_UNSTRUCTURED, GRID_CELL)        
  
      CALL add_var( p_diag_list, 'O1D_Nconc', art_chem%param%OH_chem_meta%O1D_Nconc,     &      
...
#ifdef _OPENACC
        CALL finish('mo_art_diag_state:art_create_diagnostics',  &
          &  'lart_mecca = .TRUE. is not (yet) supported on GPU')
#endif
#ifdef __ART_GPL
...
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'iart_aci_warm > 0 is not (yet) supported on GPU')
#endif
...
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'iart_aci_cold > 0 is not (yet) supported on GPU')
#endif
...
!  ENDIF !lart_diag_out
  ...
  IF (art_config(jg)%lart_chem) THEN
...
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'lart_psc = .TRUE. is not (yet) supported on GPU')
#endif
#ifdef __ART_GPL
...
#ifdef _OPENACC
      CALL finish('mo_art_diag_state:art_create_diagnostics',  &
        &  'lart_chemtracer = .TRUE. is not (yet) supported on GPU')
#endif
    ...
    ! -----------------------------------------------------
    ! -- Diagnostics for wet deposition of gases ----------
    ! -----------------------------------------------------

    cf_desc    = t_cf_var('acc_wetdepo_gas', 'tracer-unit m-2',                                 &
        &             'accumulated wet deposition of trace gases', datatype_flt)
    grib2_desc = grib2_var(255, 255, 255, DATATYPE_PACK16, GRID_UNSTRUCTURED, GRID_CELL)
    
    IF (lexist_diagxml) THEN
      CALL art_get_GRIB2_diag('acc_wetdepo_gas', grib2_desc, var_class, l_diag, l_meteogram,        &
          & p_prog_list = p_prog_list, shortnames_active_tracer = shortnames_diag,           &
          & parameterNumber_list = parameterNumbers_diag, ntrac_active_tracer = ntrac_diag)
    
      IF (ntrac_diag > 0) THEN
        shape2d_t = (/ nproma, p_patch(jg)%nblks_c , ntrac_diag /)
        CALL add_var(p_diag_list, 'acc_wetdepo_gas', art_diag%acc_wetdepo_gas,                          &
             & GRID_UNSTRUCTURED_CELL, ZA_SURFACE, cf_desc, grib2_desc,                      &
             & ldims=shape2d_t,lrestart=.FALSE.,loutput=.FALSE.,lcontainer=.TRUE.)
        ALLOCATE(art_diag%acc_wetdepo_gas_ptr(ntrac_diag))
        DO jt = 1,ntrac_diag
          WRITE(tracer_name,'(A)')shortnames_diag(jt)
          ! reset grib2_desc for each tracer, get tracer specific settings
          grib2_desc = grib2_var(255, 255, 255, DATATYPE_PACK16, GRID_UNSTRUCTURED, GRID_CELL)
          ...
          IF ( ASSOCIATED(jsp_idx) ) THEN
            WRITE(message_text,'(A,A,A,I3,A,I3)')                                 &
              &   'ART: a reference is added for diagnostic : acc_wetdepo_gas_',        &
              &   TRIM(tracer_name),', idx_diag = ',jt, ', idx_tracer = ',jsp_idx
            CALL message (TRIM(thisroutine), message_text)
            IF (jg == 1) art_diag_tracer_index(IART_ACC_WETDEPO_GAS, jsp_idx) = jt
            CALL get_and_set_int_grib2_key('discipline', grib2_tracer, grib2_desc)
            CALL get_and_set_int_grib2_key('parameterCategory', grib2_tracer, grib2_desc)
            grib2_desc%number = parameterNumbers_diag(jt)
            cf_desc = t_cf_var('acc_wetdepo_gas_'//TRIM(tracer_name), 'tracer-unit m-2',   &
              &                'accumulated wet deposition of '//TRIM(tracer_name), datatype_flt)
            ! For each specified tracer, add list reference to basic data container
            CALL add_ref(p_diag_list, 'acc_wetdepo_gas',                                          &
              &          'acc_wetdepo_gas_'//TRIM(tracer_name), art_diag%acc_wetdepo_gas_ptr(jt)%p_2d,  &
              &          GRID_UNSTRUCTURED_CELL, ZA_SURFACE, cf_desc, grib2_desc,           &
              &          hor_interp=create_hor_interp_metadata(hor_intp_type =              &
              &                                                HINTP_TYPE_LONLAT_BCTR),     &
              &          ref_idx = jt, var_class=var_class, ldims=shape2d_c,                &
          ...
          ELSE
            WRITE(message_text,'(a,a,a)')                                          &
              &   'ART: WARNING: diagnostic acc_wetdepo_gas was requested for tracer: ', &
              &    tracer_name, ', but such a tracer does not exis.'
            CALL message (TRIM(thisroutine), message_text)
          END IF
        ENDDO
      ELSE
        WRITE(message_text,'(A)') 'ART: WARNING: diagnostic acc_wetdepo_gas could not be '// &
          &                       'found in diagnostics.xml file.'
        CALL message (TRIM(thisroutine), message_text)
      END IF ! ntrac_diag > 0
    END IF ! lexist_diagxml
