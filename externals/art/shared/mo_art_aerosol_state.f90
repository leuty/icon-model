...
! Local variables
  TYPE(t_mode),POINTER                  :: &
    &  this_mode, that_mode                  !< References to current mode in list
  TYPE(t_fields_2mom),POINTER           :: &
...
!--------------------------------------------------------------------------------------------------
      CLASS IS(t_fields_1mom)
        ...
        DO iv = 1, this_list%p%nvars
          ...
          ENDIF
        ENDDO
        ! Linking SPP-fields for pollen
        SELECT TYPE(fields => this_mode%fields)
          TYPE IS(t_fields_pollen)
            that_mode => p_mode_state%p_mode_list%p%first_mode
            DO WHILE(ASSOCIATED(that_mode))
              SELECT TYPE(that_fields => that_mode%fields)
                TYPE IS(t_fields_2mom)
                  IF(TRIM(that_fields%name) == TRIM(fields%spp_name)) THEN
                    fields%spp_field => that_fields
                    WRITE(message_text,*) 'ART: '//TRIM(fields%spp_field%name)//' is the corresponding SPP for '//TRIM(fields%name)
                    CALL message (TRIM(routine)//':art_assign_tracers_to_modes', message_text)
                    EXIT
                  END IF
                CLASS DEFAULT
