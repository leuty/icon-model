...
! ICON
  ...
  USE mo_var_metadata_types,            ONLY: t_var_metadata, t_var_metadata_dynamic,  &
                                          &   POST_OP_NONE, POST_OP_SCALE,             &
                                          &   t_post_op_meta, t_vert_interp_meta,      &
...
! Local variables
  ...
  TYPE(t_var_metadata_dynamic),POINTER     :: &
    &  target_info_dyn                          !< Pointer to dynamic metadata of target_element 
                                                !  (i.e. tracer meta)
  TYPE(t_cf_var)                           :: &
    &  cf, new_cf                               !< NETCDF variable metadata, ... for post operation
  TYPE(t_grib2_var)                        :: &
  ...
  ! for post operations
  TYPE(t_post_op_meta)                     :: &
    &  post_op_arg                              !< post operation argument for add_ref
  INTEGER                                  :: &
  ...
  CHARACTER(:), ALLOCATABLE                :: &
    ...
    &  scale_units,                           & !< Unit of tracer after scaling in post operation
  ...
  LOGICAL                                  :: &
    ...
    &  l_scale_unit,                          & !< Is a unit for scaling with post operation given?
...
!  Definitions
  ...
  ! Check if unit for scaling with post operation is specified
  CALL key_value_storage_as_string(meta_storage,'scale_unit',scale_units,ierror)
  l_scale_unit = (ierror == SUCCESS)
...
! Defcase dependent settings
  SELECT CASE(TRIM(defcase))
    CASE('prog')
      ...
      IF (l_scale_unit) THEN
        new_cf = cf
        new_cf%units   = TRIM(scale_units)
      END IF
    ...
    CASE('conv')
      ...
      IF(lconv_tracer) THEN
        ...
        IF (l_scale_unit) THEN
          new_cf = cf
          new_cf%units   = TRIM(scale_units)//' s-1'
        END IF
...
! Tracer type dependent settings and creation of tracer metadata container
      ...
      IF (ierror == SUCCESS) THEN
        SELECT CASE (ADJUSTL(TRIM(c_solve)))
            ...
            CASE('linozV3')
              ...
              ALLOCATE(t_chem_meta_linozv3 :: tracer_meta)
  ...
  ! Set up final post operation argument
  IF (l_scale_unit) THEN
    post_op_arg = post_op(post_op_id, new_cf = new_cf, arg1 = scale_factor)
  ELSE 
    post_op_arg = post_op(post_op_id, arg1 = scale_factor)
  END IF
...
!>
  ...
  IF (TRIM(tolower(transport_template))=='default') THEN
    ...
    itype_vlimit = 2  ! monotonic reconstruction filter
    ihadv_tracer = 52 ! combination of miura and miura with subcycling
