...
! ART aerosol dynamics processes routines
  ...
  USE mo_art_io_constants,              ONLY: STORAGE_ATTR_SEP
  ...
  TYPE t_optical_properties
    CHARACTER(:),ALLOCATABLE :: lut_optics  !< Look-up table entry of prescribed optical properties
    LOGICAL           :: use_var_med_dia
  ...
  TYPE t_diag_optprops
    REAL(wp),POINTER :: ext_aeronet(:)      !< Mass specific extinction coefficient [m2 g-1], dim:nbands
                                            !  nbands=9
    REAL(wp),POINTER :: ext_snglwave(:)     !< Mass specific extinction coefficient [m2 g-1], dim:nbands
                                            !  nbands=1, only for 550nm
    REAL(wp),POINTER :: ext_satellite(:)    !< Mass specific extinction coefficient [m2 g-1], dim:nbands
                                            !  nbands=3
    REAL(wp),POINTER :: bsc_satellite(:)    !< Mass specific backscattering coefficient [m2 g-1], dim:nbands
                                            !  nbands=3
  END TYPE t_diag_optprops
...
!-------------------------------------------------------------------------------
  ...
  TYPE, extends(t_fields_1mom) :: t_fields_pollen
    ...
    !
    INTEGER    :: pollen_prop_idx             !< index of corresponding pollen_properties-entry
    CHARACTER(LEN=IART_VARNAMELEN) :: spp_name         !< name of corresponding SPP-mode
    TYPE(t_fields_2mom),POINTER :: spp_field => NULL() !< field of corresponding SPP-mode
    CONTAINS
  ...
  TYPE, extends(t_mode_fields) :: t_fields_2mom
    ...
    TYPE(t_diag_optprops) :: diag_opt_props          !< Optical properties of mode for diagnostics
...
! Local variables
  ...
  CALL this_fields%t_mode_fields%create(modename,idims)
  this_fields%itr     = UNDEF_INT_ART
  this_fields%pollen_prop_idx = UNDEF_INT_ART
  ...
  this_fields%spp_field => NULL()
  this_fields%spp_name = ""
...
!>
  ...
  CALL meta_key_value_store%get('spp',c_tmp, ierror)
  IF (ierror /= SUCCESS) THEN 
    CALL message(TRIM(routine)//':set_meta_t_fields_pollen', &
      &          'No spp-tag given. No bursting for '//TRIM(this_fields%name)//'.')
  ELSE
    WRITE(this_fields%spp_name,'(A)') TRIM(c_tmp)
  END IF
...
! Geometric quantities
  ...
  ! Quantities for prescriped optical properties
  CALL key_value_storage_as_string(meta_key_value_store, 'lut_optics', this_fields%opt_props%lut_optics, ierror)
  IF (ierror /= SUCCESS) THEN
    CALL message(TRIM(routine)//':set_meta_t_fields_2mom', &
         &          'WARNING: Missing <lut_optics> entry in the modes XML for mode '//TRIM(this_fields%name)// &
         &          '. Set it to the modes name instead.')
    this_fields%opt_props%lut_optics = TRIM(this_fields%name)
  ENDIF

  CALL key_value_storage_as_string(meta_key_value_store, 'cvar_dia'//STORAGE_ATTR_SEP//'lut_optics', &
    &                              c_use_var_dia, ierror)
  IF (ierror == SUCCESS) THEN
    this_fields%opt_props%use_var_med_dia = (c_use_var_dia == 'true')
  ELSE
    this_fields%opt_props%use_var_med_dia = .false.
  ENDIF
...
!Local variables
  ...
  IF(this_fields%linit) THEN
    ...
    WRITE(message_text,'(A16,A)') 'Prescr. optics: ',TRIM(this_fields%opt_props%lut_optics)
    CALL message ('', message_text)
    IF (this_fields%opt_props%use_var_med_dia) THEN
      WRITE(message_text,'(A16,A)') 'Use var. dia: ','true'
    ELSE
      WRITE(message_text,'(A16,A)') 'Use var. dia: ','false'
    ENDIF
    CALL message ('', message_text)
