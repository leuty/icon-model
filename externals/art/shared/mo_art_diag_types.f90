...
! ART
  ...
  PUBLIC :: t_art_aeronet, t_art_ceilo, t_art_max_layer
...
! Tracer emission and deposition diagnostics
    REAL(wp),POINTER             ::    &  !< new diagnostics container for accumulated tracers (nproma, nblks_c, ntracer_dry_depo)
      ...
      &  acc_wetdepo_gas(:,:,:)   => NULL(), &  !< wet deposition of trace gases
   ...
   TYPE(t_ptr_2d3d),ALLOCATABLE ::     &
      ...
      &  acc_wetdepo_gas_ptr(:),       &  !< pointer array (one pointer for each tracer) [kg m-2]
...
!>
  ...
  IF(ALLOCATED(this%acc_wetdepo_gas_ptr))   DEALLOCATE(this%acc_wetdepo_gas_ptr)
