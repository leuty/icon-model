...
void comin_main() {
  ...
  t_comin_var_descriptor foo_exc_descr = {.id = 1};
  strncpy(foo_exc_descr.name, "foo_exc", COMIN_MAX_LEN_VAR_NAME);

  if (strncmp(plugin_name, "test_lmodexclusive_plugin1", ilen) == 0) {
    ...
    if (comin_error_get() != COMIN_ERROR_VAR_REQUEST_EXISTS_IS_LMODEXCLUSIVE) {
      comin_plugin_finish(
          "test_lmodexclusive: comin_main",
          "Request lmodexclusive check (variable exists lmodexclusive) broken");
    } else {
      comin_print_info("Check successful: variables cannot be requested if "
                       "exist with lmodexclusive=true");
    }
    ...
    if (comin_error_get() !=
      ...
      comin_plugin_finish(
          "simple_fortran_plugin: comin_main",
          "Request lmodexclusive check (variable exists) broken");
    } else {
      comin_print_info("Check successful: variables cannot be requested "
                       "lmodexclusive=true if exists");
    }
  }
}
