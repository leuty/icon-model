...
mpi_procs_pernode=1 ./exp.test_nwp_R02B04.run
./exp.test_nwp_R02B04_R02B05_nest.run
...
for exp in *; do
    ...
    HASH=`md5sum ${exp}.tar.gz | awk '{ print $1 }'`
    URL="${CI_API_V4_URL}/projects/${CI_PROJECT_ID}/packages/generic/replay_data/1/${HASH}_${exp}.tar.gz"
    curl --fail \
