...
def save_component(name, component):
    ...
    if component.ndims == 0:
        if component.type == "CHARACTER":
            iprint(
                indent,
                f'CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "{name}", TRIM(descrdata_obj%{name})))',
            )
        elif component.type == "LOGICAL":
            iprint(
                indent,
                f'CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "{name}", MERGE(1,0,descrdata_obj%{name})))',
            )
        else:
            iprint(
                indent,
                f'CALL nf90(nf90_put_att(ncid, NF90_GLOBAL, "{name}", descrdata_obj%{name}))',
            )
    else:
        iprint(
            indent,
            f'var_id = nf90_utils_def_var(ncid, "{name}", {f2nf_type[component.type]}, SHAPE(descrdata_obj%{name}))',
        )
