...
def load_component(name, component):
    ...
    if component.ndims == 0:
        if "ALLOCATABLE" in component.attributes or "POINTER" in component.attributes:
            iprint(
                indent,
                f'status = nf90_inquire_attribute(ncid, NF90_GLOBAL, "{name}", len=shap(1))',
            )
            iprint(indent, "IF(status == NF90_NOERR) THEN")
            indent += 2
            if component.type == "CHARACTER":
                iprint(
                    indent, f"ALLOCATE(CHARACTER(LEN=shap(1)) :: descrdata_obj%{name})"
                )
            else:
        ...
        if component.type == "LOGICAL":
            ...
            iprint(
                indent,
                f'CALL nf90(nf90_get_att(ncid, NF90_GLOBAL, "{name}", descrdata_obj%{name}))',
            )
        if "ALLOCATABLE" in component.attributes or "POINTER" in component.attributes:
            indent -= 2
            iprint(indent, "ENDIF")
        ...
        if "ALLOCATABLE" in component.attributes or "POINTER" in component.attributes:
            iprint(
                indent,
                f'shap(1:{component.ndims}) = nf90_utils_get_shape(ncid, "{name}", {component.ndims})',
            )
            iprint(
                indent,
                f"ALLOCATE(descrdata_obj%{name}({','.join([f'shap({d})' for d in range(1,component.ndims+1)])}))",
            )
        if component.type == "LOGICAL":
            ...
            iprint(
                indent, f"CALL nf90(nf90_get_var(ncid, var_id, descrdata_obj%{name}))"
            )
        indent -= 2
        iprint(indent, "ENDIF")
