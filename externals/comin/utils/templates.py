...
def c_get_array(namespace, name, typ, kind, ndim, has_jg):
    ...
    print(
        f"""  void comin_descrdata_get_{namespace}_{name}({"int jg, " if has_jg else ""}const {c_typ}** {name}, int* arr_size);"""
    )
...
def c_get_scalar(namespace, name, typ, kind, has_jg):
    c_typ = f_to_c_type(typ, kind)
    print(
        f"""  {c_typ} comin_descrdata_get_{namespace}_{name}({"int jg" if has_jg else ""});"""
    )
...
def gen_c_properties(namespace, struct, has_jg):
    ...
    print(f"""
  static const struct comin_descrdata_property_t comin_descrdata_{namespace}_properties[] = {{""")
    for name, substruct in struct.items():
        if type(substruct) is dict:
            print(
                f'    {{"{name}", 0, "void", 0, {"true" if has_jg else "false"}, comin_descrdata_{namespace}_{name}_properties }},'
            )
        else:
            typ, kind, attr, ndims = substruct
            if typ == "CHARACTER" and ndims == 0 and kind is not None:
            ...
            c_typ = f_to_c_type(typ, kind)
            print(
                f'    {{"{name}", (void*)&comin_descrdata_get_{namespace}_{name}, "{c_typ}", {ndims}, {"true" if has_jg else "false"}, 0 }},'
            )
    print("    {0,0,0,0,0}};")
