  ...
  SUBROUTINE var_recorder_secondary_ctor() BIND(C)
    ...
    DO jg=1, global%n_dom
      ...
      DO i=1,SIZE(var_names)
        ...
        CALL comin_var_get([ep], var_descrs(jg, i), COMIN_FLAG_READ, var_ptrs(jg, i))

        var_varids(jg, i) = nf90_utils_def_var(domain_ncids(jg), TRIM(var_names(i)), &
             & NF90_DOUBLE, var_ptrs(jg, i)%array_shape(), [event_dimid])
        CALL nf90(nf90_put_att(domain_ncids(jg), var_varids(jg, i), &
                               "dim_semantics", var_ptrs(jg, i)%dim_semantics()))
        CALL nf90(nf90_put_att(domain_ncids(jg), var_varids(jg, i), &
                               "ncontained", var_ptrs(jg, i)%ncontained()))
        CALL nf90(nf90_put_att(domain_ncids(jg), var_varids(jg, i), &
                               "lcontained", MERGE(1,0,var_ptrs(jg, i)%lcontainer())))
