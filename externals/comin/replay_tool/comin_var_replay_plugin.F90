...
MODULE comin_var_replay_plugin
  ...
  USE comin_plugin_interface, ONLY: t_comin_var_handle, t_comin_plugin_info,                          &
       ...
       &                            comin_metadata_set, comin_metadata_get,                        &
  ...
  TYPE(t_comin_var_handle),        ALLOCATABLE :: var_ptrs(:,:)
...
CONTAINS
  ...
  SUBROUTINE comin_var_replay_secondary_ctor() BIND(C)
    ...
    DO jg=1,global%n_dom
      DO i=1,nvars(jg)
        CALL comin_var_get([ep], var_descrs(jg, i), COMIN_FLAG_WRITE, var_ptrs(jg, i))
        ! Check if the index positions are the same
        status = nf90_get_att(domain_ncids(jg), var_varids(jg, i), "dim_semantics", dim_semantics)
        IF(status == NF90_NOERR) THEN
          IF (ALL(dim_semantics /= var_ptrs(jg, i)%dim_semantics())) &
            CALL comin_plugin_finish("comin_var_replay_plugin", &
                                     "dim_semantics does not match for var " // TRIM(var_descrs(jg, i)%name))
        ENDIF
      END DO
    END DO
  ...
  SUBROUTINE comin_var_replay() BIND(C)
    INTEGER :: i, jg, datatype
    REAL(C_DOUBLE), POINTER :: ptr_dp(:,:,:,:,:)
    REAL(C_FLOAT), POINTER  :: ptr_sp(:,:,:,:,:)
    INTEGER(C_INT), POINTER :: ptr_i(:,:,:,:,:)

    DO jg=1,global%n_dom
      DO i=1,nvars(jg)
        CALL comin_metadata_get(var_ptrs(jg, i)%descriptor(), "datatype", datatype)
        SELECT CASE(datatype)
        CASE (COMIN_VAR_DATATYPE_DOUBLE)
          CALL var_ptrs(jg, i)%get_ptr(ptr_dp)
          CALL nf90(nf90_get_var(domain_ncids(jg), var_varids(jg, i), ptr_dp, &
               &                 [1, 1, 1, 1, 1, event_counter]))
        CASE (COMIN_VAR_DATATYPE_FLOAT)
          CALL var_ptrs(jg, i)%get_ptr(ptr_sp)
          CALL nf90(nf90_get_var(domain_ncids(jg), var_varids(jg, i), ptr_sp, &
               &                 [1, 1, 1, 1, 1, event_counter]))
        CASE (COMIN_VAR_DATATYPE_INT)
          CALL var_ptrs(jg, i)%get_ptr(ptr_i)
          CALL nf90(nf90_get_var(domain_ncids(jg), var_varids(jg, i), ptr_i, &
               &                 [1, 1, 1, 1, 1, event_counter]))
        END SELECT
      END DO
    END DO
