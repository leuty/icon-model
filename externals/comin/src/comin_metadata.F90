...
MODULE comin_metadata
  ...
  USE iso_c_binding,           ONLY: c_int, c_ptr, c_bool, c_double, c_f_pointer
  ...
  USE comin_ftnlist_ifc,       ONLY: comin_ftnlist_iterator_begin, &
    &                                comin_ftnlist_iterator_next, comin_ftnlist_iterator_value,                &
    &                                comin_ftnlist_iterator_delete, comin_ftnlist_is_end
  USE comin_errhandler,        ONLY: comin_error_set, comin_plugin_finish
  ...
  USE comin_variable_types,    ONLY: t_comin_var_descriptor, t_comin_var_item,            &
    &                                t_comin_var_descriptor_c, comin_var_descr_match,     &
...
CONTAINS
  ...
  SUBROUTINE comin_metadata_host_set_integer(descriptor, key, val)
    ...
    ELSE
      IF ( ALL(var_item%metadata%query(TRIM(key)) /= &
               (/COMIN_METADATA_TYPEID_UNDEFINED, COMIN_METADATA_TYPEID_INTEGER/) ) ) THEN
  ...
  SUBROUTINE comin_metadata_host_set_logical(descriptor, key, val)
    ...
    ELSE
      IF ( ALL(var_item%metadata%query(TRIM(key)) /= &
               (/COMIN_METADATA_TYPEID_UNDEFINED, COMIN_METADATA_TYPEID_LOGICAL/) ) ) THEN
        CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE); RETURN
      ENDIF
      CALL var_item%metadata%set(key, val)
    END IF
  ...
  SUBROUTINE comin_metadata_host_set_real(descriptor, key, val)
    ...
    ELSE
      IF ( ALL(var_item%metadata%query(TRIM(key)) /= &
           & (/COMIN_METADATA_TYPEID_UNDEFINED, COMIN_METADATA_TYPEID_REAL/) ) ) THEN
        CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE); RETURN
      ENDIF
      CALL var_item%metadata%set(key, val)
    END IF
  ...
  SUBROUTINE comin_metadata_host_set_character(descriptor, key, val)
    ...
    var_item => comin_var_get_from_exposed(descriptor)
    IF (.NOT. ASSOCIATED(var_item)) THEN
      CALL comin_error_set(COMIN_ERROR_VAR_ITEM_NOT_ASSOCIATED)
      RETURN
    ELSE
      IF ( ALL(var_item%metadata%query(TRIM(key)) /= &
               (/COMIN_METADATA_TYPEID_UNDEFINED, COMIN_METADATA_TYPEID_CHARACTER/) ) ) THEN
        CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE); RETURN
      ENDIF
      CALL var_item%metadata%set(key, val)
    END IF
  ...
  SUBROUTINE comin_metadata_get_integer(var_descriptor, key, val)
    ...
    IF ( var_item%metadata%query(TRIM(key)) /= COMIN_METADATA_TYPEID_INTEGER)  THEN
  ...
  SUBROUTINE comin_metadata_get_logical(var_descriptor, key, val)
    ...
    IF ( var_item%metadata%query(TRIM(key)) /= COMIN_METADATA_TYPEID_LOGICAL)  THEN
      CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE)
      RETURN
    ENDIF
  ...
  SUBROUTINE comin_metadata_get_real(var_descriptor, key, val)
    ...
    IF ( var_item%metadata%query(TRIM(key)) /= COMIN_METADATA_TYPEID_REAL)  THEN
      CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE)
      RETURN
    ENDIF
  ...
  SUBROUTINE comin_metadata_get_character(var_descriptor, key, val)
    ...
    IF ( var_item%metadata%query(TRIM(key)) /= COMIN_METADATA_TYPEID_CHARACTER)  THEN
      CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE)
      RETURN
    ENDIF
  ...
  SUBROUTINE comin_metadata_get_character_c(var_descriptor, key, val, len) &
    ...
    IF ( var_item%metadata%query(convert_c_string(key)) /= COMIN_METADATA_TYPEID_CHARACTER)  THEN
      CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE)
      RETURN
    ENDIF
  ...
  SUBROUTINE comin_request_set_var_metadata_integer(var_descriptor, key, val)
    ...
    DO domain_id = domain_id_start, domain_id_end
      ...
      DO WHILE (.NOT. comin_ftnlist_is_end(state%comin_var_request_list,it))
        ...
        IF (comin_var_descr_match(item%descriptor, var_descriptor_domain)) THEN
          lfound = .TRUE.
          IF ( ALL(item%metadata%query(TRIM(key)) /= &
            &      (/COMIN_METADATA_TYPEID_UNDEFINED, COMIN_METADATA_TYPEID_INTEGER/) ) ) THEN
            CALL comin_error_set(COMIN_ERROR_VAR_METADATA_INCONSISTENT_TYPE); RETURN
          ENDIF
          CALL item%metadata%set(key, val)
        END IF
