...
MODULE comin_callback
  ...
  USE comin_variable_types,       ONLY: t_comin_var_context_item, comin_var_ptr_init
  USE comin_setup_constants,      ONLY: EP_NAME
  USE comin_c_utils,              ONLY: convert_f_string
  USE comin_ftnlist_ifc,          ONLY: comin_ftnlist_push_back, comin_ftnlist_is_end, comin_ftnlist_iterator_begin, &
    &                                   comin_ftnlist_iterator_delete, comin_ftnlist_iterator_next, &
    &                                   comin_ftnlist_iterator_value, comin_ftnlist_delete
...
CONTAINS
  ...
  SUBROUTINE comin_callback_register(entry_point_id, fct_ptr) &
    ...
    TYPE(t_comin_callback_element), POINTER :: item
    ...
    ALLOCATE(item)
    item = t_comin_callback_element(entry_point_id = entry_point_id, &
      &                             plugin_info    = state%current_plugin, &
      &                             comin_callback = fct_ptr)
    CALL comin_ftnlist_push_back(state%comin_callback_list, c_loc(item))
  ...
  SUBROUTINE comin_callback_complete()
    ...
    INTEGER                       :: status
    TYPE(c_ptr)                   :: it, cptr
    TYPE(t_comin_callback_element), POINTER :: var_list_element
    ...
    CALL comin_ftnlist_iterator_begin(state%comin_callback_list, it)
    DO WHILE (.not. comin_ftnlist_is_end(state%comin_callback_list,it))
      CALL comin_ftnlist_iterator_value(it, cptr)
      CALL c_f_pointer(cptr, var_list_element)
    ...
    do while (.not. comin_ftnlist_is_end(state%comin_callback_list,it))
      call comin_ftnlist_iterator_value(it, cptr)
      call c_f_pointer(cptr, var_list_element)
      deallocate(var_list_element)
      call comin_ftnlist_iterator_next(it)
    end do
    call comin_ftnlist_iterator_delete(it)
    call comin_ftnlist_delete(state%comin_callback_list)
    ...
    IF (lcallbacks_exist) THEN
      DO thirdpi=1,state%num_plugins
        ...
        IF (ASSOCIATED(cl)) THEN
          ...
          CALL sync_vars_for_halo_region(entry_point_id, thirdpi, COMIN_FLAG_READ)
          ! set current plugin
          ...
          CALL sync_vars_for_halo_region(entry_point_id, thirdpi, COMIN_FLAG_WRITE)
  ...
  SUBROUTINE sync_vars_for_device(ep, plugin_id, rw_flag)
    ...
    CALL comin_ftnlist_iterator_begin(state%comin_var_list_context(ep , plugin_id)%var_list, it)
    DO WHILE (.not. comin_ftnlist_is_end(state%comin_var_list_context(ep , plugin_id)%var_list,it))

      CALL comin_ftnlist_iterator_value(it, cptr)
      CALL c_f_pointer(cptr, item)

      IF (IAND(item%access_flag, COMIN_FLAG_DEVICE) == 0 .AND. &
 &        IAND(item%access_flag, rw_flag) /= 0) THEN
        CALL state%sync_device_mem(comin_var_ptr_init(item%var_item), rw_flag == COMIN_FLAG_WRITE)
      END IF
      CALL comin_ftnlist_iterator_next(it)
    END DO
    CALL comin_ftnlist_iterator_delete(it)
  ...
  SUBROUTINE check_var_no_device(ep, plugin_id)
    ...
    DO WHILE (.not. comin_ftnlist_is_end(state%comin_var_list_context(ep , plugin_id)%var_list,it))
      ...
      IF (IAND(item%access_flag, COMIN_FLAG_DEVICE) /= 0) THEN
  ...
  SUBROUTINE sync_vars_for_halo_region(ep, plugin_id, rw_flag)
    INTEGER, INTENT(IN) :: ep
    INTEGER, INTENT(IN) :: plugin_id
    INTEGER, INTENT(IN) :: rw_flag
    !locals
    INTEGER             :: halo_sync_mode
    TYPE(c_ptr) :: it, cptr
    TYPE(t_comin_var_context_item), POINTER :: item
    CALL comin_ftnlist_iterator_begin(state%comin_var_list_context(ep , plugin_id)%var_list, it)
    DO WHILE (.not. comin_ftnlist_is_end(state%comin_var_list_context(ep , plugin_id)%var_list,it))
      CALL comin_ftnlist_iterator_value(it, cptr)
      CALL c_f_pointer(cptr, item)
      IF (IAND(item%access_flag, COMIN_FLAG_SYNC_HALO) /= 0 .AND. &
           &        IAND(item%access_flag, rw_flag) /= 0) THEN
        CALL determine_halo_sync_mode(item, halo_sync_mode)
        CALL state%sync_halo(comin_var_ptr_init(item%var_item), halo_sync_mode)
      END IF
      CALL comin_ftnlist_iterator_next(it)
    END DO
    CALL comin_ftnlist_iterator_delete(it)
  END SUBROUTINE sync_vars_for_halo_region

  SUBROUTINE determine_halo_sync_mode(item, halo_sync_mode)
    TYPE(t_comin_var_context_item), POINTER, INTENT(IN) :: item
    INTEGER, INTENT(OUT)                        :: halo_sync_mode
    !locals
    CHARACTER(LEN=20)                           :: metadata_key
    INTEGER                                     :: val
    metadata_key = 'zaxis_id'
    CALL item%var_item%metadata%get(metadata_key, val)
    IF (val == COMIN_ZAXIS_2D) THEN
      halo_sync_mode = COMIN_ZAXIS_2D
    ELSE IF (val == COMIN_ZAXIS_3D) THEN
      halo_sync_mode = COMIN_ZAXIS_3D
    ELSE
      CALL comin_error_set(COMIN_ERROR_VAR_SYNC_HALO_NOT_SUPPORTED_ZAXIS); RETURN
    END IF
  END SUBROUTINE determine_halo_sync_mode
  ...
  SUBROUTINE comin_callback_get_ep_name_c( iep, out_ep_name) &
    ...
    CHARACTER(len=1, kind=c_char),DIMENSION(COMIN_MAX_LEN_EP_NAME+1)  :: out_ep_name
    !
    CHARACTER(LEN=:), ALLOCATABLE :: ep_name
