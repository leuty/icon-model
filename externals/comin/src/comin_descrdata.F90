...
MODULE comin_descrdata
  ...
  USE, INTRINSIC :: iso_c_binding, ONLY: c_int, c_ptr, c_loc, c_bool
...
CONTAINS
  ...
  SUBROUTINE comin_descrdata_get_edge_indices(jg, i_blk, i_startblk, i_endblk, i_startidx, &
    ...
    i_startidx = MERGE(1, &
                       MAX(1, state%comin_descrdata_domain(jg)%edges%start_index(irl_start)), &
                       i_blk /= i_startblk)
    i_endidx   = MERGE(state%comin_descrdata_global%nproma, &
                       state%comin_descrdata_domain(jg)%edges%end_index(irl_end), &
                       i_blk /= i_endblk)
  ...
  SUBROUTINE comin_descrdata_get_vert_indices(jg, i_blk, i_startblk, i_endblk, i_startidx, &
    ...
    IF (i_blk == i_startblk) THEN
      i_startidx = state%comin_descrdata_domain(jg)%verts%start_index(irl_start)
      i_endidx   = state%comin_descrdata_global%nproma
      IF (i_blk == i_endblk) i_endidx = state%comin_descrdata_domain(jg)%verts%end_index(irl_end)
    ELSE IF (i_blk == i_endblk) THEN
      i_startidx = 1
      i_endidx   = state%comin_descrdata_domain(jg)%verts%end_index(irl_end)
    ELSE
      i_startidx = 1
      i_endidx = state%comin_descrdata_global%nproma
    ENDIF
