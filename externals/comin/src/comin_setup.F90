...
MODULE comin_setup
  ...
  USE comin_ftnlist_ifc,     ONLY: comin_ftnlist_new
...
CONTAINS
 ...
 !! @ingroup host_interface
  SUBROUTINE comin_plugin_primaryconstructor(plugin_list)
    ...
    CALL comin_ftnlist_new(state%comin_callback_list)
    CALL comin_ftnlist_new(state%comin_var_list)
    CALL comin_ftnlist_new(state%comin_var_descr_list)
    CALL comin_ftnlist_new(state%comin_var_request_list)
    ...
    DO i=1,state%num_plugins
      ...
      state%current_plugin%log_debug = plugin_list(i)%log_debug
      state%current_plugin%log_info = plugin_list(i)%log_info
      state%current_plugin%log_warning = plugin_list(i)%log_warning
      CALL setup_fct()
  ...
  SUBROUTINE comin_setup_finalize()
    ...
    ! we don't call dlclose in the finalization because it has no
    ! advantages, while it would triggers some problems:
    !
    ! - NVHPC compilers for GPU try to unload the library again in at
    ! the end of the program resulting in a segfault. See
    ! https://gitlab.dkrz.de/icon-comin/comin/-/issues/210
    !
    ! - parallel HDF5 registers calls backs at `MPI_COMM_SELF` atexit
    ! handler, which also results in a segfault as the call gets
    ! unloaded in dlclose before the handler is called.
    ! See https://gitlab.dkrz.de/icon-comin/comin/-/issues/214
    !
    ...
    !   END IF
    ! END DO
    DEALLOCATE(dl_handles, state%plugin_info)
