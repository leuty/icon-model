...
CONTAINS
  ...
  FUNCTION comin_errhandler_get_string(err_code) RESULT(string)
    CHARACTER(LEN=COMIN_MAX_LEN_ERR_MESSAGE) :: string
    ...
    CASE(COMIN_ERROR_VAR_SYNC_HALO_NOT_ASSOCIATED)
      string = "Sync halo callback not set (setup check)."
    ...
    CASE(COMIN_ERROR_VAR_GET_CONTAINER_CAN_NOT_HALO_SYNCHRONIZED)
      string = "Cannot halo sync a container"
    CASE(COMIN_ERROR_VAR_GET_IRREGULAR_VAR_CAN_NOT_HALO_SYNCHRONIZED)
      string = "Cannot halo sync an irregular variable"
    CASE(COMIN_ERROR_VAR_SYNC_HALO_NOT_SUPPORTED_ZAXIS)
      string = "Can only halo sync vars on full levels"
    CASE(COMIN_ERROR_VAR_GET_VARIABLE_WRONG_TYPE)
      string = "Wrong type used requesting the pointer of a variable"
