...
SUBROUTINE comin_plugin_finish_external(routine, text)
  USE mpi
  USE comin_state,      ONLY: state, comin_current_get_ep
  ...
  ELSE
    ...
    STOP exit_no
  END IF
END SUBROUTINE comin_plugin_finish_external
...
MODULE comin_errhandler
  ...
  USE comin_errhandler_constants, ONLY: COMIN_SUCCESS, COMIN_WARNING, &
...
CONTAINS
  ...
  SUBROUTINE comin_plugin_finish(routine, text)
    CHARACTER(LEN=*), INTENT(IN) :: routine
    CHARACTER(LEN=*), INTENT(IN) :: text
    CALL comin_plugin_finish_external(routine,text)
  ...
  SUBROUTINE comin_plugin_finish_c(routine, text) &
    ...
    CALL comin_plugin_finish_external(convert_c_string(routine), convert_c_string(text))
  ...
  SUBROUTINE comin_error_get_message_c(error_code, category, message) &
    ...
    CHARACTER(KIND=C_CHAR), INTENT(OUT) :: message(COMIN_MAX_LEN_ERR_MESSAGE)

    CHARACTER(LEN=11)                  :: category_f
    CHARACTER(LEN=COMIN_MAX_LEN_ERR_MESSAGE) :: message_f
  ...
  SUBROUTINE comin_error_check() BIND(C)
    ...
    CHARACTER(LEN=COMIN_MAX_LEN_ERR_MESSAGE) :: message
  ...
  !! @ingroup plugin_interface
  SUBROUTINE comin_error_reset() &
       & BIND(C)
    state%errcode = COMIN_SUCCESS
  END SUBROUTINE comin_error_reset

  !> Change the error handling mode.
  !> Set it to .TRUE. to handle errors manually.
  !> If it set to .FALSE. (default) comin calls finish if an error occurs.
  !! @ingroup plugin_interface
  SUBROUTINE comin_error_set_errors_return(errors_return)
    LOGICAL, INTENT(IN) :: errors_return
    state%current_plugin%errors_return = errors_return
  END SUBROUTINE comin_error_set_errors_return

  SUBROUTINE comin_error_set_errors_return_c(errors_return) &
    BIND(C, NAME="comin_error_set_errors_return")
    LOGICAL(C_BOOL), VALUE, INTENT(IN) :: errors_return
    state%current_plugin%errors_return = errors_return
  END SUBROUTINE comin_error_set_errors_return_c
