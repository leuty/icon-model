...
MODULE calc_water_column_plugin
  ...
  USE comin_plugin_interface,  ONLY : comin_callback_register,                                          &
    ...
    &                                 comin_metadata_get,                                               &
    &                                 comin_error_set_errors_return, COMIN_SUCCESS, comin_error_get,    &
    &                                 comin_print_info,                                                 &
  ...
  TYPE :: t_plugin_vars
    ! ICON data
    TYPE(t_comin_var_handle) :: rho
    TYPE(t_comin_var_handle) :: qv
    TYPE(t_comin_var_handle) :: qc
    TYPE(t_comin_var_handle) :: qi
    TYPE(t_comin_var_handle) :: qr
    TYPE(t_comin_var_handle) :: qs
    TYPE(t_comin_var_handle) :: qg
    ! variables added to ICON by this plugin
    TYPE(t_comin_var_handle) :: lwp ! liquid water path (qc+qr)
    TYPE(t_comin_var_handle) :: iwp ! ice water path    (qi+qs+qg)
    TYPE(t_comin_var_handle) :: twc ! (full) water path (qv+qc+qr+qi+qs+qg)
...
CONTAINS
  ...
  SUBROUTINE calc_water_column_constructor()  BIND(C)
    ...
    domain_loop: DO jg = 1, p_global%n_dom
      WRITE (text,'(a,i4)') ' - get required meteorological data. dom: ', jg
      CALL comin_print_info(text)
      ...
      WRITE (text,'(a,i4)') ' - get humidity tracer. dom: ',jg
      CALL comin_print_info(text)
      ! 1.2  qv
      var_desc%name = 'qv'
      var_desc%id = jg
      ...
      var_desc%name = 'qg'
      var_desc%id = jg
      CALL comin_error_set_errors_return(.TRUE.)
      CALL comin_var_get([EP_ATM_PHYSICS_BEFORE], &
           &             var_desc, COMIN_FLAG_READ, pvpp(jg)%qg)
      lqg = (comin_error_get() == COMIN_SUCCESS)
      CALL comin_error_set_errors_return(.FALSE.)
      IF (lqg) CALL check_variable(pvpp(jg)%qg, 'qg')

      WRITE (text,'(a,i4)') ' - get plugin variables - requested to be created by ICON. dom: ', jg
      CALL comin_print_info(text)
      ...
      CALL check_variable(pvpp(jg)%iwp, 'iwp',dim=2)

      CALL comin_var_get([EP_ATM_PHYSICS_BEFORE], &
           &             t_comin_var_descriptor(name='twc', id=jg), &
           &             COMIN_FLAG_WRITE, pvpp(jg)%twc)
      CALL check_variable(pvpp(jg)%twc, 'twc',dim=2)
  ...
  SUBROUTINE calc_water_column_diagfct()  BIND(C)
    ...
    WRITE (text,'(a,i4)') '- calculate water columns before physics. dom: ', jg
    CALL comin_print_info(text)

    CALL pvpp(jg)%qv%to_3d(qv_3d)
    CALL pvpp(jg)%qi%to_3d(qi_3d)
    CALL pvpp(jg)%qr%to_3d(qr_3d)
    CALL pvpp(jg)%qs%to_3d(qs_3d)
    CALL pvpp(jg)%qc%to_3d(qc_3d)
    IF (lqg) CALL pvpp(jg)%qg%to_3d(qg_3d)

    CALL pvpp(jg)%rho%to_3d(rho_3d)

    CALL pvpp(jg)%lwp%to_3d(lwp_3d)
    CALL pvpp(jg)%iwp%to_3d(iwp_3d)
    CALL pvpp(jg)%twc%to_3d(twc_3d)
    ...
    WRITE (text,'(a,i4)') '- results of plugin calc_water_column. dom: ', jg
  ...
  SUBROUTINE check_variable(var, name, dim)
  ...
  SUBROUTINE check_dimensions_3d(var, name)
    ...
    IF (ANY(var%dim_semantics() /= [COMIN_DIM_SEMANTICS_NPROMA,    &
