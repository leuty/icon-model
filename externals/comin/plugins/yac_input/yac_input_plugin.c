...
void secondary_constructor() {
  t_comin_entry_point ep = EP_ATM_TIMELOOP_START;
...
void create_grid_and_field() {
  ...
  int* num_vertices_per_cell =
      malloc(n_patch_cells * sizeof(*num_vertices_per_cell));
  ...
  const int *vertex_idx, *vertex_blk;
  ...
  int* vertex_idx_transposed =
      malloc(3 * n_patch_cells * sizeof(*vertex_idx_transposed));
  for (int i = 0; i < n_patch_cells; ++i)
    for (int k = 0; k < 3; ++k) {
      vertex_idx_transposed[k + 3 * i] =
          vertex_idx[i + k * vertex_idx_size[0] * vertex_idx_size[1]] - 1 +
          nproma *
              (vertex_blk[i + k * vertex_idx_size[0] * vertex_idx_size[1]] - 1);
    }

  int grid_id;
  yac_cdef_grid_unstruct("comin_yac_plugin_grid", n_patch_verts, n_patch_cells,
                         num_vertices_per_cell, vlon_rad, vlat_rad,
                         vertex_idx_transposed, &grid_id);
  free(vlon_rad);
  free(vlat_rad);
  ...
  int point_id;
  yac_cdef_points_unstruct(grid_id, n_patch_cells, YAC_LOCATION_CELL, clon_rad,
                           clat_rad, &point_id);
  ...
  yac_cdef_field(descr.name, comp_id, &point_id, 1, 1, dt_str,
                 YAC_TIME_UNIT_MILLISECOND, &field_id);
...
void recv_field() {
  int info, ierror;
  double* ptr = comin_var_get_ptr_double(comin_var);
