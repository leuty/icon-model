...
int main(int argc, char** argv) {
  if (argc != 2) {
    fprintf(stderr,
            "Please provide the variable name as command-line argument!\n");
    return 1;
  }
  ...
  yac_cdef_grid_reg2d(grid_name, dims, cyclic, x_vertices, y_vertices,
                      &grid_id);
  ...
  int field_id;
  yac_cdef_field(field_name, comp_id, &point_id, 1, 1, dt,
  ...
  yac_cdef_couple("yac_input_external", grid_name, field_name,
                  "yac_input_plugin", "comin_yac_plugin_grid", field_name, dt,
                  YAC_TIME_UNIT_ISO_FORMAT, YAC_REDUCTION_TIME_NONE, interp_id,
                  1, 0);
  ...
  int ierror   = 0;
  int t        = 0;
  while (info != YAC_ACTION_OUT_OF_BOUND &&
