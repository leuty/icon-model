...
def describe(obj, f, prefix=""):
    for element_name in dir(obj):
        if element_name not in ignore:
            element = getattr(obj, element_name)
            if element_name.startswith("_"):
                pass
            elif element.__class__ is comin._EntryPoint:
                print(f"- EntryPoint `{prefix}{element_name}`", file=f)
            elif inspect.isclass(element):
                print(f"- class {prefix}{element_name}", file=f)
            elif inspect.ismodule(element):
                print(f"- module {prefix}{element_name}", file=f)
                pass
                ...
                try:
                    dims = [":"] * element.ndim
                    print(
                        f"- `{prefix}{element_name}({','.join(dims)})` ({np.asarray(element).dtype})",
                        file=f,
                    )
                except Exception:
                    print(
                        f"- `{prefix}{element_name}` ({type(element).__name__})", file=f
                    )
