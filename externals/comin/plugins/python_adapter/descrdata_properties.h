...
static PyObject* BuildValue_FromPtr(std::string ctype, void* data) {
  if (ctype == "double")
    return Py_BuildValue("f", *(double*)data);
  if (ctype == "int")
    return Py_BuildValue("i", *(int*)data);
  return PyErr_Format(PyExc_RuntimeError, "Unknown data type: %s",
                      ctype.data());
}

// helper function to generate dict from properties array
static PyObject* py_comin_descrdata_properties_to_dict(
    const comin_descrdata_property_t* properties) {
  PyObject* dict                         = PyDict_New();
  const comin_descrdata_property_t* prop = properties;
  while (prop->name != NULL) {
    PyDict_SetItemString(
        dict, prop->name,
        PyCapsule_New((void*)prop, "descrdata_property", NULL));
    prop++;
  }
  return dict;
}
...
static PyObject* py_comin_descrdata_eval_property(PyObject* self,
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|i", (char**)&kwlist,
                                   &prop_cap, &jg))
    return NULL;
  comin_descrdata_property_t* prop =
      (comin_descrdata_property_t*)PyCapsule_GetPointer(prop_cap,
                                                        "descrdata_property");
  if (prop == NULL)
    return NULL;
  void* dataptr;
  if (std::string(prop->datatype) ==
      "void") { // return the dict of subproperties
    ...
    return PyErr_Format(PyExc_RuntimeError,
                        "datatype %s for scalar value %s not implemented",
                        prop->datatype, prop->name);
  } else {
    std::vector<int> arr_size(prop->ndims);
    if (prop->has_jg)
      ((void (*)(int, void**, int*))prop->get_function)(jg, &dataptr,
                                                        arr_size.data());
    else
      ((void (*)(void**, int*))prop->get_function)(&dataptr, arr_size.data());
    if (dataptr == NULL)
      return PyErr_Format(PyExc_RuntimeError, "%s get function returned NULL",
                          prop->name);
    ...
    if (std::string(prop->datatype) == "char" && prop->ndims == 1)
      return Py_BuildValue("s#", dataptr, (Py_ssize_t)arr_size[0]);
    // return MemoryView
    Py_buffer buffer;
    if (std::string(prop->datatype) == "double") {
      ...
      return PyErr_Format(PyExc_RuntimeError,
                          "datatype %s for property %s not implemented",
                          prop->datatype, prop->name);
    buffer.obj = self;
