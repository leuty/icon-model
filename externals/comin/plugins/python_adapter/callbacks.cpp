...
// stores a vector of callable python objects for each plugin id and entry
// point. In contrast to the fortran and c interface we explicitly allow to
...
void py_comin_generic_callback() {
  ...
  for (PyObject* fun : callbacks[plugin_id][ep]) {
    ...
    if (PyErr_Occurred()) {
      PyErr_Print();
      comin_plugin_finish("python_adapter", "Uncaught exception in callback");
    }
  }
}

static PyObject* py_comin_callback_register(PyObject* /*self*/,
                                            PyObject* args) {
  int ientry_point;
  PyObject* callback;
  ...
  if (callbacks[plugin_id].find(entry_point) == callbacks[plugin_id].end() &&
    ...
    comin_callback_register((t_comin_entry_point)ientry_point,
...
static PyObject* py_comin_EP_DESCTRUCTOR(PyObject* /*self*/,
                                         PyObject* /*args*/) {
...
static PyObject* py_comin_callback_get_ep_name(PyObject* /*self*/,
  ...
  comin_callback_get_ep_name((t_comin_entry_point)iep, out_ep_name);
  using namespace std::string_literals;
  py_comin_check_error("ep = "s + std::to_string(iep));
  return Py_BuildValue("s", out_ep_name);
}

std::vector<PyMethodDef> py_comin_callbacks_methods() {
  return {
      {"_callback_register", py_comin_func_wrapper<py_comin_callback_register>,
       METH_VARARGS, "Registers callback to ICON"},
      {"_EP_DESTRUCTOR", py_comin_func_wrapper<py_comin_EP_DESCTRUCTOR>,
       METH_NOARGS, ""},
      {"callback_get_ep_name",
       (PyCFunction)py_comin_func_wrapper<py_comin_callback_get_ep_name>,
       METH_VARARGS | METH_KEYWORDS,
       "C function signature: void comin_callback_get_ep_name(int iep, char "
       "out_ep_name[MAX_LEN_EP_NAME+1])"},
  };
}
