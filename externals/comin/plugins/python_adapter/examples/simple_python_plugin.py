...
def simple_python_constructor():
    ...
    pres = comin.var_get(
        [comin.EP_ATM_WRITE_OUTPUT_BEFORE], ("pres", 1), comin.COMIN_FLAG_READ
    )
    simple_python_var = comin.var_get(
        [comin.EP_ATM_WRITE_OUTPUT_BEFORE],
        ("simple_python_var", 1),
        comin.COMIN_FLAG_READ | comin.COMIN_FLAG_WRITE,
    )
    simple_python_tracer = comin.var_get(
        [comin.EP_ATM_WRITE_OUTPUT_BEFORE],
        ("simple_python_tracer", 1),
        comin.COMIN_FLAG_READ | comin.COMIN_FLAG_WRITE,
    )
    print("tracer dim_semantics:", simple_python_tracer.dim_semantics)
...
def simple_python_diagfct():
    ...
    np.asarray(simple_python_var)[:] = np.asarray(pres) + 42.0
    np.asarray(simple_python_tracer)[:] = np.asarray(simple_python_var) / 1337.0
