...
def pntsrc_constructor():
    ...
    pntsrc = comin.var_get(
        entry_points, ("pntsrc", jg), comin.COMIN_FLAG_READ | comin.COMIN_FLAG_WRITE
    )
    # ICON prepends 'ddt_' and appends '_turb'/'_conv' for the respective tendencies
    entry_points = [comin.EP_ATM_PHYSICS_BEFORE]
    ddt_pntsrc_turb = comin.var_get(
        entry_points, ("ddt_pntsrc_turb", jg), comin.COMIN_FLAG_READ
    )
    ddt_pntsrc_conv = comin.var_get(
        entry_points, ("ddt_pntsrc_conv", jg), comin.COMIN_FLAG_READ
    )
...
def pntsrc_init():
    ...
    if decomp_domain.ravel()[ii] == 0:
        ...
        message(
            f"Min at PE {comin.parallel_get_host_mpi_rank()}, clon={np.rad2deg(clon[jc_loc,jb_loc])}, clat={np.rad2deg(clat[jc_loc,jb_loc])}",
            comin.parallel_get_host_mpi_rank(),
        )
...
def pntsrc_emission():
    ...
    if comin.current_get_domain_id() == jg and lcontain_pntsrc:
        ...
        pntsrc_np[jc_loc, :, jb_loc, 0, 0] = pntsrc_np[jc_loc, :, jb_loc, 0, 0] + 1.0
...
def pntsrc_updatePhysTends():
    ...
    if comin.current_get_domain_id() == jg:
        ...
        pntsrc_np[:, :, :, 0, 0] = np.maximum(
            (
                pntsrc_np[:, :, :, 0, 0]
                + dtime
                * (
                    ddt_pntsrc_turb_np[:, :, :, 0, 0]
                    + ddt_pntsrc_conv_np[:, :, :, 0, 0]
                )
            ),
            0.0,
        )
