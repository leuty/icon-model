...
def sec_ctr():
    ...
    comin_pres_sfc = comin.var_get(
        [comin.EP_ATM_WRITE_OUTPUT_BEFORE], ("pres_sfc", 1), comin.COMIN_FLAG_READ
    )
...
def define_fields():
    ...
    yac_pres_sfc_source = Field.create(
        "pres_sfc",
        source_comp,
        icon_cell_centers,
        1,
    ...
    if rank == 0:
        yac_pres_sfc_target = Field.create(
            "pres_sfc",
            target_comp,
            cell_centers,
            1,
        ...
        kwargs = {}
        if Version(yac_version()) > Version("3.3.0"):
            kwargs["max_search_distance"] = 0.0
        nnn.add_nnn(NNNReductionType.AVG, n=1, scale=1.0, **kwargs)
        yac.def_couple(
