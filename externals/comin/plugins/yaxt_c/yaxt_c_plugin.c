...
void comin_main() {
  ...
  ncells = comin_descrdata_get_domain_cells_ncells(jg);

  int decomp_domain_size[2] = {0, 0};
  comin_descrdata_get_domain_cells_decomp_domain(jg, &decomp_domain,
                                                 decomp_domain_size);

  const int* glb_index = NULL;
  ...
  idxlist          = malloc(ncells * sizeof(*idxlist));
  prognostic_cells = 0;
  ...
  Xt_xmap xmap = xt_xmap_all2all_new(idxvec, empty_idxlist, plugin_comm);
...
void secondary_constructor() {
  t_comin_entry_point ep = EP_ATM_WRITE_OUTPUT_BEFORE;
...
void offload() {
  ...
  double* temp_ptr = comin_var_get_ptr_double(temp_var); // ensures [jc, jb, jk]
  int shape[5];
  ...
  for (int i = 0; i < ncells; ++i) {
    if (decomp_domain[i] == 0) {
      int idx0 = i % shape[0];
      int idx2 = i / shape[0];
      buffer[prognostic_cells] =
          temp_ptr[idx0 + nlev * shape[0] + idx2 * shape[0] * shape[1]];
      prognostic_cells++;
    }
  }
