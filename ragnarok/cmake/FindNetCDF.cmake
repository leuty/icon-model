...
#
# Caveat: The targets might not link properly with static libraries, setting
# NetCDF_<comp>_EXTRA_LIBRARIES may be required.
#
...
# * NetCDF_<comp>_DIR
# * NetCDF_<comp>_PATH
# * The same variables with a NETCDF, NetCDF4, or NETCDF4 prefix instead of
#   NetCDF
# * NetCDF_ROOT
# * NetCDF_DIR
# * NetCDF_PATH
...
#
# * NetCDF_<comp>_EXTRA_LIBRARIES    - added to NetCDF::NetCDF_<comp>
#   INTERFACE_LINK_LIBRARIES and NetCDF_<comp>_LIBRARIES
#
# Notes:
#
...
foreach(_comp IN ITEMS "" "C" "CXX" "Fortran" "CXX_LEGACY")
  ...
  foreach(_name IN ITEMS NetCDF NETCDF NetCDF4 NETCDF4)
    list(
      APPEND _search_hints${__comp} ${${_name}${__comp}} ENV ${_name}${__comp}
    )
  endforeach()
endforeach()
  ...
  if(NetCDF_${_comp}_LIBRARY AND NetCDF_${_comp}_INCLUDE_DIR)
    ...
    if(NOT TARGET NetCDF::NetCDF_${_comp})
      ...
      if(DEFINED NetCDF_${_comp}_EXTRA_LIBRARIES)
        target_link_libraries(
          NetCDF::NetCDF_${_comp} INTERFACE ${NetCDF_${_comp}_EXTRA_LIBRARIES}
        )
      endif()
    endif()
  endif()
...
if(_found)
  ...
  if(NETCDF_CONFIG_EXECUTABLE)
    execute_process(
      COMMAND ${NETCDF_CONFIG_EXECUTABLE} --version
  ...
  elseif(NETCDF_META_H)
    ...
    file(
      STRINGS ${NETCDF_META_H} _netcdf_version_lines
      REGEX "#define[ \t]+NC_VERSION_(MAJOR|MINOR|PATCH|NOTE)"
    )
    string(
      REGEX
      REPLACE ".*NC_VERSION_MAJOR *\([0-9]*\).*"
              "\\1"
...
# Backwards compatibility, only reachable if ECBUILD_2_COMPAT is ON Assumes the
# following internal variables are defined: - _search_components - _arg_<COMP> -
# NetCDF_<comp>_INCLUDE_DIR - NetCDF_<comp>_LIBRARY
