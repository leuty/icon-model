...
DIR* openDirectory(const char* path) {
  ...
  switch (errno) {
    ...
    case ENOENT:
      ...
      fprintf(stderr,
              "fatal error: directory disappeared after creation. Is somebody trying to do something nasty with your "
              "process?\n");
      abort();
...
bool isFileListConsistent(size_t numof_payloadFiles, FileParams* payloadFiles, int expectedDomainCount,
  ...
  for (int payloadIndex = 0; payloadIndex < numof_payloadFiles; payloadIndex++) {
    ...
    if (expectedFileCount && payloadFiles[payloadIndex].procId >= expectedFileCount)
      return false;  // we can only check the upper bound if we are expecting a certain file count
  }
  return true;
}
...
int checkDirContents(DIR* directory, const char* dirPath, int expectedDomainCount, int expectedFileCount) {
  ...
  // get the underlying file descriptor of the directory, we need this so we can use fstatat() rather than lstat(),
  // which should provide a significant performance benefit
  int dirFileDescriptor = dirfd(directory);
  if (dirFileDescriptor < 0)
    fprintf(stderr, "fatal error: dirfd() returned an error: \"%s\"", strerror(errno)), abort();
  ...
  while ((curEntry = readdir(directory))) {
    ...
    switch (type) {
      ...
      case kPayloadFile:
        ...
        payloadFiles[numof_payloadFiles++] = (FileParams){.patchId = patchId, .procId = procId};
        break;
  ...
  if (!isFileListConsistent(numof_payloadFiles, payloadFiles, expectedDomainCount, expectedFileCount)) {
    // XXX: I'm not sure what to do in this case: There are no unknown files present, but the directory is also not a
    // valid multifile.
    //      It could be a corrupted one (likely), or it could be a directory that just happens to contain a file
    //      `attributes.nc` and one or more `patchN_M.nc` files (unlikely). In the former case, the right course of
    //      action would be to throw a warning that we are overwriting a corrupted multifile, and proceed normally, in
    //      the later case, the right course of action is to abort the operation to avoid overwriting data that we don't
    //      know about. Either choice is wrong as it can lead to data loss (either by overwriting data we didn't
    //      produce, or by aborting a run we needn't abort).
    //
    //      Since the corrupted multifile explanation seems much more likely to me, I opt for the warn-and-overwrite
    //      approach here.
...
int cleanDirContents(DIR* directory, const char* dirPath) {
  // get the underlying file descriptor of the directory, we need this so we can use unlinkat() rather than unlink(),
  // which should provide a significant performance benefit
  int dirFileDescriptor = dirfd(directory);
  if (dirFileDescriptor < 0)
    fprintf(stderr, "fatal error: dirfd() returned an error: \"%s\"", strerror(errno)), abort();
  ...
  while ((curEntry = readdir(directory))) {
    ...
    switch (type) {
      ...
      case kUnknownFile:
        fprintf(stderr,
                "fatal error: unknown file present (file \"%s\" in directory \"%s\") that was not detected in the "
                "previous check. This means we were either racing with some other process creating the file, or there "
                "is a bug in the sanity checking code.\n",
                curEntry->d_name, dirPath);
        return -1;
  ...
  if (mkdir(path, 0777)) {
    ...
    switch (errno) {
      case EEXIST: {
        ...
        // So we just check that we really have a directory, and that it contains only files that would belong to a
        // restart multifile. If that turns out to be the case, we clear the contents of the directory before
        // continuing.
        DIR* directory = openDirectory(path);
        if (!directory) return -1;
      ...
      case EROFS:
        fprintf(
            stderr,
            "error: insufficient permissions for creating directory at \"%s\", check that the directory is writeable\n",
            path);
        return -1;
