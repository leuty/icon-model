...
def base_bc():
    ...
    bc.add_builders(
        "daint_cpu", "daint", "dummy_config_wrapper", config={"a": 2, "b": 3}
    )
    bc.add_builders(
        "daint_gpu", "daint", "dummy_config_wrapper", config={"c": 4, "d": 5}
    )
    bc.add_machine("mistral")
    bc.add_builders(
        "mistral_cpu",
        "mistral",
        "dummy_config_wrapper",
        config={"a": 2, "b": 3},
    )
    bc.add_builders(
        "mistral_gpu",
    ...
    bc.add_builders(
        ["mistral_intel", "mistral_gcc"], "mistral", "dummy_config_wrapper"
    )
...
def test_rmexp_machines(base_bc):
    ...
    assert (
        "mch_opr" not in base_bc.data.index
    ), "Found Experiment after 'remove_experiments'"
...
def test_set_builder_flags(base_bc):
    ...
    assert (
        base_bc.builder_meta["dwd_nec"].flag == "Active"
    ), "Builder flag has not been set correctly"
