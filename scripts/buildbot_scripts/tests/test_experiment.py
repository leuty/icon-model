...
def test_make_runscript():
    ...
    assert (
        run_path / Path(exp.run_name)
    ).exists(), "experiment file has not been created"
...
def test_make_runscript_hardcoded():
    ...
    exp = Experiment(
        "checksuite.xce.dwd.de/exp.run_ICON_02_R2B13_lam.run", "dummy_builder"
    )
    ...
    assert (
        run_path / Path(exp.run_name)
    ).is_symlink(), "hardcoded experiment file is not symlink"
