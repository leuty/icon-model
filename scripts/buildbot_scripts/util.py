...
def relate_with_config(this, other):
    if not this:
        return True  # if no with_config is given, return all builders
    if not other:
        return False  # discard builders where no information is given
...
def relate_without_config(this, other):
    if not this:
        return True  # if no without_config is given, return all builders
    if not other:
        return True  # builders without config information are fine
...
def config_dict_to_list(d):
    if not d:
        return []
    return [
        "{}={}".format(key, value) if value else key for key, value in d.items()
    ]
...
class WhitespaceSeperatedList(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        if not isinstance(value, str):
            self.fail(
                "Input must be a string, found {}".format(value), param, ctx
            )
...
class WhitespaceSeperatedFiles(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        not_found = [
            arg
            for arg in args
            if not Path("{}/{}".format(run_path, arg)).exists()
        ]
        ...
        for ra in regex_args:
            ...
            args += [
                "{}{}".format(parent, p.name) for p in Path(run_path).glob(ra)
            ]
