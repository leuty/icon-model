...
class BuildbotConfig(object):
    ...
    def builder_by_config(self, config, relate):
        return [
            b for b, v in self.builder_meta.items() if relate(config, v.config)
        ]

    def get_indices_from_args(
        self,
        machines=None,
        builders=None,
        with_config=None,
        ...
        config_builders = self.builder_by_config(
            with_config, relate_with_config
        )
        without_config_builders = self.builder_by_config(
            without_config, relate_without_config
        )
        ...
        active_builders = set(
            [
                b
                for b, v in self.builder_meta.items()
                if v.flag and v.flag.lower() == "active"
            ]
        )
    ...
    def add_experiment(
        ...
        midx, bidx = self.get_indices_from_args(
            machines, builders, with_config, without_config
        )
        ...
        try:
            ...
            builder_names = self.data.loc[
                (exp, (midx, bidx))
            ].index.get_level_values(1)
            self.data.loc[(exp, (midx, bidx))] = [
                Experiment(exp, self.builder_meta[bname], run_flags)
                for bname in builder_names
            ]

            print(
                "added experiment {} to {} configuration".format(
                    exp, ", ".join(bidx)
                )
            )
        except:
            raise RuntimeError("Could not find machine or builder")
    ...
    def remove_experiment(
        ...
        if exp not in self.data.index:
            print(
                "requested to remove experiment {}, but does not exist.".format(
                    exp
                )
            )
        else:
    ...
    def add_builders(self, builders, machine, script, config=None, flag=None):
        ...
        if machine not in self.machine_meta.keys():
            print(
                "Could not find machine {} when adding builders {}. Check for typos or add machine {} first.".format(
                    machine, " ".join(builders), machine
                )
            )
            sys.exit(1)

        mcol = pd.MultiIndex.from_product(
            [[machine], builders], names=("machine", "builder")
        )
        ...
        for builder in builders:
            self.builder_meta[builder] = Builder(
                builder, machine, script, config, flag
            )

        print(
            "added builder(s) {} to {} configuration".format(
                ", ".join(builders), machine
            )
        )
    ...
    def get_experiments_by_builder(self, builder):
        if builder not in self.builder_meta.keys():
            print(
                "builder {} is not part of this buildbot configuration".format(
                    builder
                )
            )
            sys.exit(1)
    ...
    def add_dependency_manager(
        ...
        if source_builder and target_builder:
            if len(source_experiment) == 1 and len(target_experiment) == 1:
                ...
                print(
                    "Cannot process dependency from {} to {}. Only 1:1, 1:N and N:1 relations are allowed.".format(
                        source_experiment, target_experiment
                    )
                )
                sys.exit(1)
        ...
        # if both source and target builders are undefined, assume same builder dependencies
            ...
            builder_names = self.data.loc[
                (slice(None), (midx, bidx))
            ].columns.get_level_values(1)
            for bn in builder_names:
                if len(source_experiment) == 1 and len(target_experiment) == 1:
                    self.add_dependency(
                        source_experiment[0], target_experiment[0], bn, bn
                    )
                elif len(source_experiment) > 1 and len(target_experiment) == 1:
                    self.add_dependency_many2one(
                        source_experiment, target_experiment[0], bn, bn
                    )
                elif len(source_experiment) == 1 and len(target_experiment) > 1:
                    self.add_dependency_one2many(
                        source_experiment[0], target_experiment, bn, bn
                    )
                else:
                    print(
                        "Cannot process dependency from {} to {}. Only 1:1, 1:N and N:1 relations are allowed.".format(
            ...
            print(
                "source_builder and target_builder have to be defined or undefined to switch between explicit and assumed dependency mode"
            )
            sys.exit(1)

    def add_dependency_many2one(
        self,
        source_experiments,
        target_experiment,
        source_builder,
        ...
        for se in source_experiments:
            self.add_dependency(
                se, target_experiment, source_builder, target_builder
            )

    def add_dependency_one2many(
        self,
        source_experiment,
        target_experiments,
        source_builder,
        ...
        for te in target_experiments:
            self.add_dependency(
                source_experiment, te, source_builder, target_builder
            )
    ...
    def add_dependency(
        ...
        source_eobj = self.data.loc[
            source_experiment, (source_machine, source_builder)
        ]
        target_eobj = self.data.loc[
            target_experiment, (target_machine, target_builder)
        ]

        if not isinstance(source_eobj, Experiment):
            print(
                "dependency source experiment {} does not exist on builder {}".format(
                    source_experiment, source_builder
                )
            )
        elif not isinstance(target_eobj, Experiment):
            print(
                "dependency target experiment {} does not exist on builder {}".format(
                    target_experiment, target_builder
                )
            )
        else:
    ...
    def to_string(self):
        ...
        for midx in machines:
            out += "{} ():\n".format(midx)
            builders = (
                col_idx.loc[(midx, slice(None)), "builder"].sort_index().values
            )
            for bidx in builders:
                ...
                if bobj.flag.lower() == "active":
                    experiments = (
                        self.data.loc[:, (midx, bidx)].sort_index().values
                    )
