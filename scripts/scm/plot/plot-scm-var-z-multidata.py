...
vardata2 = nc2_fid.variables[metgrm_2d3d][
    :, :90, metgrm_nvar - 1, metgrm_np - 1
]
vardata6 = nc6_fid.variables[metgrm_2d3d][:, :90, metgrm_nvar2 - 1, :].mean(
    axis=2
)
...
hours3 = [
    (d.day - dates3[0].day) * 24
    + (d.hour - dates3[0].hour)
    + (d.minute / 60)
    + (d.second / 3600)
    for d in dates3
]
...
for ns in steps6:
    ...
    datay = heights6
    ax.plot(
        datax[: datax.shape[0] - 1],
        datay[: datax.shape[0] - 1],
        linewidth=2.0,
        # color=colors[nc]), linestyle="dotted"
        label="LES-LAM",
    )  # (' + str(hours6[ns]) + ')')
