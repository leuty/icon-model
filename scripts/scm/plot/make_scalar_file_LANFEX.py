...
lwdn_o[:] = deaccumulate(
    sfile.variables["accthd_s"][:, nc]
)  # Surface down thermal radiationacc. since model start
...
swdndir = deaccumulate(
    sfile.variables["accsodird_s"][:, nc]
)  # Surface down solar direct rad.acc. since model start
swdndif = deaccumulate(
    sfile.variables["accsodifd_s"][:, nc]
)  # Surface down solar diff. rad. acc. since model start
...
shf_o[:] = -deaccumulate(
    sfile.variables["accshfl_s"][:, nc]
)  # surface sensible heat flux acc. since model start
...
tvm = sfile.variables["tvm"][
    :, nc
]  # turbulent transfer velocity for momentum tvm = C_M U_A
...
for t, date in enumerate(date_list):
    ...
    vis[t] = calculate_visibility_wrf(
        tot_qv_dia[t, :],
        tot_qc_dia[t, :],
        tot_qi_dia[t, :],
        qr[t, :],
