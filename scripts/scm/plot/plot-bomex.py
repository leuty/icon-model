...
for ifl in range(0, len(infiles)):
    ...
    ds[ifl]["time_s"] = np.round(ds[ifl].time % 1 * 3600.0 * 24)
...
for it in range(0, len(times)):
    ...
    for ifl in range(0, len(infiles)):
        ...
        for ivar in range(0, len(vars)):
            ax[ivar].plot(
                ds[ifl][vars[ivar]][indt, :, :].mean(axis=1),
                ds[ifl].z_mc.mean(axis=1),
                label=labels[ifl],
            )
...
fig, ax = plt.subplots(
    nrows=len(vars), ncols=1, figsize=(6, 4 * len(vars_surf))
)
for ifl in range(0, len(infiles)):
    for ivar in range(0, len(vars_surf)):
        ax[ivar].plot(
            ds[ifl]["time_s"].values,
            ds[ifl][vars_surf[ivar]][:, :].mean(axis=1).values,
            label=labels[ifl],
        )
