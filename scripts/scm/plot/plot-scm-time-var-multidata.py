...
metgrm_nvar = (
    21  # 20:shfl_s, 21:lhfl_s, 24:t_2m, 26:u_10m, 27:v_10m, 5:q_s, 42:clct
)
...
hours4 = [
    (d.day - dates4[0].day) * 24
    + (d.hour - dates4[0].hour)
    + (d.minute / 60)
    + (d.second / 3600)
    for d in dates4
]
...
print(
    "smoothing with window width:",
    math.ceil(tsmooth / (hours2[1] - hours2[0]) / 2) * 2 + 1,
)

vardata = savgol_filter(
    vardata, math.ceil(tsmooth / (hours[1] - hours[0]) / 2) * 2 + 1, 1, axis=0
)
vardata2 = savgol_filter(
    vardata2,
    math.ceil(tsmooth / (hours2[1] - hours2[0]) / 2) * 2 + 1,
    1,
    axis=0,
...
vardata4 = savgol_filter(
    vardata4,
    math.ceil(tsmooth / (hours4[1] - hours4[0]) / 2) * 2 + 1,
    1,
    axis=0,
)
vardata5 = savgol_filter(
    vardata5, math.ceil(tsmooth / (hours[1] - hours[0]) / 2) * 2 + 1, 1, axis=0
)
vardata6 = savgol_filter(
    vardata6,
    math.ceil(tsmooth / (hours6[1] - hours6[0]) / 2) * 2 + 1,
    1,
    axis=0,
)
