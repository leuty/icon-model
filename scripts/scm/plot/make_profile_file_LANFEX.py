...
def set_var(oname, Iname, levtyp="full", name="", unit=""):
    ...
    if isinstance(Iname, str):  # basestring):
        if levtyp == "full":
            ovar[:, :] = Ivar[:, -num_lev:, nc]
        if levtyp == "half":
            ovar[:, :] = Ivar[:, -(num_lev + 1) :, nc]
    else:
        # ovar[:,:] = Ivar[:,-num_lev:]
        if levtyp == "full":
            ovar[:, :] = Ivar[:, -num_lev:]
...
## read in file
sname = (
    "../" + expname + "/scalars_" + expname + "_ICON_SCM_DWD_DOM01_ML_0001.nc"
)
...
time_o[:] = [
    3600.0 * (dt_in_hours) * float(i) for i in range(len(time[:]))
]  # 60.0  *time[:]
...
# qv [kg/kg]
qv_o = set_var(
    "qv", "tot_qv_dia", name="Vapour specific humidity", unit="kg/kg"
)

# qc [kg/kg]
qc_o = set_var(
    "qc", "tot_qc_dia", name="Condensed water specific humidity", unit="kg/kg"
)
...
if gscp_scheme == "2mom":
    ...
    var = rho[:] * qnc[:]
    qnc_o = set_var(
        "nc", var, name="Cloud droplet number concentration", unit="1/m**3"
    )
    ...
    # Rc = np.ma.masked_where( qnc < 1.0e-12, (3.0/4.0*qc_o[:]/(rho_w*qnc[:]*np.pi))**(1.0/3.0) )
    Rc = np.ma.masked_where(
        qnc_o[:] < 1.0e-12,
        (3.0 / 4.0 * qc_o[:] / (rho_w * qnc_o[:] * np.pi)) ** (1.0 / 3.0),
    )
...
# effective radius
set_var(
    "reff_qc", "reff_qc", name="effective radius of cloud droplets", unit="m"
)
set_var(
    "reff_qr", "reff_qr", name="effective radius of rain droplets", unit="m"
)
...
set_var(
    "dt_mic",
    "ddt_temp_gscp",
    name="Temperature increment due to cloud microphysics",
    unit="K/s",
)
...
set_var(
    "wt", var, "half", name="Vertical turbulent  temperature flux", unit="Km/s"
)
#
##qv flux
...
qv_o = set_var(
    "wq", var, "half", name="Vertical turbulent moisture flux", unit="kg/kg m/s"
)

# turbulent liquid flux
var = -pfile.variables["liqfl_turb"][:, :, nc] / rho_ifc[:, :]
set_var(
    "wl", var, "half", name="Vertical turbulent liquid flux", unit="kg/kg m/s"
)
...
set_var(
    ...
    "ddt_tke_therm",
    "half",
    name="Buoyancy production of TKE",
    unit="m**2/s**3",
)
...
# tkvm [m^2/s]
set_var(
    "km", "tkvm", "half", name="Eddy diffusivity for momentum", unit="m**2/s"
)
# tkvh
...
set_var(
    "dt_turb",
    "ddt_temp_turb",
    name="Temperature increment due to turbulence",
    unit="K/s",
)
...
set_var(
    ...
    "lwflx_up",
    "half",
    name="Upwelling long-wave radiation",
    unit="W/m**2",
)
# lwdn [W/m^2]
set_var(
    ...
    "lwflx_dn",
    "half",
    name="Downwelling long-wave radiation",
    unit="W/m**2",
)
# swup [W/m^2]
set_var(
    ...
    "swflx_up",
    "half",
    name="Upwelling short-wave radiation",
    unit="W/m**2",
)
# swdn [W/m^2]
set_var(
    ...
    "swflx_dn",
    "half",
    name="Downwelling short-wave radiation",
    unit="W/m**2",
)
...
set_var(
    "dt_lw",
    "ddt_temp_radlw",
    name="Temperature increment due to long-wave radiaion",
    unit="K/s",
)
# dt_sw [K/s]
set_var(
    "dt_sw",
    "ddt_temp_radsw",
    name="Temperature increment due to short-wave radiaion",
    unit="K/s",
)
