...
ncout = Dataset(
    expdir + "/scm-GASS-format/" + location + "_icon_u_nudging." + date + ".nc",
    "w",
    format="NETCDF4",
)
...
# metadata
ncout.description = (
    "simulated and processed by Martin Koehler (martin.koehler@dwd.de)"
)
...
time.units = time.units.replace(
    "minutes", "days"
)  # convert minutes since to days since
...
for icon_name in icon_pl_names:
    ...
    names = (
        icon_name if isinstance(icon_name, tuple) else [icon_name]
    )  # convert to list
    for n_name in names[:]:
    ...
    varout = ncout.createVariable(
        var.out_name, dtype("float").char, ("time", "lev", "lat", "lon")
    )
...
for icon_name in icon_ml_names:
    ...
    var = varmeta(icon_name)  # just get metadata
    print(
        "metadata output:              ", var.out_name, var.long_name, var.units
    )
    ...
    var = varmeta(
        icon_name, varmany[:, 0], varmany[:, 1]
    )  #  rescaling of variable data

    # create variable array
    varout = ncout.createVariable(
        var.out_name, dtype("float").char, ("time", "lat", "lon")
    )
    varout.long_name = var.long_name
    varout.units = var.units
    varout.missing_value = -9999
