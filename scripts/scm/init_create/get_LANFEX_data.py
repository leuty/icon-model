...
for i in range(len(seconds_target)):
    idx = np.argmin(
        np.abs(seconds[:] - seconds_target[i])
    )  # seconds.index(seconds_target[i])
    if seconds[idx] > seconds_target[i]:
        print("PROBLEM")
    if idx + 2 >= len(seconds):
        idx = len(seconds) - 2
...
nc_time.units = "seconds since " + datetime.datetime.strftime(
