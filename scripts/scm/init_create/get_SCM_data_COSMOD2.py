...
p0 = 100000.0  # reference atm. pressure[Pa]
...
Omega = 2 * np.pi / 86400.0  # angular velocity of Earth
...
def diag_ustar(z, bflx, w, z0):
    ...
    for itime in range(0, ustar.size):
        if np.abs(bflx[itime]) > 1.0e-6:
            for i in range(0, ni):
                lmo = -ustar[itime] ** 3 / (kappa * bflx[itime])
                zeta = z / lmo
                if zeta > 0.0:
                    if zeta > 1.0e10:  # large zeta
                        ustar[itime] = 1e-10
                        exit
                    else:
...
def get_avg_fields(
    ...
    avg_left = np.ma.average(
        field[:, :, ix_down:ix_up, jx_leftleft:jx],
        axis=(2, 3),
        weights=weigths[:, :, ix_down:ix_up, jx_leftleft:jx],
    )
    avg_up = np.ma.average(
        field[:, :, ix:ix_upup, jx_left:jx_right],
        axis=(2, 3),
        weights=weigths[:, :, ix:ix_upup, jx_left:jx_right],
    )
    avg_right = np.ma.average(
        field[:, :, ix_down:ix_up, jx:jx_rightright],
        axis=(2, 3),
        weights=weigths[:, :, ix_down:ix_up, jx:jx_rightright],
    )
    avg_down = np.ma.average(
        field[:, :, ix_downdown:ix, jx_left:jx_right],
        axis=(2, 3),
        weights=weigths[:, :, ix_downdown:ix, jx_left:jx_right],
    )
    avg_centre = np.ma.average(
        field[:, :, ix_down:ix_up, jx_left:jx_right],
        axis=(2, 3),
        weights=weigths[:, :, ix_down:ix_up, jx_left:jx_right],
    )
    return avg_left, avg_up, avg_right, avg_down, avg_centre
    pass


def get_avg_fields_s(
    field,
    lats,
    lons,
    ix,
...
def get_surrounding_points(lats, lons, latx=52.209722, lonx=14.118889, delta=2):
    ...
    ix_left, jx_left = get_indices(lats, lons, latx, lonx - (delta / 2.0))
    ix_up, jx_up = get_indices(lats, lons, latx + (delta / 2.0), lonx)
    ix_right, jx_right = get_indices(lats, lons, latx, lonx + (delta / 2.0))
    ix_down, jx_down = get_indices(lats, lons, latx - (delta / 2.0), lonx)
...
def calculate_tendencies(
    LSGRADIENT_X1, LSGRADIENT_X2, U_LS1, U_LS2, U_LS3, LSGRADIENT_X3=0
):
...
def interpolate_levels(LEVELS):
    ...
    for i in range(len(int_levels)):
        int_levels[i] = (LEVELS[i + 1] + LEVELS[i]) / 2.0
...
def interpolate_pressure(
    pressure_profile_at_ij,
    height_levels_at_ij,
    surface_pressure_at_ij,
    final_height_levels,
):
    ...
    for i in range(final_height_levels.shape[0]):
        ...
        if final_height_levels[i] < height_levels_at_ij[-1]:
            ...
            height_index = find_lnearest_index(
                height_levels_at_ij, final_height_levels[i]
            )
            # steigung bestimmen m
            m = (logp[height_index - 1] - logp[height_index]) / (
                height_levels_at_ij[height_index - 1]
                - height_levels_at_ij[height_index]
            )
...
def interpolate_linearly(
    profile_at_ij, height_levels_at_ij, surface_data_at_ij, final_height_levels
):
    """interpolate_pressure takes a pressure profile at a specific point, the height levels at that specific point, the surface pressure at that specific point
    and an arbitrary array of height levels. It then interpolates the pressure to that given height levels and gives them back
    """
    ...
    for i in range(final_height_levels.shape[0]):
        ...
        if final_height_levels[i] < height_levels_at_ij[-1]:
            ...
            # steigung bestimmen m
            m = (
                profile_at_ij[height_index - 1] - profile_at_ij[height_index]
            ) / (
                height_levels_at_ij[height_index - 1]
                - height_levels_at_ij[height_index]
            )
            # achsenabschnitt bestimmen, find b
            b = (
                profile_at_ij[height_index]
                - m * height_levels_at_ij[height_index]
            )
            # interpolieren
...
lat = 52.209722
lon = 14.118889
damph = 5000.0  # uGEO and vGEO will converge tu u and v at the location --experimental
...
# constants
nlev = hlevels.size
f2 = 2.0 * Omega * np.cos(2 * np.pi * lat / 360.0)
f3 = 2.0 * Omega * np.sin(2 * np.pi * lat / 360.0)
...
d_y = (
    delta / 360.0 * 2 * np.pi * REarth
)  # calculate domain by using earth's circumference
d_x = d_y * np.cos(
    lat * np.pi / 180.0
)  # attribute for mapping factor (projection of the part-circle in x direction)
print("dx:", d_x)
print("dy:", d_y)
...
print(
    ...
    key_index_ci,
    key_index_cj,
    lats_c[key_index_ci, key_index_cj],
    lons_c[key_index_ci, key_index_cj],
    lat,
    lon,
)
...
LU_CLASS_FRACTION = nc_cid.variables["LU_CLASS_FRACTION"][
    :, key_index_ci, key_index_cj
]
...
(
    ...
    jx_right,
    jx_rightright,
) = get_surrounding_points(lats_f, lons_f, lat, lon, delta)
...
temp = nc_fid.variables["t"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
...
pressure = nc_fid.variables["pres"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
spec_hum = nc_fid.variables["q"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
cloud_mix = nc_fid.variables["clwmr"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
rain_mix = nc_fid.variables["rwmr"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
cloud_ice_mix = nc_fid.variables["QI"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
snow_mix = nc_fid.variables["snmr"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
graupel = nc_fid.variables["grle"][
    :, :, ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
HSURF = nc_cid.variables["HSURF"][
    ix_downdown:ix_upup, jx_leftleft:jx_rightright
]
...
q_t = (spec_hum + cloud_mix + cloud_ice_mix) / (1.0 + cloud_mix + cloud_ice_mix)
...
for it in range(0, timef.size):
    ...
    for ix in range(ix_downdown, ix_upup):
        for iy in range(jx_leftleft, jx_rightright):
            ...
            for iz in np.arange(heights.size - 2, -1, -1):
                mlheigths[it, iz, ix, iy] = (
                    mlheigths[it, iz + 1, ix, iy]
                    - np.log(
                        pressure[it, iz, ix, iy] / pressure[it, iz + 1, ix, iy]
                    )
                    * R_d
                    * 0.5
                    * (temp_v[it, iz, ix, iy] + temp_v[it, iz + 1, ix, iy])
                    / g
                )

    p_LSwestm, p_LSnorthm, p_LSeastm, p_LSsouthm, p_LSm = get_avg_fields(
        pressure,
        hmaskzm,
        lats_f,
        lons_f,
    ...
    t_LSwestm, t_LSnorthm, t_LSeastm, t_LSsouthm, t_LSm = get_avg_fields(
        temp,
        hmaskzm,
        lats_f,
        lons_f,
    ...
    r_LSwestm, r_LSnorthm, r_LSeastm, r_LSsouthm, r_LSm = get_avg_fields(
        r_mix,
        hmaskzm,
        lats_f,
        lons_f,
    ...
    rl_LSwestm, rl_LSnorthm, rl_LSeastm, rl_LSsouthm, rl_LSm = get_avg_fields(
        cloud_mix,
        hmaskzm,
        lats_f,
        lons_f,
    ...
    ri_LSwestm, ri_LSnorthm, ri_LSeastm, ri_LSsouthm, ri_LSm = get_avg_fields(
        cloud_ice_mix,
        hmaskzm,
        lats_f,
        lons_f,
    ...
    tv_LSnorthm = t_LSnorthm * (
        1.0 + 0.608 * r_LSnorthm - rl_LSnorthm - ri_LSnorthm
    )
    tv_LSsouthm = t_LSsouthm * (
        1.0 + 0.608 * r_LSsouthm - rl_LSsouthm - ri_LSsouthm
    )
    rho_LSm = p_LSm / (R_d * tv_LSm)

    HSURFLSwestm, HSURFLSnorthm, HSURFLSeastm, HSURFLSsouthm, HSURFLSm = (
        get_avg_fields_s(
            HSURF,
            lats_f,
            lons_f,
    ...
    for iz in np.arange(heights.size - 2, -1, -1):
        mlheigths_LSwestm[it, iz] = (
            mlheigths_LSwestm[it, iz + 1]
            - np.log(p_LSwestm[it, iz] / p_LSwestm[it, iz + 1])
            * R_d
            * 0.5
            * (tv_LSwestm[it, iz] + tv_LSwestm[it, iz + 1])
            / g
        )
        mlheigths_LSeastm[it, iz] = (
            mlheigths_LSeastm[it, iz + 1]
            - np.log(p_LSeastm[it, iz] / p_LSeastm[it, iz + 1])
            * R_d
            * 0.5
            * (tv_LSeastm[it, iz] + tv_LSeastm[it, iz + 1])
            / g
        )
        mlheigths_LSnorthm[it, iz] = (
            mlheigths_LSnorthm[it, iz + 1]
            - np.log(p_LSnorthm[it, iz] / p_LSnorthm[it, iz + 1])
            * R_d
            * 0.5
            * (tv_LSnorthm[it, iz] + tv_LSnorthm[it, iz + 1])
            / g
        )
        mlheigths_LSsouthm[it, iz] = (
            mlheigths_LSsouthm[it, iz + 1]
            - np.log(p_LSsouthm[it, iz] / p_LSsouthm[it, iz + 1])
            * R_d
            * 0.5
            * (tv_LSsouthm[it, iz] + tv_LSsouthm[it, iz + 1])
            / g
        )

    u_g1m = (
        -(
            (1.0 / (rho_LSm)) * (p_LSnorthm - p_LSsouthm)
            + g * (mlheigths_LSnorthm - mlheigths_LSsouthm)
        )
        / d_y
        / f3
    )
    u_g2m = (
        (
            (1.0 / (rho_LSm)) * (p_LSeastm - p_LSwestm)
            + g * (mlheigths_LSeastm - mlheigths_LSwestm)
        )
        / d_x
        / f3
    )
    ...
    u_g1mh[it, :] = np.interp(
        hlevels,
        mlheigths_LS[it, heights.size - 1 : 0 : -1],
        u_g1m[it, heights.size - 1 : 0 : -1],
    )
    u_g2mh[it, :] = np.interp(
        hlevels,
        mlheigths_LS[it, heights.size - 1 : 0 : -1],
        u_g2m[it, heights.size - 1 : 0 : -1],
    )

    p_LSwestmh[it, :] = np.interp(
        hlevels,
        mlheigths_LSwestm[it, heights.size - 1 : 0 : -1],
        p_LSwestm[it, heights.size - 1 : 0 : -1],
    )
    p_LSeastmh[it, :] = np.interp(
        hlevels,
        mlheigths_LSeastm[it, heights.size - 1 : 0 : -1],
        p_LSeastm[it, heights.size - 1 : 0 : -1],
    )
    p_LSnorthmh[it, :] = np.interp(
        hlevels,
        mlheigths_LSnorthm[it, heights.size - 1 : 0 : -1],
        p_LSnorthm[it, heights.size - 1 : 0 : -1],
    )
    p_LSsouthmh[it, :] = np.interp(
        hlevels,
        mlheigths_LSsouthm[it, heights.size - 1 : 0 : -1],
        p_LSsouthm[it, heights.size - 1 : 0 : -1],
    )

    for ix in range(ix_downdown, ix_upup):
        for iy in range(jx_leftleft, jx_rightright):
            temp_vh[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                temp_v[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            temp_h[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                temp[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            r_mixh[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                r_mix[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            cloud_mixh[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                cloud_mix[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            cloud_ice_mixh[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                cloud_ice_mix[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            pressure_h[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                pressure[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            u_h[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                u[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            v_h[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                v[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            w_h[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights_w.size - 1 : 0 : -1, ix, iy],
                w[it, heights_w.size - 1 : 0 : -1, ix, iy],
            )
            # w_h[it,:,ix,iy]=np.interp(hlevels, mlheigths[it,heights.size-1:0:-1,ix,iy], w[it,heights.size-1:0:-1,ix,iy])
            q_th[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                q_t[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            q_vh[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                spec_hum[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            q_ch[it, :, ix, iy] = np.interp(
                hlevels,
                mlheigths[it, heights.size - 1 : 0 : -1, ix, iy],
                q_c[it, heights.size - 1 : 0 : -1, ix, iy],
            )
            ...
            for iz in np.arange(0, hlevels.size):
                ...
                if ilps.size > 0:
                    ilp = ilps[0]
                    # print("ilp:",ilp,"iz:",iz)
                    pressure_hp[it, iz, ix, iy] = pressure[
                        it, ilp, ix, iy
                    ] * np.exp(
                        9.81
                        * (mlheigths[it, ilp, ix, iy] - hlevels[iz])
                        / R_d
                        / temp_v[it, ilp, ix, iy]
                    )
            ...
            for iz in range(0, hlevels.size):
                hmask = np.where(
                    hlevels[iz] < mlheigths[it, heights.size - 1, :, :]
                )
                hmaskz[it, iz, hmask[0], hmask[1]] = 0


print("compute means")
_, _, _, _, t_LS = get_avg_fields(
    temp_h,
    hmaskz,
    lats_f,
    lons_f,
...
p_LSwest, p_LSnorth, p_LSeast, p_LSsouth, p_LS = get_avg_fields(
    pressure_h,
    hmaskz,
    lats_f,
    lons_f,
...
u_1LSwest, u_1LSnorth, u_1LSeast, u_1LSsouth, u_1LS = get_avg_fields(
    u_h,
    hmaskz,
    lats_f,
    lons_f,
...
u_2LSwest, u_2LSnorth, u_2LSeast, u_2LSsouth, u_2LS = get_avg_fields(
    v_h,
    hmaskz,
    lats_f,
    lons_f,
...
u_3LSwest, u_3LSnorth, u_3LSeast, u_3LSsouth, u_3LS = get_avg_fields(
    w_h,
    hmaskz,
    lats_f,
    lons_f,
...
# theta_l,LS
theta_lh, theta_h = calculate_liquid_potential_temperature(
    temp_h, pressure_h, cloud_mixh
)
theta_lLSwest, theta_lLSnorth, theta_lLSeast, theta_lLSsouth, theta_lLS = (
    get_avg_fields(
        theta_lh,
        hmaskz,
        lats_f,
...
_, _, _, _, theta_LS = get_avg_fields(
    theta_h,
    hmaskz,
    lats_f,
    lons_f,
...
# q_t,LS
q_vLSwest, q_vLSnorth, q_vLSeast, q_vLSsouth, qv_LS = get_avg_fields(
    q_vh,
    hmaskz,
    lats_f,
    lons_f,
...
q_tLSwest, q_tLSnorth, q_tLSeast, q_tLSsouth, q_tLS = get_avg_fields(
    q_th,
    hmaskz,
    lats_f,
    lons_f,
...
ddt_theta_LSA, _ = calculate_tendencies(
    theta_lLS_eastwestgrad, theta_lLS_northsouthgrad, u_1LS, u_2LS, u_3LS
)
ddt_q_t_LSA, _ = calculate_tendencies(
    q_tLS_eastwestgrad, q_tLS_northsouthgrad, u_1LS, u_2LS, u_3LS
)
ddt_q_v_LSA, _ = calculate_tendencies(
    q_vLS_eastwestgrad, q_vLS_northsouthgrad, u_1LS, u_2LS, u_3LS
)
ddt_u_1_LSA, _ = calculate_tendencies(
    u_1LS_eastwestgrad, u_1LS_northsouthgrad, u_1LS, u_2LS, u_3LS
)
ddt_u_2_LSA, _ = calculate_tendencies(
    u_2LS_eastwestgrad, u_2LS_northsouthgrad, u_1LS, u_2LS, u_3LS
)
ddt_u_LSP = -(f3 * u_g2)
ddt_v_LSP = f3 * u_g1
...
levs = np.where(hmaskz[:].mean(axis=(0, 2, 3)) > 0.5)[
    0
]  # only levels, which are above the actual station height
...
nc_thIN[:, 0] = nc_thIN[:, 1] + dtdz_str * (
    nc_height[:, 0] - nc_height[:, 1]
)  # statospheric gradient of pot. temperature, levels above are extrapolated according to this gradinet in SCM
...
nc_psurf[:] = pressure_h[
    :, levs[-1], key_index_fi, key_index_fj
]  # direct input from surface file would be better
nc_ts[:] = temp_h[
    :, levs[-1], key_index_fi, key_index_fj
]  # no better input for now - should be replaced with input from surface file
nc_tg[:] = nc_ts[:]  # no better input for now - input from surface file
nc_qvs[:] = q_vh[
    :, levs[-1], key_index_fi, key_index_fj
]  ##no better input for now - input from surface file ?
nc_Ch[:] = 0.0  # no input
nc_Cq[:] = 0.0  # no input
nc_Cm[:] = 0.0  # no input
...
nc_SOILTYP.description = (
    "soil type derived from FAO Digital Soil Map of the World"
)
nc_SOILTYP.units = ""
...
nc_TOPO.units = "m"
nc_LU_CLASS_FRACTION = do.createVariable(
    "LU_CLASS_FRACTION", np.float64, "nclass_lu"
)
nc_LU_CLASS_FRACTION.description = (
    "Fraction of land use classes in target grid element"
)
