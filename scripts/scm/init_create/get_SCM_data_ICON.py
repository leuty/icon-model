...
file_ml = (
    "/hpc/uwork/mkoehler/run-icon/experiments/exp_003_"
    + inidate
    + "/exp_003_"
    + inidate
    + "_DOM01_ML_0001.nc"
...
print(
    "  clat: ", type(clat), np.shape(clat), np.amin(clat), "to", np.amax(clat)
)
print(
    "  clon: ", type(clon), np.shape(clon), np.amin(clon), "to", np.amax(clon)
)

dist = (clon - lon_scm / 180.0 * np.pi) ** 2 + (
    clat - lat_scm / 180.0 * np.pi
) ** 2
index_near = np.where(dist == dist.min())
index_near = index_near[0][0]  # convert tuple to number
print(
    "  selected lat/lon (rad) ",
    clat[index_near],
    clon[index_near],
    "and deg",
    clat[index_near] / np.pi * 180.0,
    clon[index_near] / np.pi * 180.0,
)
print("  target location: (lat/lon in deg) ", lat_scm, lon_scm)

if lon_scm / 180.0 * np.pi < np.amin(clon) or lon_scm / 180.0 * np.pi > np.amax(
    clon
):
