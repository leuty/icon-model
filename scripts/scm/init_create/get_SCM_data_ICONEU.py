...
w_in_h = np.interp(
    hlevels + HSURF, mlheigths_w[nk + 1 : 0 : -1], w[nk + 1 : 0 : -1]
)
temp_in_h = np.interp(hlevels + HSURF, mlheigths[nk:0:-1], temp[nk:0:-1])
tke_in_h = np.interp(
    hlevels + HSURF, mlheigths_w[nk + 1 : 0 : -1], tke[nk + 1 : 0 : -1]
)
pressure_in_h = np.interp(
    hlevels + HSURF, mlheigths[nk:0:-1], pressure[nk:0:-1]
)
spec_hum_in_h = np.interp(
    hlevels + HSURF, mlheigths[nk:0:-1], spec_hum[nk:0:-1]
)
cloud_mix_in_h = np.interp(
    hlevels + HSURF, mlheigths[nk:0:-1], cloud_mix[nk:0:-1]
)
cloud_ice_mix_in_h = np.interp(
    hlevels + HSURF, mlheigths[nk:0:-1], cloud_ice_mix[nk:0:-1]
)
snow_mix_in_h = np.interp(
    hlevels + HSURF, mlheigths[nk:0:-1], snow_mix[nk:0:-1]
)
...
tv_in_h = temp_in_h * (
    1.0 + 0.608 * r_mix_in_h - cloud_mix_in_h - cloud_ice_mix_in_h
)
...
for it in range(0, nt):
    ...
    it1 = trange[it] % 24
    grib_fcst = (
        nc_fcst_str + "{:02d}".format(it0) + "{:02d}".format(it1) + "0000"
    )
    nc_fcst = (
        nc_fcst_str + "{:02d}".format(it0) + "{:02d}".format(it1) + "0000.nc"
    )
    os.system(
        "cdo -f nc selgridcell,"
        + key_index_s
...
tv_n_f = temp_n_f * (
    1.0 + 0.608 * r_mix_n_f - cloud_mix_n_f - cloud_ice_mix_n_f
)
...
for iz in np.arange(nk - 2, -1, -1):
    mlheigths_n_h[:, iz] = (
        mlheigths_n_h[:, iz + 1]
        - np.log(pressure_n_ha[:, iz] / pressure_n_ha[:, iz + 1])
        * R_d
        / 0.5
        / (1.0 / tv_n_f[:, iz] + 1.0 / tv_n_f[:, iz + 1])
        / g
    )
...
for iz in np.arange(nk - 1, 0, -1):
    mlheigths_n_f[:, iz] = (
        mlheigths_n_h[:, iz + 1] + mlheigths_n_h[:, iz]
    ) * 0.5
...
for it in range(0, nt):
    u_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], u_n_f[it, nk:0:-1]
    )
    v_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], v_n_f[it, nk:0:-1]
    )
    th_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], th_n_f[it, nk:0:-1]
    )
    thv_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], thv_n_f[it, nk:0:-1]
    )
    exner_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], exner_n_f[it, nk:0:-1]
    )
    rho_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], rho_n_f[it, nk:0:-1]
    )
    qv_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], qv_n_f[it, nk:0:-1]
    )
    qc_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], qc_n_f[it, nk:0:-1]
    )
    pressure_n_h[it, :] = np.interp(
        hlevels + HSURF, mlheigths_n_f[it, nk:0:-1], pressure_n_f[it, nk:0:-1]
    )
...
key_indices[8] = get_indices(
    lats_c + 0.5 * delta, lons_c, 0.5 * delta, lat, lon
)
key_indices[2] = get_indices(
    lats_c - 0.5 * delta, lons_c, 0.5 * delta, lat, lon
)
key_indices[4] = get_indices(
    lats_c, lons_c - 0.5 * delta, 0.5 * delta, lat, lon
)
key_indices[6] = get_indices(
    lats_c, lons_c + 0.5 * delta, 0.5 * delta, lat, lon
)
...
u_f = np.zeros(
    [9, nt, nk]
)  # first dimension is location:6-east,2-south,4-west,8-north,5-center
...
for iloc in [2, 4, 5, 6, 8]:
    ...
    for iz in np.arange(nk - 2, -1, -1):
        mlheigths_h[iloc, :, iz] = (
            mlheigths_h[iloc, :, iz + 1]
            - np.log(pressure_h[iloc, :, iz] / pressure_h[iloc, :, iz + 1])
            * R_d
            / 0.5
            / (1.0 / tv_f[iloc, :, iz] + 1.0 / tv_f[iloc, :, iz + 1])
            / g
        )
...
for iz in np.arange(nk - 1, 0, -1):
    mlheigths_f[:, :, iz] = (
        mlheigths_h[:, :, iz + 1] + mlheigths_h[:, :, iz]
    ) * 0.5
...
for it in range(0, nt):
    ug_f[it, :] = (
        (
            (1.0 / (rho_f[5, it, :]))
            * (pressure_f[8, it, :] - pressure_f[2, it, :])
            + g * (mlheigths_f[8, it, :] - mlheigths_f[2, it, :])
        )
        / d_y
        / f3
    )
    vg_f[it, :] = (
        -(
            (1.0 / (rho_f[5, it, :]))
            * (pressure_f[6, it, :] - pressure_f[4, it, :])
            + g * (mlheigths_f[6, it, :] - mlheigths_f[4, it, :])
        )
        / d_x
        / f3
    )
...
for it in range(0, nt):
    ddt_qt_f[it, :] = -(
        u_f[5, it, :] * (qt_f[6, it, :] - qt_f[4, it, :]) / d_x
        + v_f[5, it, :] * (qt_f[8, it, :] - qt_f[2, it, :]) / d_y
    )
    ddt_th_f[it, :] = -(
        u_f[5, it, :] * (th_f[6, it, :] - th_f[4, it, :]) / d_x
        + v_f[5, it, :] * (th_f[8, it, :] - th_f[2, it, :]) / d_y
    )
    ddt_u_f[it, :] = -(
        u_f[5, it, :] * (u_f[6, it, :] - u_f[4, it, :]) / d_x
        + v_f[5, it, :] * (u_f[8, it, :] - u_f[2, it, :]) / d_y
    )
    ddt_v_f[it, :] = -(
        u_f[5, it, :] * (v_f[6, it, :] - v_f[4, it, :]) / d_x
        + v_f[5, it, :] * (v_f[8, it, :] - v_f[2, it, :]) / d_y
    )
...
for it in range(0, nt):
    for iloc in [2, 4, 5, 6, 8]:
        u_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            u_f[iloc, it, nk:0:-1],
        )
        v_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            v_f[iloc, it, nk:0:-1],
        )
        w_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_h[iloc, it, nk + 1 : 0 : -1],
            w_f[iloc, it, nk + 1 : 0 : -1],
        )
        temp_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            temp_f[iloc, it, nk:0:-1],
        )
        pressure_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            pressure_f[iloc, it, nk:0:-1],
        )
        spec_hum_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            spec_hum_f[iloc, it, nk:0:-1],
        )
        cloud_mix_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            cloud_mix_f[iloc, it, nk:0:-1],
        )
        cloud_ice_mix_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            cloud_ice_mix_f[iloc, it, nk:0:-1],
        )
        rho_h[iloc, it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            rho_f[iloc, it, nk:0:-1],
        )
        ug_h[it, :] = np.interp(
            hlevels + HSURF, mlheigths_f[iloc, it, nk:0:-1], ug_f[it, nk:0:-1]
        )
        vg_h[it, :] = np.interp(
            hlevels + HSURF, mlheigths_f[iloc, it, nk:0:-1], vg_f[it, nk:0:-1]
        )
        ddt_qt_h[it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            ddt_qt_f[it, nk:0:-1],
        )
        ddt_th_h[it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            ddt_th_f[it, nk:0:-1],
        )
        ddt_u_h[it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            ddt_u_f[it, nk:0:-1],
        )
        ddt_v_h[it, :] = np.interp(
            hlevels + HSURF,
            mlheigths_f[iloc, it, nk:0:-1],
            ddt_v_f[it, nk:0:-1],
        )
...
nc_thvIN[:, 0] = nc_thvIN[:, 1] + dtdz_str * (
    nc_height[:, 0] - nc_height[:, 1]
)  # statospheric gradient of pot. temperature, levels above are extrapolated according to this gradinet in SCM
nc_qvIN[0, :] = qv_in_h
nc_qvIN[1:, :] = qv_n_h[1:, :]
