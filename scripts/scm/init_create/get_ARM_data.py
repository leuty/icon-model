...
for k in range(z.size):
    ...
    if z[k] <= 50.0:
        ...
        thl[k] = 343.2 + (z[k] - 5500.0) * dtdz_fratm
...
# set the time series
t = np.array([0.0, 4.0, 6.5, 7.5, 10.0, 12.5, 14.5])
tout = np.arange(0, 15.0, 0.5)

H = np.interp(tout, t, np.array([-30.0, 90.0, 140.0, 140.0, 100.0, -10.0, -10]))
LE = np.interp(tout, t, np.array([5.0, 250.0, 450.0, 500.0, 420.0, 180.0, 0]))
...
tls = np.array([0.0, 3.0, 6.0, 9.0, 12.0, 14.5])
...
for n in range(tls.size):
    ...
    for k in range(z.size):
        ...
        if z[k] <= 1000.0:
            ...
            thlls_in[n, k] = tendthl - (z[k] - 1000.0) * (tendthl) / (
                5500.0 - 1000.0
            )
            qtls_in[n, k] = tendqt - (z[k] - 1000.0) * (tendqt) / (
                5500.0 - 1000.0
            )
...
tls *= 3600.0  # h to s
thlls /= 3600.0  # h to s
thlls_in /= 3600.0  # h to s
qtls /= 3600.0  # h to s
qtls /= 1000.0  # g to kg
qtls_in /= 3600.0  # h to s
qtls_in /= 1000.0  # g to kg
...
sfc_lat_flx = LE  # W/m2
sfc_sens_flx = H  # W/m2
p_surf = 97000.0  # surface pressure [Pa]
z0 = 0.035  # m
dt_relax = 7200.0  # s
...
for it in range(0, tout.size):  # in this case all are the same
    ...
    nc_tkeIN[it, :] = tke
    nc_thIN[it, :] = thl
    nc_thIN[it, 0] = nc_thIN[it, 1] + dtdz_str * (
        nc_height[it, 0] - nc_height[it, 1]
