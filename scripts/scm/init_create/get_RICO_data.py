...
hlevels = np.array([0.0, 740, 2260, 2980, 3260, 4000.0, 6000.0, 20000.0])
...
for k in range(z.size):
    ...
    if z[k] < 740.0:
        ...
        thl[k] = 317 + (z[k] - 4000.0) * dtdz_fratm
...
def esat(T):
    ...
    return c0 + x * (
        c1
        + x
        * (c2 + x * (c3 + x * (c4 + x * (c5 + x * (c6 + x * (c7 + x * c8))))))
    )
...
# surface parameters
p_surf = 101540.0  # surface pressure [Pa]
SST = 299.8
ths = SST / (p_surf / 1.0e5) ** (287.04 / 1005.0)
qs = qsat(p_surf, SST)
z0 = 0.0002  # m
CM = 0.001229
CQ = 0.001133
