...
def cleanUp():
    dbg(LOG["removeThem"])
    map(
        lambda x: os.remove(str(x)) if os.path.exists(str(x)) else None,
        LOG["removeThem"],
    )
    LOG["removeThem"] = []
...
def collectImageToMapByRows(images, columns, ofile):
    ...
    for column in imageColumns:
        ...
        cmd = "convert +append %s %s" % (
            " ".join(imageColumns[column]),
            rowFile,
        )
        dbg(cmd)
...
def _computeRegioMean(depth, varname, ifile, myMask, ofile, newData):
    ...
    cdo.fldmean(
        input="-div -sellevel,{0} -selname,{1} {2} {3}".format(
            depth, varname, ifile, myMask
        ),
        output=ofile,
        forceOutput=newData,
    )
...
def globalTempSalRho1D(ifile, initfile, maskfile, varNames, ofile):
    cdo.timmean(
        input="-fldmean -div -sub -selname,{0} {1} {2} {3}".format(
            varNames, ifile, initfile, maskfile
        ),
        output=ofile,
    )
...
def globalTempSalRho2D(ifile, initfile, varNames, archdir):
    ...
    cdo.timmean(
        input="-div -sub -selname,{0} -sellevidx,1 {1} -selname,{2} -sellevidx,1 {3} -sellevidx,1 -selname,{4} -seltimestep,1 {5}".format(
            varNames, ifile, varNames, initfile, maskName, ifile
        ),
        output=ofile,
    )
...
def grepYear(ifiles, year, archdir, forceOutput, experimentInfo):
    ...
    if not os.path.exists(yearFile) or forceOutput:
        ...
        for ifile in ifiles:
            dbg(ifile)
            ofile = "{0}/_catFile_{1}_{2}".format(
                archdir, year, os.path.basename(ifile)
            )
            catFiles.append(
                cdo.selyear(year, input="{0}".format(ifile), output=ofile)
            )
        ...
        cdo.cat(
            input=" ".join(catFiles),
            output=yearFile,
            options="-f nc4c -z zip_1" if cdo.version() == "1.6.9" else "",
        )
...
def splitFilesIntoYears(
    plotYears, filesOfYears, archdir, forceOutput, expInfo, procs
):
    pool = multiprocessing.Pool(procs)
    yearFiles = []
    for year in plotYears:
        files = filesOfYears[year]
        yearFile = pool.apply_async(
            grepYear, [files, str(year), archdir, forceOutput, expInfo]
        )
        yearFile, yearMeanFile = getFileNamesForYears(
            str(year), archdir, expInfo
        )
...
def computeMaskedTSRMeans1D(
    ifiles, varList, initFile, maskFile, exp, archdir, procs
):
    pool = multiprocessing.Pool(procs)
    results = []

    initFile = cdo.selname(
        ",".join(varList),
        input=initFile,
        output="/".join([archdir, ".TSR_{0}_init".format(exp)]),
    )
    # compute the 1d (vertical) year mean variance to the initial state
    for ifile in ifiles:
        _ofile = "/".join(
            [archdir, ".TSR_1D_{0}".format(os.path.basename(ifile))]
        )
        ofile = pool.apply_async(
            globalTempSalRho1D,
            [ifile, initFile, maskFile, ",".join(varList), _ofile],
        )
        ofile = _ofile
    ...
    merged = "/".join(
        [archdir, "TSR_1D_{0}_complete.nc".format(options["EXP"])]
    )
    if os.path.exists(merged):
        os.remove(merged)
    merged = cdo.cat(input=" ".join(sorted(results)), output=merged)

    # rename vertical axis if present - ignore exit status (return code differs depending on the ncp version)
    if subprocess.call(
        "ncrename -d depth_2,depth -v depth_2,depth -O {0}".format(merged),
        shell=True,
        env=os.environ,
    ):
        print("ERROR: ncrename failed")
        print(
            "ERROR: ncrename -d depth_2,depth -v depth_2,depth_2 -O {0}".format(
                merged
            )
        )
    #   exit(1)
...
def computeMaskedTSRMeans2D(ifiles, varList, exp, archdir, procs, log):
    ...
    ofile = "/".join(
        [archdir, "TSR_2D_%s_complete_timmean.nc" % (options["EXP"])]
    )
    cdo.selname(
        ",".join(varList), input=log["last30YearsMeanBias"], output=ofile
    )
    ofileMasked = "/".join(
        [
            archdir,
            "TSR_2D_{0}_complete_timmean_masked.nc".format(options["EXP"]),
        ]
    )
...
def plotOnlineDiagnostics(diagnosticFiles, options):
    plots = []
    onlineMeanValueFileName = options[
        "ARCHDIR"
    ] + "/TS_last10YearMean_%s.txt" % (options["EXP"])
    onlineMeanValueFile = open(onlineMeanValueFileName, "w")
    onlineMeanValueDocumentName = options[
        "PLOTDIR"
    ] + "/TS_last10YearMean_%s.pdf" % (options["EXP"])
    # cat all diagnostics together
    diagnosticJoined = (
        options["ARCHDIR"] + "/" + "_".join([options["EXP"], "diagnostics.txt"])
    )
    dbg(diagnosticFiles)
    ...
    rightColumnWidth = 8
    onlineMeanValueFile.write(
        "   ".join(
            [
                "Passage/Parameter".ljust(leftColumnWith),
                "".rjust(rightColumnWidth),
            ]
        )
        + "\n"
    )
    ...
    for column in columns2Plot:
        ...
        for i, date in enumerate(dates):
            if (lastCompleteYear - 10 < date.year) and (
                date.year <= lastCompleteYear
            ):
                # dbg(date)
                last10YearsDates.append(date)
                if column in passages:
                    last10YearsValues.append(values[i] / 1e9)
                else:
                    last10YearsValues.append(values[i])
        if column in passages:
            last10YearMean = str(
                round(numpy.array(last10YearsValues).mean(), 2)
            )
        else:
            last10YearMean = "%.2E" % round(
                numpy.array(last10YearsValues).mean(), 2
            )

        dbgOutput = " : ".join(
            [column.ljust(leftColumnWith), last10YearMean.rjust(8)]
        )
        dbg(dbgOutput)
        onlineMeanValueFile.write(dbgOutput + "\n")

        # complete timeseries
        ofile = "%s/TS_%s_%s_%s.png" % (
            options["PLOTDIR"],
            column,
            options["EXP"],
            options["TAG"],
        ...
        pylab.title(
            "%s , %s : %s TF - all %s years | last 10year mean: %s %s"
            % (
                options["EXP"],
                options["TAG"],
                column.upper(),
        ...
        if column in ice_fields:
            ...
            pylab.plot_date(
                dates, values, linestyle="-", marker=",", linewidth=1.0
            )
        pylab.savefig(ofile)
        pylab.clf()
        plots.append(ofile)
    ...
    cmd = "enscript %s -o - | ps2pdf - %s" % (
        onlineMeanValueFileName,
        onlineMeanValueDocumentName,
    )
    dbg(cmd)
    if subprocess.check_call(cmd, shell=True, env=os.environ):
        print(
            "Creation of data table %s failed!" % (onlineMeanValueDocumentName)
        )
    if options["DEBUG"]:
...
def processRegionMean(
    ...
    for location, regioCode in regioCodes.iteritems():
        ofile = "/".join(
            [options["ARCHDIR"], "_".join(["regioMask", location]) + ".nc"]
        )
        ofileTemp = "/".join(
            [options["ARCHDIR"], "_".join(["_regioMask", location]) + ".nc"]
        )
        cdo.eqc(
            regioCode,
            input="-selname,%s %s" % (regioMaskVar, init),
            output=ofileTemp,
        )
        regioMasks[location] = (
            ofileTemp  # cdo.div(input = '%s %s'%(ofileTemp,ofileTemp),output = ofile)
        )
    ...
    for depth in regioDepths:
        ...
        ofile = "/".join(
            [
                options["ARCHDIR"],
                "_".join(["regioVertMask", depth + "m"]) + ".nc",
            ]
        )
    ...
    for location, regioCode in regioCodes.iteritems():
        ...
        for depth in regioDepths:
            ...
            myMask = cdo.mul(
                input=" ".join(
                    [regioMasks[location], regioVertMasks[str(depth)]]
                ),
                output="/".join(
                    [
                        options["ARCHDIR"],
                        "_".join(["regioMask", location, str(depth) + "m"])
                        + ".nc",
                    ]
                ),
            )
            ...
            for varname in regioVars:
                ...
                ofile = "/".join(
                    [
                        options["ARCHDIR"],
                        "_".join(
                            ["regioMean", location, varname, str(depth) + "m"]
                        )
                        + ".nc",
                    ]
                )
                regioPool.apply_async(
                    _computeRegioMean,
                    [depth, varname, yearMeanFile, myMask, ofile, hasNewFiles],
                )
                regioLock.acquire()
...
def createOutputDocument(plotdir, firstPage, docname, doctype, debug):
    ...
    for image in images:
        ...
        if subprocess.check_call(
            "convert %s %s" % (image, ofile), shell=True, env=os.environ
        ):
    ...
    if "ps" == doctype:
        if subprocess.check_call(
            "psmerge -o%s %s" % (document, " ".join(ofiles)),
            shell=True,
            env=os.environ,
        ):
        ...
        if subprocess.check_call(
            "pdftk %s cat output %s" % (" ".join(ofiles), document),
            shell=True,
            env=os.environ,
        ):
...
def yearMeanFileName(archdir, exp, firstYear, lastYear):
    return "/".join(
        [
            archdir,
            "_".join([exp, "%s-%s" % (firstYear, lastYear), "yearmean.nc"]),
        ]
    )
...
def last30yearFileName(archdir, exp, firstYear, lastYear):
    return "/".join(
        [
            archdir,
            "_".join(
                [exp, "{0}-{1}".format(firstYear, lastYear), "allData.nc"]
            ),
        ]
    )
...
def yearMonMeanFileName(archdir, exp, firstYear, lastYear):
    return "/".join(
        [
            archdir,
            "_".join([exp, "%s-%s" % (firstYear, lastYear), "yearmonmean.nc"]),
        ]
    )
        ...
        if varname in plotConfig["availableVars"]:
            ...
            oFile = "/".join(
                [
                    options["PLOTDIR"],
                    varname
                    + "_Horizontal-atSurface_"
                    + "_".join([_filename, plotConfig["tag"]]),
                ]
            )
            cmd = [
                options["ICONPLOT"],
            ...
            if 1 == len(
                cdo.showlevel(
                    input="-selname,%s -seltimestep,1 %s"
                    % (varname, plotConfig["iFile"])
                )
            ):
                continue
            ...
            if "limits" in plotConfig:
                ...
                if "plotLevs" in limits:
                    ...
                    cmd.append(
                        "-minVar=%s -maxVar=%s %s"
                        % (limits["minVar"], limits["maxVar"], limits["mode"])
                    )
...
def maskCellVariables(ifile, maskfile, ofile, force, log):
    ...
    #
    groups = {
        "cellVars_2d": cdo.showname(
            input="-selzaxis,1 -selgrid,1 -seltimestep,1 {0}".format(ifile)
        )[0].split(" "),
        "cellVars_3d": cdo.showname(
            input="-selzaxis,2 -selgrid,1 -seltimestep,1 {0}".format(ifile)
        )[0].split(" "),
        "cellVars_ice": cdo.showname(
            input="-selzaxis,4 -selgrid,1 -seltimestep,1 {0}".format(ifile)
        )[0].split(" "),
    }
    ...
    for group, groupVars in groups.iteritems():
        if group in ["cellVars_2d", "cellVars_ice"]:
            ...
            sellevel = ""
        masked_groups[group] = cdo.div(
            input="-selname,{0} {1} {2} {3}".format(
                ",".join(groupVars), ifile, sellevel, maskfile
            ),
            output="{0}_{1}_masked".format(ifile, group),
        )
    # merge groups together
    cdo.merge(
        input="{0}".format(" ".join(masked_groups.values())), output=ofile
    )
...
def collectAllYearMeanData(years, archdir, exp, log):
    ...
    collectYearMeanDataFile = "{0}/{1}_{2}-{3}_yearmean.nc".format(
        archdir, exp, availableYears[0], availableYears[-1]
    )
    cdo.cat(
        input=" ".join(availableYearMeanFiles), output=collectYearMeanDataFile
    )
    #
...
def createYmonmeanForYears(log, givenPlotYear, options):
    ...
    #
    # cat together
    yearMonMeanFile = yearMonMeanFileName(
        options["ARCHDIR"], options["EXP"], firstPlotYear, lastPlotYear
    )
    last30yearFile = last30yearFileName(
        options["ARCHDIR"], options["EXP"], firstPlotYear, lastPlotYear
    )
    collectedPlotDataFile = cdo.cat(
        input=" ".join(plotFiles),
        output=str(last30yearFile + "_all"),
        options="-f nc4c -z zip_1",
    )
    cellVars = maskCellVariables(
        collectedPlotDataFile, log["mask"], last30yearFile, False, LOG
    )
    #
    # produce masked yearmonmean file
    LOG["last30YearsMonMean"] = cdo.ymonmean(
        input=last30yearFile, output=yearMonMeanFile, options="-f nc4c -z zip_1"
    )
    #
    # produce masked yearmean
    LOG["last30YearsMean"] = cdo.timmean(
        input=str(yearMonMeanFile),
        output="%s/%s_%s-%s_ym.nc"
        % (options["ARCHDIR"], options["EXP"], firstPlotYear, lastPlotYear),
        options="-f nc4c -z zip_1",
    )
    LOG["last30YearsMeanBias"] = cdo.sub(
        input=" {0} -selname,{1} {2}".format(
            LOG["last30YearsMean"], ",".join(cellVars), LOG["init"]
        ),
        output="%s/%s_%s-%s_ymBias.nc"
        % (options["ARCHDIR"], options["EXP"], firstPlotYear, lastPlotYear),
...
def computePlotYears(availableYears, givenPlotYear):
    ...
    if plotYear in availableYears:
        ...
        print(
            "PLOTYEAR:{0} is not available - last year {1} will be used".format(
...
LOG["init"] = cdo.seltimestep(
    1,
    input=iFiles[0],
    output="/".join([options["ARCHDIR"], "%s_init.nc" % (options["EXP"])]),
)
# }}} ==========================================================================
# COMPUTE CELL MASK FOR LATER APPLICATION {{{ ==================================
LOG["mask"] = cdo.selname(
    "wet_c",
    input="-seltimestep,1 %s" % (iFiles[0]),
    output="/".join([options["ARCHDIR"], "%s_mask.nc" % (options["EXP"])]),
)
# }}} ==========================================================================
...
if "procPsi" in options["ACTIONS"] or "plotPsi" in options["ACTIONS"]:
    ...
    psiGlobalFile = "/".join(
        [options["ARCHDIR"], "_".join([psiModelVariableName, yearInfo]) + ".nc"]
    )
    cdo.timmean(
        input="-selname,{0} -selyear,{1}/{2} {3}".format(
            psiModelVariableName,
            years4Psi[0],
            years4Psi[-1],
            LOG["last30Years"],
        ),
...
if "procRegio" in options["ACTIONS"]:
    ...
    dbg(LOG["depths"])
    dbg(len(LOG["depths"]))
    regioMeanData = processRegionMean(
        options,
        LOG["mask"],
        LOG["init"],
        ymFile,
...
if "procMoc" in options["ACTIONS"]:
    ...
    if not mocFiles:
        ...
        mocLastNtime = (
            int(mocLog[mocFiles[-1]]) - 1
        )  # avoid the last one, might be corrupted
        ...
        if mocNeededNSteps <= mocLastNtime:
            ...
            mocMeanFile = cdo.timmean(
                input="-seltimestep,{0}/{1} {2}".format(
                    mocLastNtime - mocNeededNSteps + 1,
                    mocLastNtime,
                    mocFiles[-1],
                ),
                output=mocMeanFile,
...
if "procTSR" in options["ACTIONS"]:
    ...
    t_s_rho_Output_1D = computeMaskedTSRMeans1D(
        t_s_rho_Input_1D,
        ["t_acc", "s_acc", "rhopot_acc"],
        LOG["init"],
        LOG["mask"],
        options["EXP"],
        options["ARCHDIR"],
    ...
    t_s_rho_Output_2D = applyMask(
        " -selname,t_acc,s_acc,rhopot_acc {0}".format(
            LOG["last30YearsMeanBias"]
        ),
        LOG["mask"],
        "/".join(
            [
...
if "plotPsi" in options["ACTIONS"]:
    ...
    psiSelectionConfig = {
        "indonesian_throughflow": {
            "lonlatbox": "90,150,-20,40",
        },
        "gibraltar": {
            "lonlatbox": "-20,10,25,50",
        },
        "north_atlantic": {
            "lonlatbox": "-60,20,50,80",
        },
        "drake_passage": {
            "lonlatbox": "-90,-30,-80,-40",
        },
        "beringStrait": {
            "lonlatbox": "-180,-100,30,80",
        },
        "agulhas": {
            "lonlatbox": "10,50,-55,-15",
        },
    }
    psiSelectionConfig = {}
    ...
    plotFile = (
        options["PLOTDIR"]
        + "/"
        + "_".join(["psi", yearInfo, options["EXP"], options["TAG"] + ".png"])
    )
    ...
    cmd = "{0} {1} {2}".format(
        options["CALCPSI"],
        psiGlobalFile,
        " DEBUG=1 WRITEPSI=true AREA={0} TITLE='{1}' PLOT={2}".format(
            options["GRID"], title, plotFile
        ),
    )
    dbg(cmd)
    ...
    for area, selection in psiSelectionConfig.iteritems():
        ...
        plotFile = (
            options["PLOTDIR"]
            + "/"
            + "_".join(["psi", area, options["EXP"], options["TAG"] + ".png"])
        )
        cmd = "{0} {1} {2}".format(
            options["CALCPSI"],
            psiGlobalFile,
            " AREA=local TITLE='{0}' PLOT={1} BOX={2} ".format(
                title, plotFile, selection["lonlatbox"]
            ),
        )
        dbg(cmd)
        plotCommands.append(cmd)
...
if "plotHorz" in options["ACTIONS"]:
    ...
    horizontalConfig = {
        ...
        "availableVars": cdo.showname(input=str(LOG[LOG["years"][-2]]))[
            0
        ].split(" "),
    ...
    horizontalConfig = {
        ...
        "availableVars": cdo.showname(input=str(LOG["last30YearsMean"]))[
            0
        ].split(" "),
        "sizeOpt": "-xsize=1200 -ysize=800",
...
if "plotTf" in options["ACTIONS"]:
    ...
    if "global" == options["GRID"]:
        diagnosticFiles = sorted(
            glob.glob(
                os.path.sep.join([LOG["dataDir"], "oce_diagnostics-*txt"])
            ),
            key=mtime,
        )
        if options["JOBISRUNNING"]:
        ...
        for varname in ["t_acc", "s_acc", "rhopot_acc"]:
            ...
            oFile = "/".join(
                [
                    ...
                    varname
                    + "_AtlanticProfile_"
                    + "_".join([options["EXP"], options["TAG"]]),
                ]
            )
            iFile4XSection = str(
                LOG["last30YearsMean"]
            )  # take last 30 year mean file
            # mask by devision
            # TODO:
            iFile4XSection = cdo.div(
                input="-selname,{0} {1} {2}".format(
                    ",".join(["t_acc", "s_acc", "rhopot_acc"]),
                    iFile4XSection,
                    LOG["mask"],
                ),
            ...
            if "rhopot_acc" == varname:
                # substract 1000
                iFile4XSection = cdo.subc(
                    1000.0,
                    input="-selname,{0} {1}".format(varname, iFile4XSection),
                    output=os.path.splitext(iFile4XSection)[0]
                    + "_{0}_subc1000.nc".format(varname),
                )
                add4Cleanup([iFile4XSection])
            ...
            oFile = "/".join(
                [
                    options["PLOTDIR"],
                    varname
                    + "_AtlanticProfile_BiasToInit"
                    + "_".join([options["EXP"], options["TAG"]]),
                ]
            )
            iFile4XSection = str(
                LOG["last30YearsMeanBias"]
            )  # take last 30 yearmean bias to init
            # mask by devision
            # TODO:
            iFile4XSection = cdo.div(
                input="-selname,{0} {1} {2}".format(
    ...
    for location, regioCode in regioCodes.iteritems():
        for varname in regioVars:
            ...
            for depth in regioDepths:
                ...
                dates = re.sub(
                    "(\s)+", r"\1", cdo.showdate(input=ifile)[0]
                ).split(" ")
...
if "plotMoc" in options["ACTIONS"]:
    ...
    mocPlotSetup = {
        "TITLE": "ICON: %s : %s %s"
        % (options["EXP"], mocFiles[-1], "[last 10y mean]"),
        "IFILE": mocMeanFile,
        "OFILE": "/".join(
            [options["PLOTDIR"], "MOC_%s_%s" % (options["EXP"], mocDateInfo)]
        ),
        "OTYPE": "png",
    }
    dbg(mocPlotSetup)
...
if "procMld" in options["ACTIONS"]:
    ...
    mldData = cdo.timmax(
        input=" -selname,mld,wet_c %s" % (mldLastYear),
        output="/".join(
            [
                options["ARCHDIR"],
                "_".join(["mld", os.path.basename(mldLastYear)]),
            ]
        ),
    )
if "plotMld" in options["ACTIONS"]:
    mldPlot = "/".join(
        [options["PLOTDIR"], "_".join(["mld", os.path.basename(mldLastYear)])]
    )
    mldCmd = [
        options["ICONPLOT"],
        "-iFile=%s" % (mldLastYear),
        ...
        "-maskName=wet_c",
        "-oType=" + options["OFORMAT"],
        '-tStrg="%s: ~C~ maximum MLD in the last year"' % (options["EXP"]),
        "-oFile=%s" % (mldPlot),
    ]
    print(mldCmd)
    mldCmd = " ".join(mldCmd)
...
if "procIce" in options["ACTIONS"]:
    iceData = cdo.selname(
        ...
        output="/".join(
            [
                options["ARCHDIR"],
                "sea_ice_{0}".format(os.path.basename(LOG["last30YearsMean"])),
            ]
        ),
    )
    meanIceThicknessData = cdo.setunit(
        "m",
        input=" -expr,'hiMean=hi_acc*conc_acc' {0}".format(iceData),
        output="/".join(
            [
                options["ARCHDIR"],
                "meanIceThickness_{0}".format(os.path.basename(iceData)),
            ]
        ),
    )
    ...
    for mapTypeIndex, mapType in enumerate(["NHps", "SHps"]):
        ...
        iFile = cdo.selmon(
            ...
            output="/".join(
                [
                    options["ARCHDIR"],
                    "month{0}_{1}".format(month, os.path.basename(iceData)),
                ]
            ),
        )
        icePlotConfig = {
            ...
            "sizeOpt": "-xsize=1200 -ysize=800",
            "title": "%s:~C~ SEA ICE month %s, last-30-yearmonmean"
            % (options["EXP"], month),
        ...
        iFile = cdo.selmon(
            ...
            output="/".join(
                [
                    options["ARCHDIR"],
                    "month{0}_{1}".format(
                        month, os.path.basename(meanIceThicknessData)
                    ),
                ]
            ),
        )
        icePlotConfig = {
            ...
            "sizeOpt": "-xsize=1200 -ysize=800",
            "title": "%s:~C~ SEA ICE, month %s, last-30-yearmonmean ~C~ mean sea ice thickness"
            % (options["EXP"], month),
            "tag": "last30YearMeanICE_%s" % (mapType),
            "mapType": mapType,
            "colormap": "WhBlGrYeRe",
...
if "finalDoc" in options["ACTIONS"]:
    createOutputDocument(
        options["PLOTDIR"],
        diagnosticTable,
        "_".join(["ALL", options["EXP"], options["TAG"]]),
        options["DOCTYPE"],
        options["DEBUG"],
    )
# }}} --------------------------------------------------------------------------
