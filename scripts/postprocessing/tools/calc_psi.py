...
if remapInput:
    ifile = cdo.setmisstoc(
        0.0, input="-remapcon,r360x180 " + inputfile, options="-P 8"
    )
...
if options["WRITEPSI"]:
    ...
    psiFileName = cdo.chname(
        "%s,psi" % (varName),
        input=ifile,
        output=os.path.dirname(inputfile) + "/psi_remapped.nc",
        force=True,
        options="-r",
    )
...
mapproj.drawparallels(
    np.array([-80, -60, -40, -20, 0, 20, 40, 60, 80]),
    labels=[1, 1, 0, 0],
    fontsize=6,
    linewidth=0.2,
)
mapproj.drawmeridians(
    range(0, 360, 30), labels=[0, 0, 0, 1], fontsize=6, linewidth=0.2
)
...
# filled contours
CS = plt.contourf(lonsPlot, latsPlot, psi, levels, extend="both", cmap=colormap)
# contour lines
CSBar = plt.contour(
    lonsPlot,
    latsPlot,
