...
def writeOldTimerProcsDistribution(
    outName, logFileName, timerName, counterIndex
):
...
def writeTimerProcsDistribution(
    outName, folderName, timerFilePrefix, timerName, counterIndex
):
...
def processFolderList(foldersList, timersList, counterIndex):
    ...
    for i in range(noOfFolders):
        for j in range(noOfTimers):
            ...
            (minTime, meanTime, maxTime) = getTimerValues(
                foldersList[i], timersList[j], counterIndex
            )
...
def writeParallelEfficency(name, timerList, timerIndex, coresList):
    ...
    for i in range(noOfFolders):
        ...
        outFile.write(
            "  "
            + str(referenceTime / (timerList[i][timerIndex] * coresList[i]))
        )
        outFile.write("\n")
    outFile.close()
...
def writeStatsAllList(
    name, timeList, coresList, daysList, grid_size, plotScaling
):
    # write times
    noOfFolders = len(coresList)
    noOfTimers = len(timeList[0])
    ...
    for i in range(noOfFolders):
        ...
        for j in range(noOfTimers):
            normalizedTime = (
                (timeList[i][j] / daysList[i]) * coresList[i]
            ) / grid_size[i]
            outFile.write("  " + str(normalizedTime))
        outFile.write("\n")
    outFile.close()
    ...
    if plotScaling == plotYearsPerDay:
        ...
        for i in range(noOfFolders):
            yearsPerDay = (daysList[i] * 24.0 * 3600.0) / (
                timeList[i][0] * 365.0
            )
...
def getImbalance(minTimeList, maxTimeList, refTimer):
    ...
    for i in range(noOfFolders):
        for j in range(noOfTimers):
            ...
            imbalancePerCent[i][j] = (
                imbalanceList[i][j] * 100.0
            ) / minTimeList[i][refTimer]
...
timersListTotal = [" total"]


timerExchData = [" exch_data"]
...
def stdall_timers_statistics(folders_list, resolution, cores_list, days_list):
    ...
    (minTimes, meanTimes, maxTimes) = processFolderList(
        folders_list, timersListNat, totalTimeIndex
    )
    ...
    timer_totalcomm_index = addTimers(
        meanTimes, timer_exch_data, timer_iconcomm_index
    )
    timer_radcomm_index = subtractTimers(
        meanTimes, timer_nwp_radiation_index, timer_radiaton_comp_index
    )
...
def ocean_timers_statistics(resolution, extent):
    ...
    folders_list = glob.glob(
        "omip_" + resolution + "*" + str(threads) + "threads" + extent
    )
    ...
    (minTimes, meanTimes, maxTimes) = processFolderList(
        folders_list, timersOcean, totalTimeIndex
    )
...
def ocean_timer_core_distribution(distibution_folder, timerName):
    ...
    writeTimerProcsDistribution(
        timerName.strip() + "_min",
        distibution_folder,
        timerFilePrefix,
        timerName,
        minTimeIndex,
...
threads = 12
# oceanAlltests()
ocean_timer_core_distribution(
    "omip_10km_64levels_288nodes_12threads", "exch_data"
)
...
timersListEcham = [" total", " exch_data", " echam_phy"]
...
(mTime, mProc) = getTimeMaxInFiles(
    distibution_folder, timerFilePrefix, timerName, minTimeIndex
)
print("Max of min ", timerName, ": ", mTime, mProc)
(mTime, mProc) = getTimeMaxInFiles(
    distibution_folder, timerFilePrefix, timerName, meanTimeIndex
)
print("Max of mean ", timerName, ": ", mTime, mProc)
(mTime, mProc) = getTimeMaxInFiles(
    distibution_folder, timerFilePrefix, timerName, maxTimeIndex
)
print("Max of max ", timerName, ": ", mTime, mProc)

writeTimerProcsDistribution(
    "2nodes_min_radiaton_comp",
    distibution_folder,
    timerFilePrefix,
    timerName,
    minTimeIndex,
...
timer_remaincomm_index = subtractTimers(
    meanTimes, timer_exch_data, timer_nhsolveexch_index
)
...
(min_minTimes, mean_minTimes, max_minTimes) = processFolderList(
    FoldersList_strong_scale, timersListComm, minTimeIndex
)
(min_meanTimes, mean_meanTimes, max_meanTimes) = processFolderList(
    FoldersList_strong_scale, timersListComm, meanTimeIndex
)
(min_maxTimes, mean_maxTimes, max_maxTimes) = processFolderList(
    FoldersList_strong_scale, timersListComm, maxTimeIndex
)
...
coresList_jitter = [32, 64, 128, 256, 512, 1024]

timersList_jitter = [" calculate ", " mpi_barrier ", " mpi_barrier_init"]


calculateTimer = 0


(min_minTimes, min_meanTimes, min_maxTimes) = processFolderList(
    FoldersList_jitter, timersList_jitter, minTimeIndex
)
(mean_minTimes, mean_meanTimes, mean_maxTimes) = processFolderList(
    FoldersList_jitter, timersList_jitter, meanTimeIndex
)
(max_minTimes, max_meanTimes, max_maxTimes) = processFolderList(
    FoldersList_jitter, timersList_jitter, maxTimeIndex
)
...
writeTimerProcsDistribution(
    "minCalculation", "timers", "timer.", " calculate ", minTimeIndex
)
writeTimerProcsDistribution(
    "meanCalculation", "timers", "timer.", " calculate ", meanTimeIndex
)
writeTimerProcsDistribution(
    "maxCalculation", "timers", "timer.", " calculate ", maxTimeIndex
)
