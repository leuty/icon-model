...
**************************************************************************
Function      : compute_full_level_pressure_on_ml
...
OneLineDesc   : Computes full level pressure on model levels

Description   : Computes full level pressure on model levels.
...
Return Value  : output (default='z_out.grib')
                A fieldset of full level pressure on model levels

Dependencies  : None

Example Usage :
                compute_full_level_pressure_on_ml.py lnsp.grb
"""
...
def parse_args():
    ...
    parser.add_argument(
        "-l", "--levelist", help="levelist to store", default="all"
    )
    parser.add_argument(
        "-o", "--output", help="name of the output file", default="pres.grb"
    )
    parser.add_argument(
        "lnsp",
        metavar="lnsp.grib",
        type=str,
        help=(
            "grib file with geopotential(z) and Logarithm"
    ...
    if args.levelist == "all":
        ...
        if "by" in args.levelist.lower():
            ...
            args.levelist = args.levelist.split("/")
            args.levelist = list(
                range(int(args.levelist[0]), int(args.levelist[2]) + 1)
            )
    else:
...
def check_max_level(idx, values):
    ...
    if max_level != values["nlevels"]:
        print(
            "%s [WARN] total levels should be: %d but it is %d"
...
def get_surface_pressure(idx):
    ...
    if codes_get(gid, "gridType", str) == "sh":
        print(
            "%s [ERROR] fields must be gridded, not spectral" % sys.argv[0],
            file=sys.stderr,
        )
        sys.exit(1)
    # surface pressure
...
def get_pf_levs(values, level):
    ...
    pf_lev = 0.5 * (ph_lev + ph_levplusone)
    return pf_lev, ph_levplusone
...
def compute_p_level(idx, lev, values):
    ...
    # compute the pressures (pf_lev = full-levels)
    pf_lev, ph_levplusone = get_pf_levs(values, lev)
