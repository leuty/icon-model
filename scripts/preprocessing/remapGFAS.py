...
gribNmbDict = {"ocfire": "var90", "bcfire": "var91", "so2fire": "var102"}
...
def getFilesFromList(this_list):
    ...
    for this_file in this_list:
        if (
            this_file.split("_")[0] == "gfas"
            and this_file.split(".")[-1] == "grib1"
        ):
...
opts, args = getopt.getopt(
    sys.argv[1:], "hi:g:w", ["ifile=", "--targetgrid=", "--recalc-weights"]
)
for opt, arg in opts:
    if opt == "-h":
        print(
            "Example: remapEdgar.py -i <inputfile1> -i <inputfile2> ... -g <targetgrid>"
        )
        print("Full list of options:")
        print("-h : help")
        print(
            "-i : input file (multiple files can be added by separate -i entries"
        )
        print(
            "-g : target grid (multiple files can be added by separate -g entries)"
        )
...
for targetgrid in targetgrids:
    ...
    if recalcWeights or not os.path.exists(REMAP_WEIGHTS):
        ...
        print(
            "File "
            + REMAP_WEIGHTS
            + " exists, no recalculation of remapping weights"
        )
